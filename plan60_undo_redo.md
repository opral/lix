...
- 2026-03-18 17:23:34 PDT: Closed the CLI path-extension mismatch by enforcing `.lix` at the shared db helper boundary. `lix init`, explicit `--path`, and output-path preparation now reject non-`.lix` paths before touching the filesystem, and added CLI unit tests to lock that behavior in place.
- 2026-03-18 17:34:00 PDT: Broadened the internal-access guard from DML-only writes to mutating DDL against `lix_internal_*` relations as well. `DROP TABLE`, `ALTER TABLE`, `CREATE INDEX`, `TRUNCATE`, and trigger creation/removal on internal tables now fail fast with `LIX_ERROR_INTERNAL_TABLE_ACCESS_DENIED`, and added focused execute regressions for `DROP TABLE`, `ALTER TABLE`, and `CREATE TRIGGER`.
