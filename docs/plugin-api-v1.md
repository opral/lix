...
there is no compatibility adapter. The selected author API, universal entity
...
- durable semantic entities, which remain merge authority;
...
emits a streamed whole-file replacement, entity mutations, and state updates. The host validates the
...
Every entity input and output uses the same bounded entity-page envelope.
...
- exact byte reconstruction and complete semantic entities;
