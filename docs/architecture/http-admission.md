...
`GET /lix/v1/{repository UUID}/admission` (normalized from the public `/lix/{UUID}` locator), with `lix-sync-protocol-version: 20`, returns authenticated metadata:

```
{ repositoryId, principalId, protocolEpoch: 20, storageEpoch: 81 }
```
