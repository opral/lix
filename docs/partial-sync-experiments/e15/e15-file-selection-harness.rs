...
async fn run(case: Case, snapshot: &[u8], sample: usize, rtt: u64) {
    ...
    let (file_sql, file_params) = match file_lookup.as_str() {
        "path" => ("SELECT id,path FROM lix_file WHERE path='/selected.txt'", vec![]),
        "id" => ("SELECT id,path FROM lix_file WHERE id=$1", vec![Value::Text(file.clone())]),
        "missing_path" => ("SELECT id,path FROM lix_file WHERE path='/absent-selected.txt'", vec![]),
        "missing_id" => ("SELECT id,path FROM lix_file WHERE id=$1", vec![Value::Text(uuid::Uuid::nil().to_string())]),
        "prefix" => ("SELECT id,path FROM lix_file WHERE path LIKE '/selected%' ORDER BY path,id", vec![]),
