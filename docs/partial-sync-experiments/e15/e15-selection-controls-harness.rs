...
async fn run(case: Case, snapshot: &[u8], sample: usize, rtt: u64) {
    ...
    let (file_sql, file_params) = match file_lookup.as_str() {
        "path" => ("SELECT id,path FROM lix_file WHERE path='/selected.txt'".to_owned(), vec![]),
        ...
        "prefix" => ("SELECT id,path FROM lix_file WHERE path LIKE '/selected%' ORDER BY path,id".to_owned(), vec![]),
        ...
        "id_batch16" | "id_batch256" => {
            ...
            let ids = authority.execute(
                &format!("SELECT id FROM lix_file WHERE path='/selected.txt' OR path LIKE '/unrelated-%' ORDER BY path LIMIT {count}"),
                &[],
            ).await.unwrap();
