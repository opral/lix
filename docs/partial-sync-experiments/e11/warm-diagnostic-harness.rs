...
async fn fixture(case: Case, path: &PathBuf) {
    ...
    put(&authority, "/selected.txt", b"initial".to_vec()).await;
    for chunk in (0..case.width).collect::<Vec<_>>().chunks(500) {
        let values = chunk
            .iter()
            .map(|i| format!("('/unrelated-{i}.txt', $1)"))
            .collect::<Vec<_>>()
            .join(",");
        authority
            .execute(
...
async fn run(case: Case, snapshot: &[u8], sample: usize, rtt: u64) {
    ...
    let authority = open_lix()
        ...
        .unwrap();
    let file = authority
        .execute("SELECT id FROM lix_file WHERE path='/selected.txt'", &[])
        .await
        .unwrap()
        .rows()[0]
        .get::<String>("id")
    ...
    replica
        .execute(
            "SELECT id,path FROM lix_file WHERE path='/selected.txt'",
            &[],
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        native_requests(&diff(&probe.snapshot(), &before)),
        0,
        "covered query must be local offline"
    );
    let mut warm_repeats_us = vec![warm_us];
    let mut warm_repeats_network = Vec::new();
    for _ in 0..4 {
        let before = probe.snapshot();
        let started = Instant::now();
        let repeated = replica.execute(&sql, &params).await.unwrap();
        warm_repeats_us.push(started.elapsed().as_micros());
        assert_eq!(repeated.rows(), expected.rows());
        let network = diff(&probe.snapshot(), &before);
        assert_eq!(native_requests(&network), 0, "repeated covered query must stay local");
        warm_repeats_network.push(network);
    }
