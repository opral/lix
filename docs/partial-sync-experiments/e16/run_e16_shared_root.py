...
p=Path('/root/repos/history-sync-research');w=Path('/root/repos/lix-partial-sync-shared-root-reader')
while Path('/proc/354413').exists():time.sleep(5)
assert 'E16 pushdown controls and broad gates complete' in (p/'e16-pushdown-orchestrator.log').read_text()
...
with (p/'e16-shared-root-nextest.log').open('w') as log:
...
patch=subprocess.check_output(['git','diff','0b46fb063','--','packages/lix'],cwd=w);(p/'e16-shared-root-engine.patch').write_bytes(patch);print('Frozen shared root '+hashlib.sha256(patch).hexdigest(),flush=True)
...
try:
 ...
 with (p/'e16-shared-root-build.jsonl').open('w') as out,(p/'e16-shared-root-build.log').open('w') as err:
 ...
 subprocess.run([sys.executable,str(p/'save_build_binary.py'),str(p/'e16-shared-root-build.jsonl'),str(w),str(p/'e16-shared-root-candidate-opt2')],check=True)
finally:h.write_bytes(original)
print('Shared root binary built; canonical harness restored',flush=True)
...
for name,baseline,cases,rtt in [('vs-exact','e16-exact-candidate-opt2','deep4,deep16,deep64',0),('vs-accepted-rtt0','e16-deep-baseline-opt2','dense64,dirs1600,wide16000,deep4,deep16,deep64',0),('vs-accepted-rtt25','e16-deep-baseline-opt2','dense64,dirs1600,wide16000,deep4,deep16,deep64',25)]:
 output=p/f'e16-shared-root-{name}.jsonl';assert not output.exists()
 with (p/f'e16-shared-root-{name}-runner.log').open('w') as log:
  subprocess.run([sys.executable,str(p/'paired_file_selection.py'),str(p/baseline),str(p/'e16-shared-root-candidate-opt2'),'--pairs','10','--cases',cases,'--rtt',str(rtt),'--fixtures',str(p/'fixtures'),'--output',str(output)],env=env,stdout=log,stderr=subprocess.STDOUT,check=True)
 print(name+' pairs complete',flush=True)
env=dict(os.environ,CARGO_TARGET_DIR='/tmp/lix-transparent-target',CARGO_BUILD_JOBS='8',CARGO_INCREMENTAL='0')
for name,cmd in [('doctest',['cargo','test','-p','lix','--doc']),('fmt',['cargo','fmt','-p','lix','--','--check'])]:
 with (p/f'e16-shared-root-{name}.log').open('w') as log:result=subprocess.run(cmd,cwd=w,env=env,stdout=log,stderr=subprocess.STDOUT)
 print(name+': '+str(result.returncode),flush=True);assert result.returncode==0,name
print('E16 shared root controls and broad gates complete',flush=True)
