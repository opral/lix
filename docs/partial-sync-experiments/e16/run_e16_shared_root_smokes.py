...
for mode in ['id','missing_id','id_batch16','id_batch256','path','missing_path','prefix','invalid_id']:
 output=p/f'e16-shared-root-{mode}-smoke.jsonl';assert not output.exists()
 ...
 with output.open('w') as out,(p/f'e16-shared-root-{mode}-smoke.log').open('w') as err:
  subprocess.run([str(p/'e16-shared-root-candidate-opt2')],env=env,stdout=out,stderr=err,check=True)
