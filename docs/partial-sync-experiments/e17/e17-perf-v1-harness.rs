...
async fn run(case: Case, snapshot: &[u8], sample: usize, rtt: u64) {
    ...
    let warmup_ms: u64 = std::env::var("LIX_PROFILE_CPU_WARMUP_MS").unwrap_or("0".into()).parse().unwrap();
    ...
    if let Some(c) = control.as_mut() { c.command("disable"); }
