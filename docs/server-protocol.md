...
- `POST /lix/v1/{lix_id}/sync/replica/replace` durably retires an inaccessible
  replica before its replacement opens. Clients send a stable `lix-replica-id`
  header; retired identities cannot publish again. See [Replica replacement](./replica-replacement.md) for ordering, retained local work, and ownership requirements.
