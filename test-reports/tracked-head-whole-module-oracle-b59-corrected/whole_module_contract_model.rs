//! Dependency-free publication/read model for the TrackedHead hard cut.
...
impl Plan {
    pub const fn no_read_no_durable_work(cohort: Cohort, decision: Decision) -> Self {
        Self {
            cohort,
            decision,
            coherent_views: 0,
    ...
    pub const fn read_fail_closed(cohort: Cohort, decision: Decision) -> Self {
        Self {
            coherent_views: 1,
            ..Self::no_read_no_durable_work(cohort, decision)
        }
    }
...
pub fn validate(plan: Plan) -> Result<(), &'static str> {
    ...
    match plan.decision {
        ...
        Decision::Unsupported | Decision::FailClosed
            if plan.coherent_views == 1
                && plan.prepared_publications == 0
                && plan.storage_plans == 0
                && plan.prepared_writes == 0
...
mod tests {
    ...
    #[test]
    fn noop_has_no_view_or_durable_work() {
        assert!(validate(Plan::no_read_no_durable_work(Cohort::Init, Decision::Noop)).is_ok());
    }

    #[test]
    fn unsupported_has_one_validation_view_and_zero_writes() {
        assert!(validate(Plan::read_fail_closed(Cohort::Gc, Decision::Unsupported)).is_ok());
    }

    #[test]
    fn corruption_has_one_validation_view_and_zero_writes() {
        assert!(validate(Plan::read_fail_closed(Cohort::Schema, Decision::FailClosed)).is_ok());
    }
    ...
    fn corruption_fallback_is_rejected() {
        let mut plan = Plan::read_fail_closed(Cohort::SqlWorkingDiff, Decision::FailClosed);
