...
the pinned baseline. In this v3 successor, that file is generated by running
the documented `source_gate.sh` command against the immutable cd91 checkout;
its exit status remains 1 and its bytes are covered by `SHA256SUMS`.
