...
#[macro_export]
macro_rules! simulation_test2 {
    ($name:ident, |$sim:ident| $body:expr) => {
        $crate::simulation_test2!(
            $name,
            options =
                $crate::support::simulation_test::engine2::Engine2SimulationOptions::default(),
            |$sim| $body
        );
    };
    ...
    (@single $name:ident, $simulation:ident, $mode:ident, $options:expr, |$sim:ident| $body:expr) => {
        paste::paste! {
                ...
                fn [<$name _ $simulation>]() {
                    let simulation_mode =
                        $crate::support::simulation_test::engine2::Engine2SimulationMode::$mode;
                    let simulation_name = stringify!($simulation);
                    let timeout_secs = std::env::var("LIX_SIMULATION_TEST_TIMEOUT_SECS")
                        .ok()
                        .and_then(|raw| raw.parse::<u64>().ok())
                            ...
                            let run_result =
                                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                                    let runtime = tokio::runtime::Builder::new_current_thread()
                                        ...
                                        .expect("failed to build tokio runtime");
                                    runtime.block_on(async {
                                        $crate::support::simulation_test::engine2::run_single_simulation_test(
                                            simulation_mode,
                                            $options,
                                            case_id,
                                            |$sim| $body,
                    ...
                    match result_rx.recv_timeout(std::time::Duration::from_secs(timeout_secs)) {
                        Ok(Ok(())) => {
                            thread.join().expect(concat!(
                                stringify!($name),
                                " simulation_test2 thread panicked"
                            ));
                        }
                        Ok(Err(payload)) => {
                            let _ = thread.join();
                        ...
                        Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                            panic!(
                                "simulation_test2 timed out after {}s (simulation={}, case={})",
                                timeout_secs, simulation_name, case_id
                            );
                        }
                        Err(std::sync::mpsc::RecvTimeoutError::Disconnected) => {
                            ...
                            panic!(
                                "simulation_test2 thread exited without reporting result (simulation={}, case={})",
                                simulation_name, case_id
                            );
                        }
                    }
