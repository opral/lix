    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (\
             ...
             'lix_deterministic_mode', 'lix_key_value', NULL, 'global', NULL, '{\"key\":\"lix_deterministic_mode\",\"value\":{\"enabled\":true}}', '1'\
             )", &[])
        .await
        .unwrap();
}
...
simulation_test!(insert_applies_cel_default, |sim| async move {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'cel_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await
        .unwrap();
...
simulation_test!(insert_uses_json_default_fallback, |sim| async move {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'json_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await
        .unwrap();
...
simulation_test!(insert_x_lix_default_overrides_default, |sim| async move {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'override_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await
        .unwrap();
...
simulation_test!(insert_does_not_override_explicit_null, |sim| async move {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'null_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\",\"status\":null}', '1')", &[])
        .await
        .unwrap();
...
simulation_test!(update_does_not_backfill_defaults, |sim| async move {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'update_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await
        .unwrap();

    engine
        .execute(
            "UPDATE lix_state_by_version SET snapshot_content = '{\"name\":\"Renamed\"}' WHERE schema_key = 'update_default_schema' AND entity_id = 'entity-1' AND file_id IS NULL AND version_id = lix_active_version_id()", &[])
        .await
        .unwrap();
...
async fn run_insert_applies_uuid_function_default(sim: SimulationArgs) {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'uuid_fn_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await
        .unwrap();
...
async fn run_insert_applies_timestamp_function_default(sim: SimulationArgs) {
    ...
    engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'timestamp_fn_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await
        .unwrap();
...
simulation_test!(insert_fails_on_unknown_cel_variable, |sim| async move {
    ...
    let result = engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version) VALUES ('entity-1', 'unknown_var_default_schema', NULL, lix_active_version_id(), NULL, '{\"name\":\"Sample\"}', '1')", &[])
        .await;
...
simulation_test!(
    ...
    |sim| async move {
        ...
        .register_schema(
            &serde_json::from_str::<serde_json::Value>(
                r#"{"x-lix-key":"shared_default_outcome_schema","x-lix-version":"1","x-lix-primary-key":["/id"],"type":"object","properties":{"id":{"type":"string"},"name":{"type":"string"},"slug":{"type":"string","x-lix-default":"name + '-slug'"}},"required":["id","name"],"additionalProperties":false}"#,
            )
            .unwrap(),
        )
        .await
        ...
        engine
        .execute(
            "INSERT INTO lix_state_by_version (entity_id, schema_key, file_id, version_id, plugin_key, snapshot_content, schema_version, global) VALUES ('state-row', 'shared_default_outcome_schema', NULL, 'global', NULL, '{\"id\":\"state-row\",\"name\":\"Sample\"}', '1', true)",
            &[],
        )
        .await
        .unwrap();
