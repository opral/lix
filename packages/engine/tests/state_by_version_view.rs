...
simulation_test!(
    ...
    |sim| async move {
        ...
        insert_version(&engine, "version-a").await;
        insert_version(&engine, "version-b").await;
        insert_state_row(&engine, "entity-upd-multi", "version-a", "{\"value\":\"before-a\"}")
            .await;
        insert_state_row(&engine, "entity-upd-multi", "version-b", "{\"value\":\"before-b\"}")
            .await;
...
simulation_test!(
    ...
    |sim| async move {
        ...
        insert_version(&engine, "version-a").await;
        insert_version(&engine, "version-b").await;
        insert_state_row(&engine, "entity-del-multi", "version-a", "{\"value\":\"before-a\"}")
            .await;
        insert_state_row(&engine, "entity-del-multi", "version-b", "{\"value\":\"before-b\"}")
            .await;
