...
#[tokio::test]
#[ignore = "prints log12 physical layout storage-size reference rows"]
async fn log12_physical_layout_storage_accounting() {
...
fn raw_sqlite_storage_bytes(rows: &[PointerRow]) -> u64 {
    ...
    let db_path = dir.path().join("log12-physical-layout.sqlite");
