...
pub(crate) async fn execute_read_statement_in_session_from_parsed(
    ...
    // The plan owns a cheap clone of the batch-scoped session while execution
    // materializes the full result. Callers must keep `ReadSqlSession` inside
    // the storage-read closure that backs its providers.
