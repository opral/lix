...
pub(super) async fn register_lix_directory_by_version_provider(
    session: &SessionContext,
    surface_name: &str,
    live_state: Arc<dyn LiveStateReader>,
    version_ref: Arc<dyn VersionRefReader>,
...
pub(super) async fn register_lix_directory_by_version_write_provider(
    session: &SessionContext,
    surface_name: &str,
    ...
    session
        .register_table(
            surface_name,
            Arc::new(LixDirectoryProvider::by_version_with_write(write_ctx)),
        )
        .map_err(datafusion_error_to_lix_error)?;
    Ok(())
}

pub(super) async fn register_lix_directory_active_write_provider(
    session: &SessionContext,
    surface_name: &str,
    write_ctx: SqlWriteContext,
) -> Result<(), LixError> {
...
pub(super) fn lix_directory_by_version_schema() -> SchemaRef {
