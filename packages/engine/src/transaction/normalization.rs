...
pub(crate) fn remember_pending_registered_schema(
    ...
    let (domain, key, schema) =
        pending_registered_schema(snapshot, domain, schema_catalog.snapshot())?;
    reject_visible_generated_surface_collision(&key, &schema_catalog.snapshot().schema_jsons())?;
    schema_catalog.insert_schema_for_domain(domain, key, schema)?;
    Ok(())
}

pub(crate) fn remember_pending_registered_schemas<'a>(
    snapshots: impl IntoIterator<Item = (Option<&'a JsonValue>, Domain)>,
    schema_catalog: &mut TransactionCatalog,
) -> Result<(), LixError> {
    ...
    let mut visible_schemas = schema_catalog.snapshot().schema_jsons();
    visible_schemas.extend(pending.iter().map(|(_, _, schema)| schema).cloned());
    for (_, key, _) in &pending {
        reject_visible_generated_surface_collision(key, &visible_schemas)?;
    }
    schema_catalog.insert_schemas_for_domains(pending)?;
    Ok(())
}

fn pending_registered_schema(
    snapshot: Option<&JsonValue>,
    domain: Domain,
    schema_catalog: &crate::catalog::CatalogSnapshot,
) -> Result<(Domain, SchemaKey, JsonValue), LixError> {
...
pub(crate) fn reject_visible_generated_surface_collision(
    key: &SchemaKey,
    visible_schemas: &[JsonValue],
) -> Result<(), LixError> {
    let visible_keys = visible_schemas
        .iter()
        .map(crate::schema::schema_key_from_definition)
        .collect::<Result<Vec<_>, LixError>>()?;
    let Some(surface_name) = PublicCatalog::generated_surface_collision_for_schema_key(
        &key.schema_key,
        visible_keys.iter().map(|key| key.schema_key.as_str()),
    ) else {
        return Ok(());
    };
    Err(LixError::new(
        LixError::CODE_SCHEMA_DEFINITION,
        format!(
            "schema '{}' would generate public SQL surface '{surface_name}', which is already generated by another visible schema",
            key.schema_key
        ),
    )
    .with_hint(
        "Choose a schema key whose base, `_by_branch`, and `_history` names do not overlap another schema's generated names.",
    ))
}
