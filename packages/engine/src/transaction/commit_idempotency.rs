use crate::transaction::PersistenceStatementSink;
use crate::{LixBackend, LixBackendTransaction, LixError, Value};

pub(crate) async fn init_commit_idempotency_storage(
    ...
    crate::backend::execute_ddl_batch(backend, "commit", &statement_refs).await
}
...
pub(crate) fn append_commit_idempotency_row(
    sink: &mut impl PersistenceStatementSink,
    write_lane: &str,
    idempotency_key: &str,
    idempotency_kind: &str,
    idempotency_value: &str,
    ...
    sink.push_persistence_statement(
