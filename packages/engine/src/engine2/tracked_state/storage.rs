...
use crate::engine2::tracked_state::codec::PendingChunkWrite;
use crate::engine2::tracked_state::tree_types::{TrackedStateRootId, TRACKED_STATE_HASH_BYTES};
use crate::LixError;
...
pub(crate) async fn read_many_chunks(
    store: &mut impl KvStore,
    hashes: &[[u8; TRACKED_STATE_HASH_BYTES]],
) -> Result<Vec<([u8; TRACKED_STATE_HASH_BYTES], Vec<u8>)>, LixError> {
    let mut out = Vec::with_capacity(hashes.len());
    for hash in hashes {
        if let Some(data) = read_chunk(store, hash).await? {
            out.push((*hash, data));
        }
    }
    Ok(out)
...
pub(crate) fn verify_chunk_hash(
    ...
    let actual = crate::engine2::tracked_state::codec::hash_bytes(bytes);
...
mod tests {
    ...
    async fn chunk_roundtrips_through_kv_storage() {
        ...
        let chunk = PendingChunkWrite {
            hash: crate::engine2::tracked_state::codec::hash_bytes(&data),
            data: data.clone(),
        };
