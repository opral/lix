...
pub(crate) fn encode_key(key: &TrackedStateKey) -> Vec<u8> {
    ...
    push_sized_bytes(&mut out, key.entity_id.as_bytes());
    out
}
...
pub(crate) fn decode_key(bytes: &[u8]) -> Result<TrackedStateKey, LixError> {
    ...
    let entity_id = read_sized_string(bytes, &mut cursor, "entity_id")?;
    if cursor != bytes.len() {
        return Err(LixError::new(
            "LIX_ERROR_UNKNOWN",
...
pub(crate) fn compare_encoded_key_to_key(encoded: &[u8], key: &TrackedStateKey) -> Ordering {
    encoded.cmp(encode_key(key).as_slice())
}
