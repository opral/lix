...
fn visible_directory_descriptor_rows_for_pairs_sql(
    ...
    let requested_rows = pairs
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let untracked_table = quote_ident(&tracked_relation_name(FILESYSTEM_DIRECTORY_SCHEMA_KEY));
    ...
    format!(
        ...
        tracked_hidden_expr = tracked_hidden_expr,
        untracked_table = untracked_table,
        tracked_table = tracked_relation_name(FILESYSTEM_DIRECTORY_SCHEMA_KEY),
        version_id = escape_sql_string(version_id),
        file_id = escape_sql_string(FILESYSTEM_DESCRIPTOR_FILE_ID),
    )
}
...
fn visible_file_descriptor_rows_for_triplets_sql(
    ...
    let requested_rows = triplets
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let untracked_table = quote_ident(&tracked_relation_name(FILESYSTEM_FILE_SCHEMA_KEY));
    ...
    format!(
        ...
        tracked_hidden_expr = tracked_hidden_expr,
        untracked_table = untracked_table,
        tracked_table = tracked_relation_name(FILESYSTEM_FILE_SCHEMA_KEY),
        version_id = escape_sql_string(version_id),
        file_id = escape_sql_string(FILESYSTEM_DESCRIPTOR_FILE_ID),
