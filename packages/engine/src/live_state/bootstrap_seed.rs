...
impl<'engine, 'tx> InitExecutor<'engine, 'tx> {
    ...
    async fn ensure_seeded_system_directory(
        ...
        let table_name = quote_ident(&tracked_relation_name("lix_directory_descriptor"));
    ...
    pub(crate) async fn seed_lix_id(&mut self) -> Result<(), LixError> {
        let table = tracked_relation_name(key_value_schema_key());
    ...
    pub(crate) async fn seed_boot_account(&mut self) -> Result<(), LixError> {
        ...
        let bootstrap_commit_id = self.load_global_version_commit_id().await?;
        let account_table = tracked_relation_name(account_schema_key());
    ...
    pub(crate) async fn seed_materialized_version_descriptor(
        ...
        let table = tracked_relation_name(version_descriptor_schema_key());
        let check_sql = format!(
            "SELECT 1 \
             FROM {table} \
             WHERE schema_key = '{schema_key}' \
    ...
    pub(crate) async fn seed_materialized_version_ref(
        ...
        let table = tracked_relation_name(version_ref_schema_key());
        let check_sql = format!(
            "SELECT 1 \
             FROM {table} \
             WHERE schema_key = '{schema_key}' \
...
fn normalized_seed_values(
    ...
    Ok(normalized_values_for_schema(schema_key, None, snapshot_content)?
        .into_iter()
        .collect())
}
