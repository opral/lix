...
pub(crate) async fn apply_commit_projections_best_effort_in_transaction(
    ...
    if let Err(_mirror_error) = apply_committed_version_ref_mirrors_in_transaction(
        transaction,
        &receipt.updated_version_refs,
    )
    .await
    {
        // Compatibility mirrors are disposable derived state. They must not
        // block canonical commit durability or affect projection readiness.
    }
    ...
    if let Err(_projection_error) =
        crate::live_state::rebuild_scope_in_transaction(transaction, &rebuild_request).await
    {
        crate::live_state::mark_needs_rebuild_at_canonical_watermark_in_transaction(
            transaction,
            &receipt.canonical_watermark,
...
async fn apply_committed_version_ref_mirrors_in_transaction(
    transaction: &mut dyn LixBackendTransaction,
...
mod tests {
    use super::{
        apply_canonical_receipt_in_transaction, catch_up_live_state_to_watermark,
    ...
    async fn apply_canonical_receipt_routes_commit_boundary_through_projection_layer() {
        let mut transaction = FakeTransaction {
            ...
            latest_watermark: Some(("change-1".to_string(), "2026-03-15T01:02:02Z".to_string())),
