...
fn build_lix_active_version_view_query() -> Query {
    let selection = and_expr(
        and_expr(
            and_expr(
                and_expr(
                    column_eq_text("schema_key", active_version_schema_key()),
                    column_eq_text("file_id", active_version_file_id()),
                ),
                column_eq_text("version_id", active_version_storage_version_id()),
            ),
    ...
    select_query_from_table(
        vec![
            aliased_column_select_item("entity_id", "id"),
            aliased_select_item(
                lix_json_text_expr("snapshot_content", "version_id"),
                "version_id",
            ),
