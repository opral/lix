...
#[derive(Default)]
pub(crate) struct HistoryPushdown {
    change_predicates: Vec<String>,
    requested_predicates: Vec<String>,
    cse_predicates: Vec<String>,
}

pub(crate) fn rewrite_lix_state_history_query(query: Query) -> Result<Option<Query>, LixError> {
    rewrite_query_with_select_rewriter(query, &mut rewrite_lix_state_history_select)
}

fn rewrite_lix_state_history_select(
    select: &mut Select,
    changed: &mut bool,
) -> Result<(), LixError> {
    ...
    for table in &mut select.from {
        rewrite_lix_state_history_table_with_joins(
            table,
...
fn rewrite_lix_state_history_table_with_joins(
    ...
    rewrite_lix_state_history_table_factor(
        &mut table.relation,
        selection,
        allow_unqualified,
        count_fast_path,
...
fn rewrite_lix_state_history_table_factor(
    ...
    match relation {
            ...
            if object_name_matches(name, LIX_STATE_HISTORY_VIEW_NAME) =>
        {
            ...
            let derived_alias = alias
                .clone()
                .or_else(|| Some(default_alias(LIX_STATE_HISTORY_VIEW_NAME)));
...
pub(crate) fn take_history_pushdown_predicates(
    selection: &mut Option<Expr>,
    relation_name: &str,
    allow_unqualified: bool,
) -> HistoryPushdown {
...
pub(crate) fn build_lix_state_history_view_query(
...
#[cfg(test)]
mod tests {
    use super::rewrite_lix_state_history_query;
    ...
    fn pushes_history_schema_and_root_commit_filters_into_ctes_for_count_fast_path() {
        ...
        let rewritten = rewrite_lix_state_history_query(query)
