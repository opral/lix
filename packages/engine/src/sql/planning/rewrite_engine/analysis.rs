...
pub(crate) fn active_version_from_mutations(
    ...
    for mutation in mutations.iter().rev() {
        ...
        let snapshot = mutation.snapshot_content.as_ref().ok_or_else(|| LixError { code: "LIX_ERROR_UNKNOWN".to_string(), description: "active version mutation is missing snapshot_content".to_string(),
        })?;
        let snapshot_content = serde_json::to_string(snapshot).map_err(|error| LixError { code: "LIX_ERROR_UNKNOWN".to_string(), description: format!("active version mutation snapshot_content invalid JSON: {error}"),
        })?;
...
pub(crate) fn active_version_from_update_validations(
    ...
    for plan in plans.iter().rev() {
        ...
        let snapshot_content = serde_json::to_string(snapshot).map_err(|error| LixError { code: "LIX_ERROR_UNKNOWN".to_string(), description: format!("active version update snapshot_content invalid JSON: {error}"),
        })?;
        return parse_active_version_snapshot(&snapshot_content).map(Some);
    }
