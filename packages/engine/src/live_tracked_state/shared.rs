...
use super::contracts::{ExactTrackedRowRequest, TrackedRow};

pub(super) fn exact_row_constraints(request: &ExactTrackedRowRequest) -> Vec<ScanConstraint> {
    ...
    for property_name in required_columns {
        let Some(column) = access
            ...
            return Err(LixError::new(
                "LIX_ERROR_UNKNOWN",
                &format!(
                    "tracked scan for schema '{}' requested unknown property '{}'",
...
pub(super) fn decode_tracked_row(
    row: &[Value],
    ...
    let created_at = required_text_cell(row, 10, schema_key, "created_at")?;
    let updated_at = required_text_cell(row, 11, schema_key, "updated_at")?;

    let mut values = BTreeMap::new();
    for (offset, column) in selected_columns.iter().enumerate() {
        let value = row.get(12 + offset).ok_or_else(|| {
            LixError::new(
                "LIX_ERROR_UNKNOWN",
                &format!(
...
fn required_text_cell(
    ...
    row.get(index).and_then(text_from_value).ok_or_else(|| {
        LixError::new(
            "LIX_ERROR_UNKNOWN",
            &format!(
                "tracked row for schema '{}' is missing text column '{}'",
...
fn required_bool_cell(
    ...
    row.get(index).and_then(bool_from_value).ok_or_else(|| {
        LixError::new(
            "LIX_ERROR_UNKNOWN",
            &format!(
                "tracked row for schema '{}' is missing boolean column '{}'",
                schema_key, column_name
            ),
        )
    })
