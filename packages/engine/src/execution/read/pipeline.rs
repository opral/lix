...
use crate::{ExecuteResult, LixBackend, LixBackendTransaction, LixError, QueryResult};
...
pub(crate) async fn execute_prepared_read_batch_with_backend(
    ...
    for statement in &prepared.statements {
        let result = execute_prepared_read_statement_with_backend(backend, host, statement).await?;
        results.push(result);
    }
...
async fn execute_prepared_read_statement_with_backend(
    backend: &dyn LixBackend,
    host: &dyn ReadExecutionHost,
    ...
    let result = match &prepared.artifact {
        PreparedReadArtifact::Public(public) => {
            execute_prepared_public_read_artifact_with_backend(backend, host, public).await
        }
        PreparedReadArtifact::Scalar(artifact) => {
            execute_prepared_scalar_read_artifact_with_backend(backend, &artifact.prepared_batch)
                .await
        }
    };
...
async fn execute_prepared_scalar_read_artifact_with_backend(
    backend: &dyn LixBackend,
