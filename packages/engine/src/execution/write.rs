use super::statement_batch::{WriteBatch, WriteStep};
use crate::backend::{PreparedBatch, PreparedStatement};
use crate::{LixBackend, LixBackendTransaction, LixError, QueryResult, TransactionBeginMode};

pub(crate) async fn execute(
    transaction: &mut dyn LixBackendTransaction,
    write_batch: WriteBatch,
) -> Result<QueryResult, LixError> {
    execute_steps(transaction, write_batch).await
}
...
async fn execute_with_backend(
    ...
    let mut transaction = backend
        ...
        .await?;
    let result = execute_steps(transaction.as_mut(), write_batch).await;
    match result {
        Ok(result) => {
...
mod tests {
    ...
    use super::execute_with_backend;
    use crate::execution::WriteBatch;
    ...
    async fn owned_runner_commits_on_success() {
        ...
        execute_with_backend(&backend, write_batch)
            .await
    ...
    async fn owned_runner_rolls_back_on_failure() {
        ...
        let error = execute_with_backend(&backend, write_batch)
            .await
