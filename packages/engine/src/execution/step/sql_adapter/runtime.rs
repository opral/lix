...
use crate::transaction::{
    ...
    normalize_sql_error_with_transaction_and_relation_names, PendingWriteView,
    TransactionWriteDelta, WriteExecutionBindings,
};
...
pub(crate) enum PreparedWriteExecutionRoute<'a> {
    ...
    TransactionWriteDelta(&'a TransactionWriteDelta),
    PublicWriteNoop,
...
impl PreparedWriteExecutionStep {
    ...
    pub(crate) fn route(&self) -> PreparedWriteExecutionRoute<'_> {
        ...
        if let Some(delta) = self.transaction_write_delta.as_ref() {
            return PreparedWriteExecutionRoute::TransactionWriteDelta(delta);
        }
...
pub(crate) async fn execute_prepared_write_execution_step_with_transaction(
    ...
    match step.route() {
        ...
        PreparedWriteExecutionRoute::TransactionWriteDelta(delta) => {
            let execution = delta
                .execute(bindings, transaction, pending_public_commit_session)
                .await?;
