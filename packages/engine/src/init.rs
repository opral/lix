...
///
/// V64 binds one branch/generation authenticated identity tree plus content-addressed variant chunks and tracked HOT cut while
...
const REPOSITORY_PROTOCOL_VALUE: &[u8] = b"immutable-physical-commit-state.v64";
    ...
    StorageImpl: Storage + Clone + Send + Sync + 'static,
{
...

    {
        ...
        let global_untracked_control = crate::live_state::stage_untracked_deltas(
            &read,
            &mut writes,
            GLOBAL_BRANCH_ID,
        ...
        for branch in &plan.branch_controls {
            ...
            if branch.branch_id == GLOBAL_BRANCH_ID {
                control.current_state_revision = global_untracked_control.current_state_revision;
            }
