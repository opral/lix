...
fn parse_first_payload(row: Option<&Vec<Value>>) -> Result<Option<JsonValue>, LixError> {
    ...
    let parsed: JsonValue = serde_json::from_str(&raw).map_err(|err| LixError { code: "LIX_ERROR_UNKNOWN".to_string(), title: "Unknown error".to_string(), description: format!("deterministic mode snapshot_content invalid JSON: {err}"),
    })?;
