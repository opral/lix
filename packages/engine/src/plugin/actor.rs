...
///
/// The semantic root is authority, not a byte digest. Two roots that happen to
/// render identical bytes intentionally produce different observations.
...
impl PluginActorCache {
    ...
    pub(crate) async fn lease_for_transition(
        ...
        {
            Some((guard.document, Arc::clone(&guard.bytes)))
        } else {
            guard
                .history
                .iter()
                ...
                .map(|historical| (historical.document, Arc::clone(&historical.bytes)))
        };
        let Some((observed_document, observed_bytes)) = observed else {
            return Err(stale_observation(
...
impl PluginActorLease {
    ...
    pub(crate) fn accepted_bytes(&self) -> Arc<[u8]> {
        Arc::clone(
            &self
                .guard
                .as_deref()
    ...
    pub(crate) fn observed_bytes(&self) -> Arc<[u8]> {
        Arc::clone(&self.observed_bytes)
    }
        ...
        for revision in 2..=(DEFAULT_MAX_PLUGIN_FILE_HISTORY as u64 + 2) {
            ...
            lease
                .complete_guest_call(
                    ...
                    Arc::<[u8]>::from(format!("version-{revision}").into_bytes()),
