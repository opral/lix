...
/// Recursive v2 plugins retain semantic indexes alongside accepted source
/// bytes. Keep this policy local to v2 so existing v1 components retain their
/// 64 MiB default.
...
fn plugin_v2_wasm_limits(max_memory_bytes: u64) -> Result<WasmLimits, LixError> {
    if max_memory_bytes == 0 {
        return Err(LixError::new(
            ...
            "v2 plugin memory limit must be positive",
        ));
    }
    Ok(WasmLimits {
        max_memory_bytes,
...
fn default_plugin_v2_wasm_limits() -> WasmLimits {
    plugin_v2_wasm_limits(DEFAULT_PLUGIN_V2_MEMORY_BYTES)
        .expect("the default v2 plugin memory limit is positive")
}
...
impl PluginRuntimeHost {
    pub(crate) fn new(wasm_runtime: Arc<dyn WasmRuntime>) -> Self {
        ...
        .expect("default v2 plugin resource limits are valid")
    }
...
mod tests {
    ...
    #[test]
    fn v2_memory_policy_does_not_change_the_v1_default() {
