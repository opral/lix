...
pub(crate) struct BinaryCasGcSweep {
    pub(crate) manifests: Vec<BlobHash>,
    pub(crate) manifest_chunk_keys: Vec<Vec<u8>>,
    pub(crate) chunks: Vec<BlobHash>,
}
...
pub(crate) async fn plan_gc(
    ...
    let manifest_entries = scan_all_entries(store, BINARY_CAS_MANIFEST_SPACE, Vec::new()).await?;
    let mut manifests = BTreeMap::new();
    for (key, value) in manifest_entries {
        let hash = BlobHash::from_bytes(<[u8; 32]>::try_from(key.as_slice()).map_err(|_| {
            LixError::new(
                LixError::CODE_INTERNAL_ERROR,
                "binary CAS contains a manifest with a non-32-byte key",
            )
        })?);
        manifests.insert(hash, decode_binary_cas_manifest(&value)?);
    }

    let manifest_chunk_entries =
        scan_all_entries(store, BINARY_CAS_MANIFEST_CHUNK_SPACE, Vec::new()).await?;
    let mut manifest_chunks = BTreeMap::<BlobHash, Vec<(Vec<u8>, BlobHash)>>::new();
    for (key, value) in manifest_chunk_entries {
        if key.len() != 40 {
            return Err(LixError::new(
                LixError::CODE_INTERNAL_ERROR,
                "binary CAS contains a manifest chunk with a non-40-byte key",
            ));
        }
        let blob_hash = BlobHash::from_bytes(
            <[u8; 32]>::try_from(&key[..32]).expect("manifest chunk prefix length checked"),
        );
        let (chunk_hash, _) = decode_binary_cas_manifest_chunk(&value)?;
        manifest_chunks
            .entry(blob_hash)
            .or_default()
            .push((key, BlobHash::from_bytes(chunk_hash)));
    }

    let chunk_keys = scan_all_keys(store, BINARY_CAS_CHUNK_SPACE, Vec::new()).await?;
    let stored_chunks = chunk_keys
        .into_iter()
        .map(|key| {
            <[u8; 32]>::try_from(key.as_slice())
                ...
                .map_err(|_| {
                    LixError::new(
                        LixError::CODE_INTERNAL_ERROR,
                        "binary CAS contains a chunk with a non-32-byte key",
                    )
                })
        })
        .collect::<Result<BTreeSet<_>, _>>()?;
    let presence_keys = scan_all_keys(store, BINARY_CAS_CHUNK_PRESENCE_SPACE, Vec::new()).await?;
    let stored_presence = presence_keys
        .into_iter()
        .map(|key| {
            <[u8; 32]>::try_from(key.as_slice())
                .map(BlobHash::from_bytes)
                .map_err(|_| {
                    LixError::new(
                        LixError::CODE_INTERNAL_ERROR,
                        "binary CAS contains a chunk-presence row with a non-32-byte key",
                    )
                })
        })
        .collect::<Result<BTreeSet<_>, _>>()?;
    ...
    for blob_hash in live_blobs {
        let Some(manifest) = manifests.get(blob_hash) else {
            return Err(LixError::new(
                LixError::CODE_INTERNAL_ERROR,
                format!(
                    "live binary blob '{}' has no CAS manifest",
                    blob_hash.to_hex()
                ),
            ));
        };
        match manifest {
            ...
            BinaryCasManifest::SingleChunk { chunk_hash, .. } => {
                live_chunks.insert(BlobHash::from_bytes(*chunk_hash));
            }
            BinaryCasManifest::Chunked { chunk_count, .. } => {
                let entries = manifest_chunks
                    .get(blob_hash)
                    .map(Vec::as_slice)
                    .unwrap_or(&[]);
                if entries.len() != *chunk_count as usize {
                    return Err(LixError::new(
                        LixError::CODE_INTERNAL_ERROR,
                        format!(
                            "live binary blob '{}' expected {} manifest chunks, found {}",
                            blob_hash.to_hex(),
                            chunk_count,
                            entries.len()
                        ),
                ...
                live_chunks.extend(entries.iter().map(|(_, chunk_hash)| *chunk_hash));
            }
        }
    }
    if let Some(missing) = live_chunks.difference(&stored_chunks).next() {
        return Err(LixError::new(
            LixError::CODE_INTERNAL_ERROR,
            format!("live binary CAS chunk '{}' is missing", missing.to_hex()),
        ));
    }

    let all_chunk_rows = stored_chunks
        .union(&stored_presence)
        .copied()
        .collect::<BTreeSet<_>>();
    Ok(BinaryCasGcSweep {
        manifests: manifests
            .keys()
            .filter(|hash| !live_blobs.contains(hash))
            .copied()
            .collect(),
        manifest_chunk_keys: manifest_chunks
            .into_iter()
            .filter(|(blob_hash, _)| !live_blobs.contains(blob_hash))
            .flat_map(|(_, entries)| entries.into_iter().map(|(key, _)| key))
            .collect(),
        chunks: all_chunk_rows.difference(&live_chunks).copied().collect(),
    })
}

pub(crate) fn stage_gc_sweep(writes: &mut StorageWriteSet, sweep: &BinaryCasGcSweep) {
    for blob_hash in &sweep.manifests {
        writes.delete(BINARY_CAS_MANIFEST_SPACE, key(manifest_key(*blob_hash)));
    }
    for manifest_chunk_key in &sweep.manifest_chunk_keys {
        writes.delete(
            BINARY_CAS_MANIFEST_CHUNK_SPACE,
            key(manifest_chunk_key.clone()),
        );
    }
    for chunk_hash in &sweep.chunks {
        writes.delete(BINARY_CAS_CHUNK_SPACE, key(chunk_key(*chunk_hash)));
        writes.delete(BINARY_CAS_CHUNK_PRESENCE_SPACE, key(chunk_key(*chunk_hash)));
    }
}
...
async fn scan_all_entries(
    ...
    loop {
        ...
        for entry in page.value.entries {
            let Some(value) = full_value(entry.value) else {
                return Err(LixError::new(
                    LixError::CODE_INTERNAL_ERROR,
                    "binary CAS GC scan unexpectedly omitted a value",
                ));
            };
            entries.push((entry.key.0.to_vec(), value.to_vec()));
        }
        if !page.value.has_more || resume_after.is_none() {
            break;
        }
