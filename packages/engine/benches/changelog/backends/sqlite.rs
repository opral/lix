...
#[derive(Clone)]
pub(crate) struct SqliteChangelogBenchBackend {
...
pub(crate) struct SqliteChangelogBenchTransaction {
    connection: Arc<Mutex<Connection>>,
    finalized: bool,
}

impl SqliteChangelogBenchBackend {
    pub(crate) fn tempfile() -> Result<Self, LixError> {
        let temp_dir = Arc::new(TempDir::new().map_err(|error| {
            LixError::new(
                "LIX_ERROR_UNKNOWN",
                format!("sqlite changelog bench tempdir: {error}"),
            )
        })?);
    ...
    fn lock_connection(&self) -> Result<std::sync::MutexGuard<'_, Connection>, LixError> {
        self.connection.lock().map_err(|_| {
            LixError::new(
                "LIX_ERROR_UNKNOWN",
                "sqlite changelog bench connection poisoned",
            )
        })
    }
}
...
#[async_trait]
impl Backend for SqliteChangelogBenchBackend {
    async fn begin_read_transaction(
        ...
        connection
            ...
            .map_err(sqlite_error)?;
        drop(connection);
        Ok(Box::new(SqliteChangelogBenchTransaction {
...
#[async_trait]
impl BackendReadTransaction for SqliteChangelogBenchTransaction {
    async fn get_values(
        &mut self,
...
#[async_trait]
impl BackendWriteTransaction for SqliteChangelogBenchTransaction {
...
impl SqliteChangelogBenchTransaction {
    fn lock_connection(&self) -> Result<std::sync::MutexGuard<'_, Connection>, LixError> {
        self.connection.lock().map_err(|_| {
            LixError::new(
                "LIX_ERROR_UNKNOWN",
...
impl Drop for SqliteChangelogBenchTransaction {
    fn drop(&mut self) {
        if !self.finalized {
...
fn sqlite_error(error: rusqlite::Error) -> LixError {
    LixError::new(
        "LIX_ERROR_UNKNOWN",
        format!("sqlite changelog bench backend: {error}"),
    )
}
