...
const last = await lix.history.entity({
  schema: "function",
  name:   "applyDiscount",
}).last();
...
function highlightCode(code: string) {
  const pattern =
    /(`[^`]*`|"[^"]*"|'[^']*'|\bORDER BY\b|\bGROUP BY\b|\b(?:import|from|const|let|await|async|return|use|SELECT|FROM|JOIN|WHERE|AND|AS|DESC)\b|\b(?:openLix|createBackend|createBetterSqlite3Backend|sqlite|execute|transaction|create_session|createVersion|switchVersion|branch|diff|history|entity|write|commit|file|lix_json_get_text)\b|\b(?:lix_change|lix_file|xlsx_row)\b)/g;
  ...
  return code.split(pattern).map((part, index) => {
    ...
    } else if (/^(import|from|const|let|await|async|return|use|SELECT|FROM|JOIN|WHERE|AND|AS|DESC|ORDER BY|GROUP BY)$/.test(part)) {
      className = "text-[#6d28d9]";
    } else if (/^(openLix|createBackend|createBetterSqlite3Backend|sqlite|execute|transaction|create_session|createVersion|switchVersion|branch|diff|history|entity|write|commit|file|lix_json_get_text)$/.test(part)) {
...
function RustIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 224 224" fill="currentColor" aria-hidden="true">
      <path
        fill="#ce422b"
...
function JsIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <text x="12" y="17" textAnchor="middle" fontSize="11" fontWeight="800" fill="#0a0a0a" fontFamily="Geist Mono, monospace">JS</text>
    </svg>
  );
}
...
function PyIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 2.5c-3.2 0-3 1.5-3 1.5v2H12v.7H6.5s-2.2-.2-2.2 3.4 1.9 3.5 1.9 3.5h2v-1.8s-.1-2 1.9-2h3.7s1.9 0 1.9-1.8V4s.3-1.5-3.7-1.5zM10.2 3.5a.85.85 0 1 1 0 1.7.85.85 0 0 1 0-1.7z" fill="#3776AB" />
      <path d="M12 21.5c3.2 0 3-1.5 3-1.5v-2H12v-.7h5.5s2.2.2 2.2-3.4-1.9-3.5-1.9-3.5h-2v1.8s.1 2-1.9 2H10.2s-1.9 0-1.9 1.8V20s-.3 1.5 3.7 1.5zm1.8-1a.85.85 0 1 1 0-1.7.85.85 0 0 1 0 1.7z" fill="#FFD43B" />
    </svg>
  );
}
  ...
  return (
    <svg width="18" height="16" viewBox="0 0 28 24" aria-hidden="true">
      <g fill="#00ADD8">
        ...
        <text x="14" y="17" fontFamily="Geist Mono, monospace" fontWeight="700" fontSize="11">go</text>
      </g>
    </svg>
  );
}
...
function StarCount() {
  ...
  return <span>Star on GitHub <span className="text-[#6b6b66]">{githubStars.toLocaleString()}</span></span>;
}
...
function CodeBlock({ children, small = false }: { children: ReactNode; small?: boolean }) {
  return (
    <pre
      className={cn(
        fontMono,
...
function SemanticSqlVisual() {
  ...
  return (
    <div>
      <CodeBlock small>{<>
<span className={jsKeyword}>const</span>{` rows = `}<span className={jsKeyword}>await</span>{` `}<span className={jsStrong}>lix</span>.<span className={jsMethod}>execute</span>{`(
    ...
    <div className="mt-5 max-w-[460px]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 rounded-[10px] border border-[#e7e6e1] bg-[#f5f4ee] px-3.5 py-3">
          ...
          <div className={cn(fontMono, "mt-2 flex items-center gap-2 text-[12px] text-[#6b6b66]")}>
...
function BackendLogos() {
  ...
  return (
    <div className={cn(fontMono, "mt-5 flex max-w-[460px] flex-wrap items-center gap-x-5 gap-y-3 text-[12px] text-[#2b2b2b]")}>
...
function LangTabs() {
  ...
  const tabs = [
    { key: "js", label: "JavaScript", Icon: JsIcon, stable: true, href: undefined },
    { key: "rust", label: "Rust", Icon: RustIcon, stable: true, href: undefined },
    { key: "python", label: "Python", Icon: PyIcon, stable: false, href: pythonIssueUrl },
  ...
  return (
    <div className="mt-10 max-w-[1100px] overflow-hidden rounded-[10px] border border-[#e7e6e1] bg-[#fafaf7]">
      <div className="flex items-center justify-between gap-[18px] border-b border-[#e7e6e1] bg-[#f3f2ec] max-md:flex-col max-md:items-stretch max-md:gap-0">
        ...
        <div className={cn(fontMono, "shrink-0 pr-[18px] text-[11.5px] text-[#6b6b66] max-md:border-t max-md:border-[#e7e6e1] max-md:px-4 max-md:py-2.5")}>
          @lix-js/sdk · v0.6.0
        </div>
      </div>
...
function Hero() {
  return (
    <section className={cn(sectionClass, "pt-16 lg:pt-20 lg:pb-16")}>
      ...
      <h1 className={cn(displayClass, "max-w-[1040px] text-[clamp(42px,10.5vw,54px)] md:text-[clamp(50px,5.25vw,66px)] [&_br]:max-md:hidden")}>
        An embeddable version<br />{" "}control system for AI agents.
      </h1>
      ...
      <div className="mt-7 flex flex-wrap items-center gap-3 max-md:items-stretch">
        <Button href={npmUrl}>
          <span className="!text-[#08b5d6] [-webkit-text-fill-color:#08b5d6]">$</span>
      ...
      <div className="mx-auto grid w-full max-w-[1280px] grid-cols-2 px-6 md:px-8 lg:grid-cols-4 lg:px-14">
        {stats.map(([label, sub]) => (
          ...
          >
            ...
            <span className="mt-1 block text-xs leading-[1.35] text-[#6b6b66]">{sub}</span>
          </div>
        ))}
      </div>
    </section>
...
function WhyBuilt() {
  return (
    <section className={cn(sectionClass, "grid gap-10 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-16")}>
      <SectionHeader eyebrow="§01 / why we built it" title={<>Git wasn't designed<br />{" "}to be embedded.</>} />
      <div className="max-w-[740px]">
        <p className="mb-[22px] text-[19px] leading-[1.55] text-[#2b2b2b]">
          ...
          rollback. They need versioning as <em className={cn(fontMono, "not-italic")}>infrastructure</em>, not as a CLI.
        </p>
        <p className="mb-[22px] text-[19px] leading-[1.55] text-[#2b2b2b]">
        ...
        <div className="mb-[22px] grid max-w-[720px] grid-cols-1 gap-2.5 md:grid-cols-2">
          {overhead.map((item) => (
            <div className={cn(fontMono, "flex min-w-0 gap-2 rounded-md border border-[#e7e6e1] px-3.5 py-2.5 text-xs leading-[1.4] text-[#2b2b2b]")} key={item}>
        ...
        <p className="text-[17px] leading-[1.55] text-[#0a0a0a]">
          <strong className="font-medium">Lix is built the other way around.</strong> Sessions, not
          worktrees. A SQL engine you embed, with semantic diffs and branches as
          first-class state - backed by infrastructure you already run.
        </p>
      </div>
    </section>
  );
...
function Inside() {
  return (
    <section className={sectionClass}>
      <SectionHeader eyebrow="§02 / what's inside v0.6" title="What you get with Lix." />
      <div className="mt-12 border-t border-[#e7e6e1]">
        {features.map((feature) => (
          <article
            ...
            key={feature.tag}
          >
            <div className={cn(fontMono, "pt-1.5 text-xs text-[#6b6b66]")}>{feature.tag}</div>
            <div>
              <h3 className="m-0 text-[22px] font-medium tracking-[-0.02em] text-[#0a0a0a]">{feature.title}</h3>
              <p className="mt-2.5 max-w-[460px] text-[15.5px] leading-[1.6] text-[#2b2b2b]">{feature.body}</p>
              {"visual" in feature && feature.visual === "sql-query" ? <AgentQuestionCard /> : null}
              {"visual" in feature && feature.visual === "backend" ? <BackendLogos /> : null}
            </div>
  ...
  return (
    <section className={sectionClass}>
      <div className="mb-12 block gap-8 md:flex md:items-end md:justify-between">
        <SectionHeader eyebrow="§03 / what teams build with lix" title="What you can build." />
        <p className={cn(fontMono, "mt-[18px] max-w-[280px] text-left text-xs leading-normal text-[#6b6b66] md:mt-0 md:text-right")}>
          v0.6 ships the engine.<br />{" "}new shapes land in every release.
        </p>
      </div>
      <div className="grid grid-cols-1 border-l border-t border-[#e7e6e1] md:grid-cols-2">
        {patterns.map((pattern) => (
          <article className="min-w-0 border-b border-r border-[#e7e6e1] px-6 py-[30px] md:p-8 md:pb-9" key={pattern.kind}>
            <div className={cn(fontMono, "text-xs text-[#066f86]")}>{pattern.kind}</div>
            <h3 className="mt-3 text-[26px] font-medium tracking-[-0.02em] text-[#0a0a0a]">{pattern.title}</h3>
            <p className="mt-3 max-w-[480px] text-base leading-[1.55] text-[#2b2b2b]">{pattern.body}</p>
            <div className={cn(fontMono, "mt-[18px] flex flex-wrap gap-2")}>
              {pattern.stack.map((item) => (
                <span className="inline-flex min-h-[26px] items-center rounded-full border border-[#e7e6e1] px-2.5 text-[11.5px] text-[#2b2b2b]" key={item}>
                  {item}
                </span>
              ))}
            </div>
  ...
  return (
    <section className={cn(sectionClass, "grid gap-10 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-16")}>
      ...
      <div className="relative pl-8">
        <div className="absolute bottom-3 left-[9px] top-3 w-px bg-[#d7d5ce]" aria-hidden="true" />
        <RoadmapMilestone title="now / v0.6" subtitle="Ready to embed" items={shipped} shipped />
        <RoadmapMilestone title="v0.7" subtitle="CLI" items={roadmap07} />
        <RoadmapMilestone title="v0.8" subtitle="file plugin API" items={roadmap08} />
        <RoadmapMilestone title="v0.9" subtitle="merge conflicts" items={roadmap09} />
        <RoadmapMilestone title="v0.10" subtitle="working changes" items={roadmap10} />
      </div>
    </section>
  );
}

function RoadmapMilestone({ title, subtitle, items, shipped = false }: { title: string; subtitle: string; items: string[]; shipped?: boolean }) {
  return (
    <div className="relative border-b border-[#e7e6e1] py-6 first:pt-0 last:border-b-0 last:pb-0">
      ...
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h3 className={cn(fontMono, "m-0 text-[11.5px] font-medium uppercase tracking-[0.04em]", shipped ? "text-[#066f86]" : "text-[#6b6b66]")}>{title}</h3>
        <p className={cn(fontMono, "m-0 text-[11.5px] text-[#6b6b66]")}>{subtitle}</p>
      </div>
      <ul className="mt-3.5 grid list-none gap-2.5 p-0 md:grid-cols-2">
        {items.map((item) => (
          <li className={cn("flex gap-2 text-[14.5px] leading-[1.55]", shipped ? "text-[#0a0a0a]" : "text-[#2b2b2b]")} key={item}>
            <span className={shipped ? "text-[#066f86]" : "text-[#6b6b66]"}>{shipped ? "✓" : "○"}</span>
            {item}
          </li>
        ))}
      </ul>
...
function GetStarted() {
  return (
    <section className={cn(sectionClass, "grid items-center gap-10 py-16 lg:grid-cols-2 lg:gap-16 lg:py-24")}>
      <div>
        <SectionHeader
          eyebrow="install"
          title={<span className="block max-w-[540px] text-[clamp(44px,5.2vw,60px)]">Get started<br />{" "}in a minute.</span>}
        />
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3 max-md:items-stretch">
          ...
          <a className={cn(fontMono, focusClass, "text-[13px] text-[#6b6b66] hover:text-[#0a0a0a] max-md:w-full")} href="/docs/what-is-lix">Read the docs →</a>
        </div>
        <div className={cn(fontMono, "mt-[18px] text-xs leading-normal text-[#6b6b66]")}>MIT · TypeScript · Node, Deno, Bun, Tokio, edge runtimes.</div>
      </div>
      <CodeBlock>{<>
        <span className="text-[#6b6b66]"># 1 · install</span>{"\n"}
        $ npm install @lix-js/sdk{"\n\n"}
        <span className="text-[#6b6b66]"># 2 · open</span>{"\n"}
        <span className="text-[#b65aff]">import</span>{" { "}<span className="text-[#2b2b2b]">openLix</span>{" } "}<span className="text-[#b65aff]">from</span>{" "}<span className="text-[#058a3e]">"@lix-js/sdk"</span>;{"\n"}
        <span className="text-[#b65aff]">const</span> lix = <span className="text-[#b65aff]">await</span> <span className="text-[#066f86]">openLix</span>();{"\n\n"}
        <span className="text-[#6b6b66]"># 3 · write & commit</span>{"\n"}
        <span className="text-[#b65aff]">await</span> lix.<span className="text-[#066f86]">file</span>.<span className="text-[#066f86]">write</span>(<span className="text-[#058a3e]">"hello.json"</span>, payload);{"\n\n"}
        <span className="text-[#6b6b66]"># 4 · branch & ask</span>{"\n"}
        <span className="text-[#b65aff]">const</span> b = <span className="text-[#b65aff]">await</span> lix.<span className="text-[#066f86]">branch</span>(<span className="text-[#058a3e]">"draft"</span>);{"\n"}
        <span className="text-[#b65aff]">const</span> diff = <span className="text-[#b65aff]">await</span> lix.<span className="text-[#066f86]">diff</span>({"{ "}from: <span className="text-[#058a3e]">"main"</span>, to: b {"}"});{"\n"}
      </>}</CodeBlock>
    </section>
  );
}
  ...
  return (
    <footer className={cn(fontMono, "mx-auto flex w-full max-w-[1280px] items-end justify-between gap-8 border-t border-[#e7e6e1] px-6 pb-12 pt-9 text-xs text-[#6b6b66] max-md:block lg:px-14 lg:pb-14")}>
      ...
      <nav className="flex flex-wrap justify-end gap-6 text-[13px] max-md:mt-7 max-md:justify-start max-md:gap-4" aria-label="V2 footer navigation">
        ...
        <a href="https://discord.gg/gdMPPWy57R" target="_blank" rel="noopener noreferrer">discord</a>
        <a href="/blog">blog</a>
