...
page_sizes=${PREPARED_CAS_PAGE_SIZES:-1,8,32,64}
...
run_adapter() {
    ...
    (
        ...
        env "${common_env[@]}" \
            timeout 1200 \
            cargo test -p lix --lib "${test_prefix}_${adapter}" \
            -- --exact --nocapture --test-threads=1
    ) 2>&1 | tee "$log"
}

# The implementation must register these private owner-boundary tests.
# A missing test is a hard stop, not a fallback to the standalone model.
run_adapter memory
run_adapter rocksdb
run_adapter slatedb

"$(dirname "$0")/verify_adapter_results.sh" "$results/results.csv" "$files" "$payload_bytes"
