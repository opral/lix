...
pub fn hint_after_init() -> Vec<String> {
    vec![
        ...
        "Store files with: lix sql execute \"INSERT INTO lix_file (path, content) VALUES ('/readme.txt', CAST('hello' AS BYTEA))\"".into(),
    ]
}
...
pub fn hint_blob_in_result(result: &ExecuteResult) -> Vec<String> {
    ...
    if has_blob {
        vec!["Tip: use CAST(content AS TEXT) to view text content".into()]
    } else {
        Vec::new()
    }
}
...
pub fn are_hints_enabled(lix: &crate::db::SqliteLix) -> bool {
