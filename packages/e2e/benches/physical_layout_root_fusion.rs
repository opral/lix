//! EXP-ROOT-FUSION-21: global tree versus fused schema roots.
//!
...
const INLINE_ROOT_CAP: usize = 1 << 10;
const SCHEMA_ROOT_ENTRY_BYTES: usize = 52;
...
impl FusionRepo {
    fn build(rows: Vec<LogicalRow>) -> Self {
        ...
        let entries = schema_root_entries(&subtrees);
        let inline_bytes = 8 + entries.len() * SCHEMA_ROOT_ENTRY_BYTES;
        let (root_bytes, directory) = if inline_bytes <= INLINE_ROOT_CAP {
            (encode_inline_root(&entries), None)
        } else {
            let directory = AuthTree::build(
                entries
                    .iter()
                    .map(|entry| Entry {
                        key: entry.schema.to_vec(),
                        value: encode_schema_root_value(entry),
                    })
                    .collect(),
                DIRECTORY_BINDING,
            );
            let mut root = b"LXFE".to_vec();
            root.extend_from_slice(&directory.root());
            (root, Some(directory))
        };
        let root_id = *blake3::hash(&root_bytes).as_bytes();
        let repo = Self {
        ...
        repo.verify().expect("fused repository verifies");
        repo
    }

    fn verify(&self) -> Result<(), &'static str> {
        if blake3::hash(&self.root_bytes).as_bytes() != &self.root_id {
            return Err("repository root ObjectId mismatch");
        }
        let expected = schema_root_entries(&self.subtrees);
        let should_inline = 8 + expected.len() * SCHEMA_ROOT_ENTRY_BYTES <= INLINE_ROOT_CAP;
        let actual = match (self.root_bytes.get(..4), &self.directory, should_inline) {
            (Some(b"LXFI"), None, true) => decode_inline_root(&self.root_bytes)?,
            (Some(b"LXFE"), Some(directory), false)
                if self.root_bytes.len() == 36
                    && self.root_bytes.get(4..36) == Some(directory.root().as_slice()) =>
            {
                directory.verify()?;
                directory_entries(directory)?
            }
            _ => return Err("inline/external root mode mismatch"),
        };
        if actual != expected {
            return Err("schema-root inventory mismatch");
        }
        for entry in &actual {
            let subtree = self
                .subtrees
                .get(&entry.schema)
                .ok_or("missing schema subtree")?;
            if subtree.binding != entry.schema || subtree.root() != entry.root {
                return Err("wrong schema-root binding or substituted subtree");
            }
            subtree.verify()?;
        }
        Ok(())
    ...
    fn point(&self, row: &LogicalRow, missing: bool) -> (bool, ReadMetric) {
        let mut metric = self.authenticate_root();
        let entry = self.lookup_schema(row.schema, &mut metric);
        let subtree = self.subtrees.get(&row.schema).expect("schema subtree");
        assert_eq!(entry.root, subtree.root());
    ...
    fn typed_range(&self, schema: [u8; 16], limit: usize) -> (usize, [u8; 32], ReadMetric) {
        let mut metric = self.authenticate_root();
        let entry = self.lookup_schema(schema, &mut metric);
    ...
    fn lookup_schema(&self, schema: [u8; 16], metric: &mut ReadMetric) -> SchemaRootEntry {
        if let Some(directory) = &self.directory {
            let (value, directory_metric) = directory.point(&schema);
            add_metric(metric, directory_metric);
            decode_schema_root_value(schema, &value.expect("schema directory member"))
                .expect("authenticated schema-root entry")
        } else {
            decode_inline_root(&self.root_bytes)
                .expect("authenticated inline root")
                .into_iter()
                .find(|entry| entry.schema == schema)
                .expect("inline schema-root entry")
        }
    }
    ...
    fn objects(&self) -> BTreeMap<[u8; 32], Vec<u8>> {
        let mut objects = self
            .directory
            .as_ref()
            .map_or_else(BTreeMap::new, |directory| directory.objects.clone());
...
fn schema_root_entries(subtrees: &BTreeMap<[u8; 16], AuthTree>) -> Vec<SchemaRootEntry> {
    subtrees
        .iter()
        .map(|(schema, tree)| SchemaRootEntry {
            schema: *schema,
            root: tree.root(),
            rows: tree.levels[0].iter().map(|page| page.entries).sum(),
        })
        .collect()
}

fn encode_schema_root_value(entry: &SchemaRootEntry) -> Vec<u8> {
    let mut value = entry.root.to_vec();
    push_u32(&mut value, entry.rows);
    value
}

fn decode_schema_root_value(
    schema: [u8; 16],
    value: &[u8],
) -> Result<SchemaRootEntry, &'static str> {
    if value.len() != 36 {
        return Err("malformed schema-root value");
    }
    Ok(SchemaRootEntry {
        schema,
        root: value[..32].try_into().map_err(|_| "schema root")?,
        rows: read_u32(value, 32)?,
    })
}

fn encode_inline_root(entries: &[SchemaRootEntry]) -> Vec<u8> {
    let mut root = b"LXFI".to_vec();
    push_u32(&mut root, entries.len());
    for entry in entries {
        root.extend_from_slice(&entry.schema);
        root.extend_from_slice(&entry.root);
        push_u32(&mut root, entry.rows);
    }
    root
}

fn decode_inline_root(bytes: &[u8]) -> Result<Vec<SchemaRootEntry>, &'static str> {
    if bytes.get(..4) != Some(b"LXFI") || bytes.len() < 8 {
        return Err("malformed inline root");
    }
    let count = read_u32(bytes, 4)?;
    if bytes.len() != 8 + count * SCHEMA_ROOT_ENTRY_BYTES || bytes.len() > INLINE_ROOT_CAP {
        return Err("inline root count/size mismatch");
    }
    let mut entries = Vec::with_capacity(count);
    for index in 0..count {
        let offset = 8 + index * SCHEMA_ROOT_ENTRY_BYTES;
        let schema = bytes[offset..offset + 16]
            .try_into()
            .map_err(|_| "schema identity")?;
        entries.push(decode_schema_root_value(
            schema,
            &bytes[offset + 16..offset + SCHEMA_ROOT_ENTRY_BYTES],
        )?);
    }
    if entries
        .windows(2)
    ...
    {
        return Err("duplicate/unsorted inline schema-root entry");
    }
    Ok(entries)
}

fn directory_entries(directory: &AuthTree) -> Result<Vec<SchemaRootEntry>, &'static str> {
    let mut entries = Vec::new();
    for page in &directory.levels[0] {
        for entry in decode_page(&page.bytes, DIRECTORY_BINDING, false)? {
            ...
            entries.push(decode_schema_root_value(schema, &entry.value)?);
        }
    }
    if entries
        .windows(2)
        .any(|pair| pair[0].schema >= pair[1].schema)
    {
        return Err("duplicate/unsorted external schema-root entry");
    }
    Ok(entries)
}
...
fn main() {
    let max_n = std::env::var("LIX_ROOT_FUSION_MAX_N")
        .ok()
        ...
        for n in COUNTS.into_iter().filter(|n| *n <= max_n) {
            for schemas in SCHEMA_COUNTS.into_iter().filter(|schemas| *schemas <= n) {
                for distribution in Distribution::ALL {
                    ...
                    let schema = FusionRepo::build(rows);
...
fn benchmark_points(
    ...
    for missing in [false, true] {
        for (layout, run) in [("global", 0_u8), ("root_fusion", 1_u8)] {
...
fn benchmark_mutations(
    ...
    for d in [1, 10, (n / 100).max(1)] {
        for across in [false, true] {
            ...
            let began = Instant::now();
            let next_schema = FusionRepo::build(changed);
...
fn schema_id(index: usize) -> [u8; 16] {
    ...
    hasher.update(b"EXP-ROOT-FUSION-21.schema\0");
...
fn fixture_pk(kind: KeyKind, ordinal: usize) -> Vec<u8> {
    match kind {
        ...
        KeyKind::Uuid => {
            let mut hasher = blake3::Hasher::new();
            hasher.update(b"EXP-ROOT-FUSION-21.uuid\0");
...
fn corruption_controls(repo: &FusionRepo) {
    ...
    if repo.subtrees.len() > 1 {
        ...
        let successor = FusionRepo::build(changed_rows);
    ...
    let mut truncated = repo.clone();
    truncated.root_bytes.pop();
    truncated.root_id = *blake3::hash(&truncated.root_bytes).as_bytes();
    assert!(truncated.verify().is_err());

    let mut wrong_mode = repo.clone();
    wrong_mode.root_bytes[..4].copy_from_slice(if repo.directory.is_some() {
        b"LXFI"
    } else {
        b"LXFE"
    });
    wrong_mode.root_id = *blake3::hash(&wrong_mode.root_bytes).as_bytes();
    assert!(wrong_mode.verify().is_err());

    if repo.directory.is_none() && repo.subtrees.len() > 1 {
        let mut duplicate = repo.clone();
        duplicate.root_bytes.copy_within(8..24, 60);
        duplicate.root_id = *blake3::hash(&duplicate.root_bytes).as_bytes();
        assert!(duplicate.verify().is_err());

        let mut unsorted = repo.clone();
        let first = unsorted.root_bytes[8..60].to_vec();
        let second = unsorted.root_bytes[60..112].to_vec();
        unsorted.root_bytes[8..60].copy_from_slice(&second);
        unsorted.root_bytes[60..112].copy_from_slice(&first);
        unsorted.root_id = *blake3::hash(&unsorted.root_bytes).as_bytes();
        assert!(unsorted.verify().is_err());
    }

    if let Some(directory) = &repo.directory {
        ...
        malformed_repo.directory = Some(malformed_directory);
        malformed_repo.root_bytes = b"LXFE".to_vec();
        malformed_repo.root_bytes.extend_from_slice(
            &malformed_repo
                .directory
                .as_ref()
        ...
        let directory_page = &directory.levels[0][0];
...
fn insertion_order_control() {
    ...
    let canonical = FusionRepo::build(rows.clone());
    let mut reversed = rows;
    reversed.reverse();
    let reordered = FusionRepo::build(reversed);
