//! EXP-SUBRANGE-MERKLE-12: authenticated subrange hashes inside C2 leaves.
//!
...
const CSV_VERSION: &str = "exp-subrange-merkle-12-v1";
...
impl Geometry {
    const fn label(self) -> &'static str {
        match self {
            Self::C2 => "c2_slotted",
            Self::C2Merkle => "c2_subrange_merkle",
    ...
    for pk in config.pks {
        for &n in &config.ns {
            ...
            for &geometry in &config.geometries {
                let block_sizes: &[usize] = if geometry == Geometry::C2Merkle {
                    &config.block_sizes
                } else {
                    &[0]
                };
                for &block_size in block_sizes {
                    ...
                    let tree = Tree::build(rows.clone(), geometry, page_target, block_size);
...
impl Config {
    fn parse() -> Self {
        ...
        let mut geometry = env_or("LIX_ART_GEOMETRY", "c2,merkle");
        ...
        let mut block_sizes = env_or("LIX_SUBRANGE_BLOCK_SIZES", "16,32,64");
        for argument in std::env::args().skip(1) {
            ...
            match name {
                ...
                "--block-sizes" => block_sizes = value.to_owned(),
        ...
        Self {
            ...
            block_sizes: block_sizes
                .split(',')
                .map(|value| value.parse().expect("block size is an integer"))
                .collect(),
...
fn parse_geometries(value: &str) -> Vec<Geometry> {
    value
        ...
        .map(|part| match part {
            "c2" => Geometry::C2,
            "merkle" | "subrange" => Geometry::C2Merkle,
...
impl Tree {
    fn build(rows: Vec<Row>, geometry: Geometry, page_target: usize, block_size: usize) -> Self {
        assert!(!rows.is_empty());
        assert!(rows.windows(2).all(|pair| pair[0].key < pair[1].key));
        let mut objects = BTreeMap::new();
        let root = match geometry {
            Geometry::C2 | Geometry::C2Merkle => {
                build_c2(&rows, geometry, page_target, block_size, &mut objects)
            }
    ...
    fn point(&self, key: &[u8]) -> Result<(Option<Row>, IoMetric), String> {
        match self.geometry {
            Geometry::C2 | Geometry::C2Merkle => self.point_c2(key),
    ...
    fn mutate(&self, kind: &str, amount: usize, pk: PkKind) -> (Self, OpMetric) {
        ...
        let (updated, mutation_io, rebuild_model, wall, cpu_ns) =
            if inserted.is_some() && matches!(self.geometry, Geometry::C2 | Geometry::C2Merkle) {
                let started = Instant::now();
                let cpu_started = cpu_time_ns();
                let updated = Self::build(
                    rows.clone(),
                    self.geometry,
                    self.page_target,
                    self.block_size,
    ...
    fn diff(&self, other: &Self) -> Result<OpMetric, String> {
        let (changed, mut metric) = self.diff_rows(other)?;
        metric.digest = digest_rows(changed.iter());
        Ok(metric)
    }

    fn diff_rows(&self, other: &Self) -> Result<(Vec<Row>, OpMetric), String> {
        let started = Instant::now();
        let cpu_started = cpu_time_ns();
    ...
    fn three_way_merge(&self, target: &Self, source: &Self) -> Result<OpMetric, String> {
        let started = Instant::now();
        let cpu_started = cpu_time_ns();
        let (target_rows, target_metric) = self.diff_rows(target)?;
        let (source_rows, source_metric) = self.diff_rows(source)?;
        let mut merged = target_rows
            .into_iter()
            .map(|row| (row.key.clone(), row))
            .collect::<BTreeMap<_, _>>();
        // Current LWW behavior: source wins exact-key conflicts.
        ...
        Ok(OpMetric {
            ...
            cpu_ns: cpu_time_ns().saturating_sub(cpu_started),
            io: IoMetric {
                calls: target_metric.io.calls + source_metric.io.calls,
                objects: target_metric.io.objects + source_metric.io.objects,
                bytes: target_metric.io.bytes + source_metric.io.bytes,
            },
            diff_nodes: target_metric.diff_nodes + source_metric.diff_nodes,
            digest: digest_rows(merged.values()),
            ..OpMetric::default()
        })
    }
}
...
impl<'a> MutationBuilder<'a> {
    ...
    fn update(&mut self, updates: &BTreeMap<Vec<u8>, Vec<u8>>) -> Result<Object, String> {
        ...
        match self.base.geometry {
            Geometry::C2 | Geometry::C2Merkle => self.update_c2(self.base.root.id, updates),
    ...
    fn update_c2(
        ...
        match raw.get(..4) {
            Some(b"LXC2") | Some(b"LXCM") => {
                let parsed = parse_c2_leaf_details(&raw, false)?;
                ...
                let block_size =
                    (self.base.geometry == Geometry::C2Merkle).then_some(self.base.block_size);
                let reused_hashes = if let Some(block_size) = block_size {
                    let mut hashes = parsed.block_hashes;
                    for (block_index, block) in rows.chunks(block_size).enumerate() {
                        let start = block_index * block_size;
                        let end = start + block.len();
                        if rows[start..end]
                            .iter()
                            .any(|row| updates.contains_key(&row.key))
                        {
                            hashes[block_index] = subrange_hash(block);
                        }
                    }
                    Some(hashes)
                } else {
                ...
                Ok(self.add(encode_c2_leaf(&rows, block_size, reused_hashes.as_deref())))
            }
            Some(b"LXCI") => {
...
fn build_c2(
    ...
    while start < rows.len() {
        ...
        level.push(encode_c2_leaf(
            &rows[start..end],
            (geometry == Geometry::C2Merkle).then_some(block_size),
            None,
        ));
        start = end;
    }
...
fn encode_c2_leaf(
    ...
    reused_hashes: Option<&[[u8; 32]]>,
) -> Object {
    let prefix = common_prefix(rows.iter().map(|row| row.key.as_slice()));
    let mut raw = Vec::new();
    raw.extend_from_slice(if block_size.is_some() {
        b"LXCM"
    } else {
        b"LXC2"
    });
    ...
    if let Some(block_size) = block_size {
        assert!(matches!(block_size, 16 | 32 | 64));
        push_u32(&mut raw, block_size);
        push_u32(&mut raw, rows.len().div_ceil(block_size));
        if let Some(hashes) = reused_hashes {
            assert_eq!(hashes.len(), rows.len().div_ceil(block_size));
            for hash in hashes {
                raw.extend_from_slice(hash);
            }
        } else {
            for block in rows.chunks(block_size) {
                raw.extend_from_slice(&subrange_hash(block));
            }
        }
    }
    ...
    make_object(
        if block_size.is_some() { 6 } else { 2 },
        raw,
        rows[0].key.clone(),
...
fn subrange_hash(rows: &[Row]) -> [u8; 32] {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"lix.forktree.c2-subrange.v1\0");
    for row in rows {
...
fn verify_node(
    ...
    let summary = match (geometry, raw.get(..4)) {
        ...
        (Geometry::C2Merkle, Some(b"LXCM")) => {
            summary_rows(&parse_c2_leaf_details(&raw, true)?.rows)
        }
        (Geometry::C2, Some(b"LXCI")) | (Geometry::C2Merkle, Some(b"LXCI")) => {
            let node = parse_c2_internal(&raw)?;
...
fn parse_c2_leaf(raw: &[u8]) -> Result<Vec<Row>, String> {
    Ok(parse_c2_leaf_details(raw, false)?.rows)
}

fn parse_c2_leaf_details(raw: &[u8], verify_hashes: bool) -> Result<ParsedC2Leaf, String> {
    let mut cursor = Cursor::new(raw);
    let merkle = match raw.get(..4) {
        Some(b"LXC2") => false,
        Some(b"LXCM") => true,
        _ => return Err("wrong C2 leaf domain".into()),
    };
    cursor.take(4)?;
    cursor.fingerprint()?;
    ...
    let (block_size, block_hashes) = if merkle {
        let block_size = cursor.u32()?;
        if !matches!(block_size, 16 | 32 | 64) {
            return Err("non-canonical subrange block size".into());
        }
        let block_count = cursor.u32()?;
        if block_count != count.div_ceil(block_size) {
            return Err("subrange block count mismatch".into());
        }
    ...
    if verify_hashes
        ...
        && rows
            .chunks(block_size)
            .map(subrange_hash)
            .ne(block_hashes.iter().copied())
    {
        return Err("subrange hash mismatch".into());
    }
    Ok(ParsedC2Leaf {
        rows,
        block_size,
...
fn collect_range(
    ...
    require_domain(bytes, domain_for_raw(&raw)?)?;
    match raw.get(..4) {
        Some(b"LXC2") | Some(b"LXCM") => output.extend(
            parse_c2_leaf(&raw)?
                .into_iter()
...
fn diff_nodes(
    ...
    metric.diff_nodes += 1;
    let before_bytes = authenticated_object(&before.objects, before_id)?;
    let after_bytes = authenticated_object(&after.objects, after_id)?;
    let before_raw = decode_envelope(before_bytes)?;
    let after_raw = decode_envelope(after_bytes)?;
    metric.io.objects += 2;
    metric.io.calls += 2;
    metric.io.bytes += before_bytes.len() + after_bytes.len();
    if before_raw.starts_with(b"LXCM") && after_raw.starts_with(b"LXCM") {
        let before_leaf = parse_c2_leaf_details(&before_raw, false)?;
        let after_leaf = parse_c2_leaf_details(&after_raw, false)?;
        if before_leaf.block_size == after_leaf.block_size
            && before_leaf.block_hashes.len() == after_leaf.block_hashes.len()
            && before_leaf.rows.len() == after_leaf.rows.len()
            && before_leaf.rows.first().map(|row| &row.key)
                == after_leaf.rows.first().map(|row| &row.key)
            && before_leaf.rows.last().map(|row| &row.key)
                == after_leaf.rows.last().map(|row| &row.key)
        {
            for (index, (before_hash, after_hash)) in before_leaf
                .block_hashes
                .iter()
                .zip(&after_leaf.block_hashes)
                .enumerate()
            {
                if before_hash != after_hash {
                    let start = index * before_leaf.block_size;
                    let end = (start + before_leaf.block_size).min(before_leaf.rows.len());
                    append_changed_rows(
                        &before_leaf.rows[start..end],
                        &after_leaf.rows[start..end],
                        changed,
                    );
                }
            }
    ...
    let before_node = decode_object(before_id, before_bytes)?;
    let after_node = decode_object(after_id, after_bytes)?;
    ...
    append_changed_rows(&before_rows, &after_rows, changed);
    Ok(())
}

fn append_changed_rows(before_rows: &[Row], after_rows: &[Row], changed: &mut Vec<Row>) {
    let mut before_map = before_rows
        .into_iter()
        .map(|row| (row.key.clone(), row.tuple.clone()))
        .collect::<BTreeMap<_, _>>();
...
fn run_operations(tree: &Tree, pk: PkKind) {
    ...
    print_row(
        "model",
        if matches!(tree.geometry, Geometry::C2 | Geometry::C2Merkle) {
    ...
    for (operation, d) in [
        ...
        let (source, _) = tree.mutate("mutate_source", d, pk);
        let merge = tree
            .three_way_merge(&changed, &source)
            .expect("three-way merge authenticates");
        let merge_operation = match operation {
            "hash_diff_1" => "merge_1",
            "hash_diff_10" => "merge_10",
            _ => "merge_1pct",
        };
        print_row(
            "model",
...
fn run_corruption_controls(tree: &Tree, pk: PkKind) {
    ...
    *bytes = Bytes::from(changed);
    ...
    if tree.geometry == Geometry::C2Merkle {
        controls += run_subrange_corruption_controls(tree);
    }
...
fn run_subrange_corruption_controls(tree: &Tree) -> usize {
    let rows = tree.rows[..tree.rows.len().min(64)].to_vec();
    let one_page = Tree::build(rows, Geometry::C2Merkle, 256 << 10, tree.block_size);
    assert!(one_page.root.children.is_empty());
    let raw = decode_envelope(&one_page.root.bytes).expect("one-page Merkle leaf decodes");
    assert!(raw.starts_with(b"LXCM"));

    let first_len = u32::from_be_bytes(raw[20..24].try_into().expect("first-key length")) as usize;
    let last_len_offset = 24 + first_len;
    let last_len = u32::from_be_bytes(
        raw[last_len_offset..last_len_offset + 4]
            .try_into()
            .expect("last-key length"),
    ) as usize;
    let count_offset = last_len_offset + 4 + last_len;
    let block_size_offset = count_offset + 4;
    let block_count_offset = block_size_offset + 4;
    let block_count = u32::from_be_bytes(
        raw[block_count_offset..block_count_offset + 4]
            .try_into()
            .expect("block count"),
    ) as usize;
    let hash_offset = block_count_offset + 4;
    let prefix_len_offset = hash_offset + block_count * 32;
    let prefix_len = u32::from_be_bytes(
        raw[prefix_len_offset..prefix_len_offset + 4]
            .try_into()
            .expect("prefix length"),
    ) as usize;
    let directory_offset = prefix_len_offset + 4 + prefix_len;
    let mut controls = 0;

    let mut bad_hash = raw.clone();
    bad_hash[hash_offset] ^= 1;
    assert_rehashed_root_rejected(&one_page, encode_envelope(6, bad_hash));
    controls += 1;

    if block_count > 1 {
        let mut moved_across_block = raw.clone();
        let left = tree.block_size - 1;
        let right = tree.block_size;
        let left_offset = directory_offset + left * 16;
        let right_offset = directory_offset + right * 16;
        let left_slot: [u8; 16] = moved_across_block[left_offset..left_offset + 16]
            .try_into()
            .expect("left directory slot");
        let right_slot: [u8; 16] = moved_across_block[right_offset..right_offset + 16]
            .try_into()
            .expect("right directory slot");
        moved_across_block[left_offset..left_offset + 16].copy_from_slice(&right_slot);
        moved_across_block[right_offset..right_offset + 16].copy_from_slice(&left_slot);
        assert_rehashed_root_rejected(&one_page, encode_envelope(6, moved_across_block));
        controls += 1;
    }

    let mut wrong_count = raw.clone();
    wrong_count[block_count_offset..block_count_offset + 4]
        .copy_from_slice(&((block_count + 1) as u32).to_be_bytes());
    assert_rehashed_root_rejected(&one_page, encode_envelope(6, wrong_count));
    controls += 1;

    let mut wrong_fence = raw.clone();
    wrong_fence[24] ^= 1;
    assert_rehashed_root_rejected(&one_page, encode_envelope(6, wrong_fence));
    controls += 1;

    let mut duplicate_slot = raw.clone();
    duplicate_slot[directory_offset + 16..directory_offset + 20]
        .copy_from_slice(&0_u32.to_be_bytes());
    assert_rehashed_root_rejected(&one_page, encode_envelope(6, duplicate_slot));
    controls += 1;

    let mut truncated = raw;
    truncated.pop();
    assert_rehashed_root_rejected(&one_page, encode_envelope(6, truncated));
    controls += 1;
...
fn print_header() {
    println!(
        "version,backend,operation,geometry,pk,n,d,page_target,block_size,tuple_width,height,objects,object_bytes,wall_ns,cpu_ns,rss_kb,calls,read_objects,read_bytes,writes,write_bytes,settled_bytes,shared_objects,diff_nodes,result_digest,timing_scope,cold_reopen,status"
    );
}
...
fn print_row(
    ...
    println!(
        "{CSV_VERSION},{backend},{operation},{geometry},{pk},{n},{d},{target},{block_size},{width},{height},{objects},{object_bytes},{wall_ns},{cpu_ns},{rss},{calls},{read_objects},{read_bytes},{writes},{write_bytes},{settled_bytes},{shared},{diff_nodes},{digest},{timing_scope},{cold_reopen},{status}",
