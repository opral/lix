...
async fn measure<T, F>(backend: &str, operation: &str, future: F) -> T
where
    F: Future<Output = T>,
{
...
async fn run<S, O>(backend: &str, root: PathBuf, open: O)
where
    S: Storage + Clone + Send + Sync + 'static,
    O: Fn(&Path) -> S,
{
    ...
    measure(backend, "parse_to_native_rows_insert", async {
    ...
    let exact = measure(backend, "exact_row_query", async {
        lix.execute(
            "SELECT id, kind, payload_json FROM markdown_node \
             WHERE lixcol_file_id = $1 AND kind = 'paragraph' ORDER BY id LIMIT 1",
    ...
    let paragraph_id = exact.rows()[0]
        .get::<String>("id")
        .expect("paragraph id");

    let range = measure(backend, "bounded_row_range_query", async {
        lix.execute(
            "SELECT id, kind, order_key, payload_json FROM markdown_node \
    ...
    let diff = measure(backend, "historical_diff", async {
        lix.execute(
    ...
    let history = measure(backend, "history_depth_one", async {
        lix.execute(
    ...
    measure(backend, "transaction_insert_17_files", async {
    ...
    let branch_read = measure(backend, "branch_shared_read", read_file(&lix, PATH)).await;
    ...
    lix.close().await.expect("close workspace");
    let reopened = open_lix()
        .with_storage(open(&root))
        .await
        .expect("reopen workspace");
    let cold = measure(backend, "cold_reopen_exact_file_read", read_file(&reopened, PATH)).await;
    ...
    reopened.close().await.expect("close reopened workspace");
...
async fn main() {
    ...
    match backend.as_str() {
        "rocksdb" => {
            run::<RocksDB, _>("rocksdb", root, |path| {
                RocksDB::open(path.join(".lix")).expect("open RocksDB")
            })
            .await
        }
        "slatedb" => {
            run::<SlateDB, _>("slatedb", root, |path| {
                SlateDB::open(path.join(".lix")).expect("open SlateDB")
            })
            .await
        }
