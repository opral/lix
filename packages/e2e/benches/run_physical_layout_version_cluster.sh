...
geometries=(separate_history version_cluster)
if [[ "$mode" == quick ]]; then
  pks=(integer text)
  jsons=(absent)
  counts=(1000)
...
printf 'sample\tpk\tshape\tjson\tn\th\tgeometry\ttarget\tstdout\ttime\n' > "$manifest"
      ...
      for json in "${jsons[@]}"; do
        for n in "${counts[@]}"; do
          ...
          for geometry in "${geometries[@]}"; do
            stem=$(printf 's%02d-%s-%s-%s-n%s-h%s-%s' "$sample" "$pk" "$shape" "$json" "$n" "$history" "$geometry")
            stdout="$output_dir/$stem.csv"
            timing="$output_dir/$stem.time"
            timeout 1200 /usr/bin/time -v -o "$timing" \
              env LIX_VERSION_N="$n" \
                  LIX_VERSION_PK="$pk" \
                  LIX_VERSION_SHAPE="$shape" \
                  LIX_VERSION_JSON="$json" \
                  LIX_VERSION_TARGET="$target" \
                  LIX_VERSION_H="$history" \
                  LIX_VERSION_GEOMETRY="$geometry" \
                  "$binary" > "$stdout"
            printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
              "$sample" "$pk" "$shape" "$json" "$n" "$history" "$geometry" "$target" "$stdout" "$timing" >> "$manifest"
          done
          done
        done
      done
