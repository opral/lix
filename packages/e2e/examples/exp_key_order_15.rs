//! EXP-PREDICATE-DIRECTORY-16: external authenticated typed predicate directory.
//!
//! Compares identical Schema-v1 typed tuple bytes in byte-identical canonical
//! state pages with either:
//! - baseline non-PK filtering by authenticating/scanning all state pages; or
//! - an external root-referenced authenticated directory for the typed `seq`
//!   predicate, followed by authentication of only selected canonical rows.
//!
...
const LEDGER: &str = "EXP-PREDICATE-DIRECTORY-16";
const PAGE_ROWS: usize = 64;
const FILE_OWNERS: u64 = 4;
// 1,024 fixed-width (value,key) entries are 16 KiB before the authenticated
// header: one deterministic physical policy, not workload tuning.
const DIRECTORY_BUCKET_WIDTH: i64 = 1_024;
const SCHEMA_ID: [u8; 16] = [0x31; 16];
const LAYOUT_ID: [u8; 16] = [0x62; 16];
const COLUMN_ID: [u8; 16] = [0x73; 16];
...
const DIRECTORY_PAGE_SPACE: StorageSpace = experiment_storage_space(
    0x00fe_2005,
    "exp.predicate_directory.page",
    ValueSemantics::Immutable,
);
const DIRECTORY_CATALOG_SPACE: StorageSpace = experiment_storage_space(
    0x00fe_2006,
    "exp.predicate_directory.catalog",
    ValueSemantics::Immutable,
);
...
impl Layout {
    fn name(self) -> &'static str {
        match self {
            ...
            Self::PredicateDirectory => "predicate_directory",
        }
    }
    ...
    fn tag(self) -> u8 {
        match self {
            Self::Baseline => 1,
            Self::PredicateDirectory => 2,
        }
    }

    fn parse(value: &str) -> Self {
        match value {
            "baseline" | "old_order" => Self::Baseline,
            "predicate_directory" | "new_order" => Self::PredicateDirectory,
...
impl Counters {
    fn delta(self, before: Self) -> Self {
        Self {
            ...
            directory_pages: self.directory_pages - before.directory_pages,
            directory_entries: self.directory_entries - before.directory_entries,
        }
    }
    ...
    fn accumulate(&mut self, other: Self) {
        ...
        self.directory_pages += other.directory_pages;
...
struct Root {
    ...
    pages: Vec<ObjectId>,
    /// Predicate narrowing only. It never serves tuple values.
    predicate_directory: Option<ObjectId>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
struct DirectoryCatalog {
    buckets: Vec<DirectoryBucket>,
}
...
async fn run_case<S: Storage + Clone>(
    ...
    let mut predicates = expected
        .values()
        .map(|value| predicate_from_tuple(value).unwrap())
        .collect::<Vec<_>>();
    predicates.sort_unstable();
    let range_index = n / 4;
    let range_end_index = (range_index + 100).min(n);
    let range_start = predicates[range_index];
    let range_end = predicates
        .get(range_end_index)
        .copied()
        .unwrap_or_else(|| predicates[range_end_index - 1] + 1);
    let before = store.counters;
    let started = Instant::now();
    let ranged = predicate_range(&mut store, root, range_start, range_end)
        .await
        .unwrap();
    assert_eq!(
        ranged,
        expected
            .iter()
            .filter(|(_, value)| {
                let predicate = predicate_from_tuple(value).unwrap();
                range_start <= predicate && predicate < range_end
            })
            .map(|(key, value)| (*key, value.clone()))
            .collect()
    );
    ...
    for _ in 0..sample_count() {
        let started = Instant::now();
        let sampled = predicate_range(&mut store, root, range_start, range_end)
            .await
            .unwrap();
    ...
    println!(
        "{LEDGER},kind=control,backend={backend},layout={},n={n},control=authenticated_corruption,status=pass,cases=missing_object+wrong_child+root_substitution+directory_gap+directory_overlap+directory_page_substitution+truncation",
        layout.name()
    );
...
fn emit(
    ...
    println!(
        "{LEDGER},kind=operation,backend={backend},layout={},n={n},h={h},d={d},operation={operation},wall_us={wall_us},get_calls={},get_keys={},get_bytes={},put_calls={},puts={},put_bytes={},decoded_pages={},decoded_rows={},directory_pages={},directory_entries={},settled_bytes={}",
        layout.name(),
        c.get_calls,
        c.get_keys,
        c.get_bytes,
...
async fn build_initial<S: Storage + Clone>(
    ...
    let mut root = Root {
        ...
        predicate_directory: None,
    };
    if layout == Layout::PredicateDirectory {
        root.predicate_directory = Some(put_directory(store, rows).await?);
    }
...
async fn commit<S: Storage + Clone>(
    ...
    if root.layout == Layout::PredicateDirectory {
        next.predicate_directory = Some(
            root.predicate_directory
                .ok_or_else(|| "predicate layout is missing its directory".to_owned())?,
        );
    }
...
async fn predicate_range<S: Storage + Clone>(
    store: &mut Store<S>,
    root_id: ObjectId,
    lower: i64,
    upper: i64,
    ...
    if root.layout == Layout::Baseline {
        let rows = materialize_on_read(&mut read, root_id)
            .await?
            .into_iter()
            .filter_map(|(key, value)| {
                let predicate = predicate_from_tuple(&value).ok()?;
                (lower <= predicate && predicate < upper).then_some((key, value))
            })
            .collect::<BTreeMap<_, _>>();
        return Ok(rows);
    }
    let catalog_id = root
        .predicate_directory
        .ok_or_else(|| "predicate query has no authenticated directory".to_owned())?;
    let catalog =
        decode_directory_catalog(&read.get_one(DIRECTORY_CATALOG_SPACE, catalog_id).await?)?;
    let selected = catalog
        .buckets
        .iter()
        .filter(|bucket| bucket.lower < upper && lower < bucket.upper)
        .cloned()
        .collect::<Vec<_>>();
    let ids = selected
        .iter()
        .map(|bucket| bucket.page)
        .collect::<Vec<_>>();
    let objects = read.get_many(DIRECTORY_PAGE_SPACE, &ids).await?;
    let mut keys = BTreeSet::new();
    for (bucket, bytes) in selected.iter().zip(objects) {
        for entry in decode_directory_page(read.counters, &bytes, bucket.lower, bucket.upper)? {
            if lower <= entry.value && entry.value < upper {
                keys.insert(entry.logical_key);
            }
        }
    }
    ...
    let page_ids = page_indices
        ...
        .map(|index| {
            root.pages
                .get(*index)
                .copied()
                .ok_or_else(|| "predicate key escaped selected state root".to_owned())
        })
        .collect::<Result<Vec<_>, _>>()?;
    let pages = read.get_many(PAGE_SPACE, &page_ids).await?;
    let mut rows = BTreeMap::new();
    for (index, bytes) in page_indices.into_iter().zip(pages) {
        let page = decode_page_counted(read.counters, &bytes, root.layout, root.count)?;
        validate_page_index(&page, index)?;
        for key in keys.range((index * PAGE_ROWS) as u64..((index + 1) * PAGE_ROWS) as u64) {
            let value = page
                .get(key)
                .ok_or_else(|| "predicate directory references a missing state row".to_owned())?;
            let predicate = predicate_from_tuple(value)?;
            if predicate < lower || predicate >= upper {
                return Err("predicate directory/state value substitution".to_owned());
            }
...
async fn corruption_control<S: Storage + Clone>(
    ...
    if decoded.layout == Layout::PredicateDirectory {
        let directory_id = decoded
            .predicate_directory
            .ok_or_else(|| "candidate root lost predicate directory".to_owned())?;
        let catalog =
            decode_directory_catalog(&store.get_one(DIRECTORY_CATALOG_SPACE, directory_id).await?)?;
        if catalog.buckets.len() >= 2 {
            let mut gap = catalog.clone();
            gap.buckets[1].lower += 1;
            let bytes = encode_directory_catalog(&gap);
            let catalog_id = ObjectId::of(&bytes);
            store
                .put_objects(DIRECTORY_CATALOG_SPACE, &[(catalog_id, bytes)])
                .await?;
            let mut selected = decoded.clone();
            selected.predicate_directory = Some(catalog_id);
            let selected_id = put_root(store, &selected).await?;
            assert!(
                predicate_range(store, selected_id, i64::MIN, i64::MAX)
                    .await
                    .is_err()
            );

            let mut overlap = catalog.clone();
            overlap.buckets[1].lower -= 1;
            let bytes = encode_directory_catalog(&overlap);
            let catalog_id = ObjectId::of(&bytes);
            store
                .put_objects(DIRECTORY_CATALOG_SPACE, &[(catalog_id, bytes)])
                .await?;
        ...
        let first = &catalog.buckets[0];
        let page_bytes = store.get_one(DIRECTORY_PAGE_SPACE, first.page).await?;
        let mut truncated = page_bytes;
        truncated.pop();
        let truncated_id = ObjectId::of(&truncated);
        store
            .put_objects(DIRECTORY_PAGE_SPACE, &[(truncated_id, truncated)])
            .await?;
        let mut substituted = catalog.clone();
        substituted.buckets[0].page = truncated_id;
        let catalog_bytes = encode_directory_catalog(&substituted);
        let catalog_id = ObjectId::of(&catalog_bytes);
        store
            .put_objects(DIRECTORY_CATALOG_SPACE, &[(catalog_id, catalog_bytes)])
            .await?;
        let mut selected = decoded.clone();
        selected.predicate_directory = Some(catalog_id);
        let selected_id = put_root(store, &selected).await?;
        assert!(
            predicate_range(store, selected_id, first.lower, first.upper)
                .await
                .is_err()
        );
    }
...
async fn put_directory<S: Storage + Clone>(
    store: &mut Store<S>,
    rows: &BTreeMap<u64, Vec<u8>>,
) -> Result<ObjectId, String> {
    let mut grouped = BTreeMap::<i64, Vec<DirectoryEntry>>::new();
    for (key, value) in rows {
        let predicate = predicate_from_tuple(value)?;
        grouped
            .entry(bucket_lower(predicate))
            .or_default()
            .push(DirectoryEntry {
                value: predicate,
                logical_key: *key,
    ...
    let Some((&first, _)) = grouped.first_key_value() else {
        return Err("predicate directory cannot index an empty state".to_owned());
    };
    let last = *grouped.last_key_value().unwrap().0;
    let mut pages = Vec::new();
    let mut buckets = Vec::new();
    let mut lower = first;
    while lower <= last {
        let mut entries = grouped.remove(&lower).unwrap_or_default();
        entries.sort();
        let upper = lower
            .checked_add(DIRECTORY_BUCKET_WIDTH)
            .ok_or_else(|| "predicate bucket overflow".to_owned())?;
        let bytes = encode_directory_page(lower, upper, &entries);
        let page = ObjectId::of(&bytes);
        pages.push((page, bytes));
        buckets.push(DirectoryBucket { lower, upper, page });
        lower = upper;
    }
    store.put_objects(DIRECTORY_PAGE_SPACE, &pages).await?;
    let catalog_bytes = encode_directory_catalog(&DirectoryCatalog { buckets });
    let catalog = ObjectId::of(&catalog_bytes);
    store
        .put_objects(DIRECTORY_CATALOG_SPACE, &[(catalog, catalog_bytes)])
        .await?;
    Ok(catalog)
}

fn encode_directory_catalog(catalog: &DirectoryCatalog) -> Vec<u8> {
    let mut out = b"PDC1".to_vec();
    out.extend_from_slice(&SCHEMA_ID);
    out.extend_from_slice(&LAYOUT_ID);
    out.extend_from_slice(&COLUMN_ID);
    out.extend_from_slice(&(catalog.buckets.len() as u32).to_be_bytes());
    for bucket in &catalog.buckets {
        out.extend_from_slice(&bucket.lower.to_be_bytes());
        out.extend_from_slice(&bucket.upper.to_be_bytes());
        out.extend_from_slice(&bucket.page.0);
    }
    out
}

fn decode_directory_catalog(bytes: &[u8]) -> Result<DirectoryCatalog, String> {
    let mut input = Decoder::new(bytes);
    input.magic(b"PDC1")?;
    if input.bytes(16)? != SCHEMA_ID
        || input.bytes(16)? != LAYOUT_ID
        || input.bytes(16)? != COLUMN_ID
    {
        return Err("predicate catalog schema/layout/column binding mismatch".to_owned());
    }
    ...
    for _ in 0..count {
        buckets.push(DirectoryBucket {
            lower: input.i64()?,
            upper: input.i64()?,
            page: ObjectId(input.fixed_32()?),
        });
    }
    input.finish()?;
    validate_directory_buckets(&buckets)?;
    Ok(DirectoryCatalog { buckets })
}

fn validate_directory_buckets(buckets: &[DirectoryBucket]) -> Result<(), String> {
    if buckets.is_empty() {
        return Err("predicate directory has no buckets".to_owned());
    }
    for (index, bucket) in buckets.iter().enumerate() {
        if bucket.upper - bucket.lower != DIRECTORY_BUCKET_WIDTH {
            return Err("predicate directory has malformed bucket width".to_owned());
        }
        if index > 0 && buckets[index - 1].upper != bucket.lower {
            return Err("predicate directory has a gap or overlap".to_owned());
        }
    }
    Ok(())
}

fn encode_directory_page(lower: i64, upper: i64, entries: &[DirectoryEntry]) -> Vec<u8> {
    let mut out = b"PDP1".to_vec();
    out.extend_from_slice(&SCHEMA_ID);
    out.extend_from_slice(&LAYOUT_ID);
    out.extend_from_slice(&COLUMN_ID);
    ...
    for entry in entries {
        ...
        out.extend_from_slice(&entry.logical_key.to_be_bytes());
    }
    out
}
...
fn decode_directory_page(
    ...
    if input.bytes(16)? != SCHEMA_ID
        ...
        || input.bytes(16)? != COLUMN_ID
    {
        return Err("predicate page schema/layout/column binding mismatch".to_owned());
    }
    if input.i64()? != lower || input.i64()? != upper {
        return Err("predicate page range binding mismatch".to_owned());
    }
    let count = input.u32()? as usize;
    ...
    for _ in 0..count {
        let entry = DirectoryEntry {
            value: input.i64()?,
            logical_key: input.u64()?,
        };
        if entry.value < lower || entry.value >= upper {
            return Err("predicate entry escaped its authenticated bucket".to_owned());
        }
        if entries.last().is_some_and(|previous| previous >= &entry) {
            return Err("predicate entries are duplicate or unordered".to_owned());
        }
        entries.push(entry);
    }
    input.finish()?;
    counters.directory_pages += 1;
    counters.directory_entries += entries.len() as u64;
    Ok(entries)
}

fn bucket_lower(value: i64) -> i64 {
    value.div_euclid(DIRECTORY_BUCKET_WIDTH) * DIRECTORY_BUCKET_WIDTH
}
...
fn encode_page(layout: Layout, count: u64, rows: &BTreeMap<u64, Vec<u8>>) -> Vec<u8> {
    ...
    // Ordinary state pages are byte-identical in both experiment arms.
    out.push(1);
...
fn decode_page(
    ...
    input.magic(b"KOP1")?;
    if input.u8()? != 1 {
        return Err("unknown canonical state-page format".to_owned());
    }
...
fn encode_root(root: &Root) -> Vec<u8> {
    ...
    match root.predicate_directory {
        None => out.push(0),
        Some(id) => {
            out.push(1);
...
fn decode_root(bytes: &[u8]) -> Result<Root, String> {
    ...
    let predicate_directory = match input.u8()? {
        0 => None,
        1 => Some(ObjectId(input.fixed_32()?)),
        _ => return Err("unknown predicate-directory root tag".to_owned()),
    };
...
fn physical_rows(layout: Layout, logical_rows: &BTreeMap<u64, Vec<u8>>) -> BTreeMap<u64, Vec<u8>> {
    ...
    logical_rows
        .iter()
        .map(|(logical, value)| (*logical, value.clone()))
        .collect()
}
...
fn key_codec_controls() {
    let count = 1_000;
    for layout in [Layout::Baseline, Layout::PredicateDirectory] {
    ...
    let new = (0..FILE_OWNERS)
        .map(|owner| {
            logical_to_physical(Layout::PredicateDirectory, pk * FILE_OWNERS + owner, count)
        })
        .collect::<Vec<_>>();
    assert!(new.windows(2).all(|pair| pair[1] == pair[0] + 1));
    let old = (0..FILE_OWNERS)
        .map(|owner| logical_to_physical(Layout::Baseline, pk * FILE_OWNERS + owner, count))
        .collect::<Vec<_>>();
    assert!(old.windows(2).all(|pair| pair[1] == pair[0] + 1));

    let root = Root {
        layout: Layout::PredicateDirectory,
        count,
        pages: vec![ObjectId([1; 32])],
        predicate_directory: Some(ObjectId([2; 32])),
    };
    ...
    let canonical = encode_page(Layout::PredicateDirectory, count, &rows);
    assert_eq!(
        decode_page(&canonical, Layout::PredicateDirectory, count).unwrap(),
        rows
    );
    assert!(decode_page(&canonical, Layout::PredicateDirectory, count + 4).is_err());
    let mut malformed_key = canonical.clone();
    let first_key_start = 4 + 1 + 8 + 4 + 4;
    malformed_key[first_key_start + 3 + 16 + 1] ^= 1;
    assert!(decode_page(&malformed_key, Layout::PredicateDirectory, count).is_err());
    let mut truncated_page = canonical;
    truncated_page.pop();
    assert!(decode_page(&truncated_page, Layout::PredicateDirectory, count).is_err());
...
fn tuple(key: u64, generation: u64) -> Vec<u8> {
    ...
    encode_body(
        ...
        &[
            ...
            BodyValue::Int8(predicate_value(key)),
...
fn predicate_value(key: u64) -> i64 {
    // Stable, deliberately decorrelated from canonical StateKey/page order.
    // Multiplication is a permutation over this odd modulus.
    ((key.wrapping_mul(73)) % 100_003) as i64
}

fn predicate_from_tuple(bytes: &[u8]) -> Result<i64, String> {
    let values = decode_body(&TUPLE_PLAN, bytes).map_err(|error| error.to_string())?;
    match values.get(1) {
        Some(BodyValue::Int8(value)) => Ok(*value),
        _ => Err("typed predicate column is not canonical int8".to_owned()),
    }
}
...
fn parse_config() -> Config {
    let quick = std::env::var("EXP_PREDICATE_DIRECTORY_QUICK")
        .ok()
        .as_deref()
        == Some("1");
    Config {
        backends: list_env("EXP_PREDICATE_DIRECTORY_BACKENDS", "rocksdb,slatedb"),
        layouts: list_env(
            "EXP_PREDICATE_DIRECTORY_LAYOUTS",
            "baseline,predicate_directory",
        )
        .iter()
        .map(|value| Layout::parse(value))
        .collect(),
        sizes: usize_env(
            "EXP_PREDICATE_DIRECTORY_SIZES",
            if quick {
                "1000"
            } else {
        ...
        histories: usize_env("EXP_PREDICATE_DIRECTORY_HISTORIES", "1"),
        deltas: list_env("EXP_PREDICATE_DIRECTORY_DELTAS", "1,1pct")
            .into_iter()
            .map(|value| {
            ...
            .collect(),
        root: std::env::var_os("EXP_PREDICATE_DIRECTORY_ROOT").map(PathBuf::from),
    }
}
