...
//!
//! ```text
//! cargo bench -p lix_sdk --bench profile_sql_point_queries --no-default-features \
...
use lix_engine::{Engine, Storage, Value};
...
async fn open_initialized(path: &Path) -> (SlateDB, lix_engine::SessionContext<SlateDB>) {
...
async fn measure<S: Storage + Clone + Send + Sync + 'static>(
    session: &lix_engine::SessionContext<S>,
    sql: &str,
    rows: usize,
    rounds: usize,
    repeated: bool,
