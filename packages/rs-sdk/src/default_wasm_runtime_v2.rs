...
fn decode_change_packet(
    ...
    if max_bytes == 0 || max_bytes > limits.max_page_bytes {
        return Err(v2_limit(
            "v2 change cursor max-bytes is outside its transition limit",
        ));
    }
    if payload.len() > max_bytes as usize {
        return Err(v2_limit(
            "v2 guest change payload exceeds the requested max-bytes",
        ));
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn output_backed_edit_does_not_consume_inline_allowance() {
        let mut actor = csv_test_actor().await;
        ...
        let page = actor
            ...
            .await
            .expect("an output-backed edit should fit a zero inline allowance")
            .expect("the row update should emit one edit page");
        assert_eq!(page.edits.len(), 1);
        assert!(page.outputs.is_some());
        assert!(matches!(&page.edits[0].insert, WasmGuestBytes::Output(_)));
        assert!(
            actor
                .next_edit_page(
                    rendered.transition,
                ...
                .await
                .expect("output-backed cursor should reach EOF")
                .is_none()
        );
        actor
            .finish_transition(rendered.transition)
            .await
            .expect("output-backed edit transition should finish");
        actor
            .drop_document(renderer_fork)
            .await
        ...
        actor
            .drop_document(accepted)
            .await
            .expect("accepted document should drop");
    }
    ...
    async fn csv_v2_initial_import_retains_64_mib_efficiency_invariant() {
        ...
        let transition = actor
            ...
            .await
            .expect("220k-row initial import must fit under the 64 MiB acceptance limit");
    ...
    async fn csv_v2_cold_open_and_warm_edit_retain_64_mib_efficiency_invariant() {
        ...
        let transition = actor
            ...
            .await
            .expect("220k-row cold open must fit under the 64 MiB acceptance limit");
        let page = actor
            .next_edit_page(
                transition.transition,
                transition.edits,
        ...
        let detection = actor
            ...
            .await
            .expect("one-row warm detection must fit under the 64 MiB acceptance limit");
        let detected = actor
            .next_change_page(
...
impl WasmComponentV2Actor for WasmtimeV2Actor {
    ...
    async fn next_edit_page(
        ...
        let mut edits = Vec::with_capacity(page.edits.len());
        for edit in page.edits {
                ...
                bindings::exports::lix::plugin::api::OutputBytes::Inline(bytes) => {
                    inline_bytes =
                        inline_bytes
                            ...
                            .ok_or_else(|| {
                                self.retire_now();
                                v2_limit("v2 guest edit inline-byte count overflowed")
                            })?;
                    record_bytes =
                        record_bytes
                            .checked_add(bytes.len() as u64)
            ...
            if record_bytes > u64::from(limits.max_record_bytes) {
                self.retire_now();
                return Err(v2_limit("v2 guest edit record exceeds max-record-bytes"));
            }
            page_bytes = page_bytes.checked_add(record_bytes).ok_or_else(|| {
                self.retire_now();
                v2_limit("v2 guest edit page byte count overflowed")
            })?;
            edits.push(WasmOutputSplice {
                offset: edit.offset,
                delete_len: edit.delete_len,
                insert,
            });
        ...
        if inline_bytes > u64::from(max_inline_bytes) {
            self.retire_now();
            return Err(v2_limit(
                "v2 guest edit page exceeds requested max-inline-bytes",
            ));
        }
    ...
    fn validate_output_ranges(
        ...
        for range in ranges {
            if range.offset.checked_add(range.length).is_none() {
                self.retire_now();
                return Err(v2_invalid_plugin("v2 output range overflowed"));
            }
        }
        ...
        for range in ranges {
            ...
            let end = range
                .offset
                .checked_add(range.length)
                .expect("all output ranges were checked before calling the guest");
            if end > length {
                self.retire_now();
