...
impl WasmRuntime for WasmtimeRuntime {
    async fn init_component(
        ...
        if let Some(component) = self
            ...
            .cloned()
        {
            return Ok(Arc::new(WasmtimeInstance {
...
#[async_trait(?Send)]
impl WasmComponentInstance for WasmtimeInstance {
    async fn call(&self, export: &str, input: &[u8]) -> Result<Vec<u8>, LixError> {
        ...
        match export {
            ...
            other => Err(LixError {
                code: "LIX_ERROR_UNKNOWN".to_string(),
                description: format!("unsupported export '{other}' for WasmtimeRuntime"),
            }),
        }
    }
}
