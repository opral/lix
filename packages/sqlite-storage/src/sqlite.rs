...
impl SQLiteFactory {
    pub fn new() -> Self {
        Self {
            temp_dir: tempfile::tempdir().expect("create sqlite storage temp dir"),
...
impl StorageFactory for SQLiteFactory {
    ...
    fn create_fixture(&self) -> Self::Fixture {
        ...
        let path = self
            .temp_dir
            .path()
            .join(format!("storage-{database_id}.sqlite"));
        SQLiteFixture { path }
    }
...
impl StorageFixture for SQLiteFixture {
    type Storage = SQLite;

    fn open(&self) -> impl Future<Output = Self::Storage> + Send {
        async move { SQLite::open(&self.path).expect("open sqlite storage") }
    }
}

impl SQLite {
    pub fn new(options: SQLiteOptions) -> Result<Self, StorageError> {
    ...
    pub fn format_version(&self) -> Result<u32, StorageError> {
    ...
    fn begin_read(
        ...
        async move {
            let conn = self
                .read_pool
                .lock()
                .map_err(|error| StorageError::Io(format!("sqlite read pool poisoned: {error}")))?
                .pop()
                .map(Ok)
    ...
    fn begin_write(
        ...
        async move {
            let conn = self
                .write_pool
                .lock()
                .map_err(|error| StorageError::Io(format!("sqlite write pool poisoned: {error}")))?
                .pop()
                .map(Ok)
                .unwrap_or_else(|| self.connect())?;
...
impl SQLiteWrite {
    ...
    fn finish(&mut self, sql: &str) -> Result<(), StorageError> {
...
fn initialize_database(path: &Path) -> Result<Connection, StorageError> {
...
fn legacy_table_exists(conn: &Connection) -> Result<bool, StorageError> {
    let mut stmt = conn
...
fn pin_read_snapshot(conn: &Connection) -> Result<(), StorageError> {
    let mut stmt = conn
...
fn sqlite_user_version(conn: &Connection) -> Result<u32, StorageError> {
    let value = conn
        .pragma_query_value(None, "user_version", |row| row.get::<_, i64>(0))
        .map_err(sqlite_error)?;
    u32::try_from(value)
        .map_err(|_| StorageError::Corruption(format!("sqlite user_version was negative: {value}")))
}

fn execute_cached(conn: &Connection, sql: &str) -> Result<(), StorageError> {
    let mut stmt = conn.prepare_cached(sql).map_err(sqlite_error)?;
...
fn read_points_chunk(
    ...
    while let Some(row) = rows.next().map_err(sqlite_error)? {
        ...
        let index = usize::try_from(index).map_err(|_| {
            StorageError::Corruption(format!("sqlite requested ordinal was negative: {index}"))
        })?;
...
fn blob_ref<'a>(value: SqlValueRef<'a>, column: &str) -> Result<&'a [u8], StorageError> {
    match value {
...
fn ignore_no_transaction(error: StorageError) -> Result<(), StorageError> {
    match error {
