...
* typed `EntityPk` arity and component kinds (text, integer, UUID);
...
path-aware: it scans the direct SQL reader closure strictly, reports deferred
writer/GC row-group symbols separately, and rejects any direct fallback or
second authority. It requires the ForkTree boundary symbols and rejects:

* `TrackedHeadContext`, `TrackedStateStoreReader`, and legacy tracked-head or
  branch-control acquisition in the direct SQL reader closure;
...
On exact b59 it is expected to remain red because the broader legacy context,
columnar writers, and GC physical owners still exist. That expected red is
