...
| `conversations_before_resolved.lixsnap` | Generated on 2026-09-23 at Lix `fc455768b` (conversation titles, before `detached_target` and `resolved`) by a throwaway public-API test that inserts a file, a file-targeted conversation titled "Launch plan", a standalone conversation, one comment each, a checkpoint, a global commit discussion, and one more standalone conversation, then exports a snapshot. The IDs are the constants in `tests/conversation_resolved_upgrade.rs`. | Proves built-in rows written before appended `lix_conversation` columns open, read defaults (`resolved = false`), update, detach, and appear in `lix_as_of` without migration. | `91bd497fff0170d9951074611e16ab23618d43ed26f91e20b9cb569df079b459` |
