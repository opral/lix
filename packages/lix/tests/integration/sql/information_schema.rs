...
simulation_test!(
    logical_discovery_matches_json_and_row_ref_results_across_surfaces,
    |sim| async move {
        use lix::ResultColumnType;
        ...
        let inserted = session.execute(
            "INSERT INTO lix_key_value (key, value) VALUES ('logical-types', CAST('{\"nested\":[1,true,null]}' AS JSONB)) RETURNING value",
            &[],
        ).await.expect("JSONB insert should return native JSON");
        assert_eq!(inserted.column_types(), [ResultColumnType::Jsonb]);
        assert_rows_eq(
            inserted,
            vec![vec![Value::Jsonb(
                serde_json::json!({"nested": [1, true, null]}).into(),
            )]],
        );

        for (function, surface, column) in [
            (
                "lix_as_of",
                "lix_as_of('lix_key_value', lix_active_branch_commit_id())",
                "value",
            ),
            ("lix_history", "lix_history('lix_key_value')", "to_value"),
            (
                "lix_diff",
            ...
            let contract = session.execute(
                "SELECT data_type FROM information_schema.table_functions WHERE function_name = $1 AND source_relation = 'lix_key_value' AND result_column = $2",
                &[Value::Text(function.into()), Value::Text(column.into())],
            ).await.expect("function logical contract should be discoverable");
            assert_rows_eq(contract, vec![vec![Value::Text("JSONB".into())]]);
            let result = session
                .execute(
                    &format!(
                        "SELECT CAST({column} AS JSONB) FROM {surface} WHERE key = 'logical-types'"
                    ),
                    &[],
                )
                .await
                .expect("advertised JSONB cast should preserve native JSON");
            assert_eq!(result.column_types(), [ResultColumnType::Jsonb]);
            assert!(!result.rows().is_empty());
            assert!(
                result
                    .rows()
                    .iter()
                    .all(|row| matches!(&row.values()[0], Value::Jsonb(_)))
            );

            if function != "lix_as_of" {
                let contract = session.execute(
                    "SELECT data_type FROM information_schema.table_functions WHERE function_name = $1 AND source_relation = 'lix_key_value' AND result_column = 'row_ref'",
                    &[Value::Text(function.into())],
                ).await.expect("row reference contract should be discoverable");
                assert_rows_eq(contract, vec![vec![Value::Text("ROW_REF".into())]]);
                let result = session
                    .execute(
                        &format!("SELECT row_ref FROM {surface} WHERE key = 'logical-types'"),
                        &[],
                    )
                    .await
                    .expect("advertised ROW_REF should match native row references");
                assert_eq!(result.column_types(), [ResultColumnType::RowRef]);
                assert!(!result.rows().is_empty());
                assert!(
                    result
                        .rows()
                        .iter()
                        .all(|row| matches!(&row.values()[0], Value::RowRef(_)))
                );
            }
        }
        let columns = session
            .execute(
                "SELECT * FROM information_schema.table_functions LIMIT 0",
                &[],
            )
            .await
            .expect("function discovery should be readable");
        assert!(
            !columns
                .columns()
                .iter()
