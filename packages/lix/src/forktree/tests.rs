...
async fn selected_commit_member_authenticates_canonical_owner_source_and_generation() {
    ...
    let source_state = selected.source_state;
    assert_eq!(source_commit.commit_id, seed.commit_id);
    assert_eq!(
        source_state.change_id,
...
fn immutable_objects_and_typed_state_codecs_fail_closed() {
    ...
    *malformed_tombstone.last_mut().expect("state ref tag") = 2;
    ...
    assert_eq!(decoded_key.schema_key, "app_row");
    let row_pk = RowPk::single("typed-key");
    assert!(super::encode_state_row_prefix("app_row", &row_pk).len() < key.len());
    let live_key = encode_state_key(StateKeyRef {
        schema_key: "app_row",
        file_id: None,
        row_pk: &RowPk::single("live"),
    });
...
fn canonical_prefix_bounds_handle_carry_and_reject_invalid_untracked_bounds() {
    ...
    let bounds = super::encode_state_row_prefix_bounds("app_row", &row_pk);
...
async fn coherent_state_point_and_range_preserve_overlay_semantics() {
    ...
    let bounds = super::encode_state_row_prefix_bounds("app_row", &RowPk::empty());
    ...
    let replacement_bounds =
        super::encode_state_row_prefix_bounds("app_row", &RowPk::empty());
    ...
    assert!(
        view.replace_state_tree_range(
            view.branch_snapshot().local_state_root,
            super::encode_state_row_prefix_bounds("app_row", &RowPk::empty()).lower,
            super::encode_state_row_prefix_bounds("app_row", &RowPk::empty()).upper,
            vec![(
                seed.state_keys[0].clone(),
