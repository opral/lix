    ...
    S: StorageAdapterRead,
{
    ...
    pub(crate) async fn validate_tree_diff_batch_and_load_payloads(
        ...
        TrackedStatePayloadBatch::from_payloads_with_typed(
            changes
                .into_iter()
        ...
        if let Some(base_root) = base_root.as_ref() {
            ...
            for delta in &deltas {
                ...
                for scope in [
                    ...
                    delta.file_id.map(|file_id| (delta.schema_key, Some(file_id))),
                ]
                .into_iter()
                .flatten()
                {
...
fn validate_diff_row_against_changelog(
    ...
    let change_deleted = change.snapshot.is_none() && change.typed_payload.is_none();
