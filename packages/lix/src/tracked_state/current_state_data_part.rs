...
//! a rebuildable serving accelerator. Every live row carries native typed bytes.
...
#[derive(Debug, Clone, PartialEq, musli::Encode, musli::Decode)]
#[musli(packed)]
...
fn validate_rows(rows: &[CurrentStateDataRow]) -> Result<(), LixError> {
    if rows.is_empty()
        ...
        || rows
            .iter()
            .any(|row| row.encoded_key.is_empty() || row.value.deleted || row.snapshot.is_empty())
        || rows
            .windows(2)
...
mod tests {
    ...
    fn native_parts_round_trip_exact_provenance_and_reject_corruption() {
        ...
        rows[0].metadata = Some(lix_schema::Jsonb::from_value(
    ...
    fn native_parts_round_trip_snapshot_without_json() {
        ...
        assert_eq!(
            ...
            .expect("typed payload decodes against its current-state envelope"),
            typed
        );
    }
}
