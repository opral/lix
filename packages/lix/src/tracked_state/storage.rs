...
use crate::tracked_state::types::{
    AuthoredHistoryBodyCoordinate, AuthoredHistoryBodyEntry, AuthoredHistoryBodyInventory,
    ColumnarPageSource,
...
// Version 17 removes whole-row JSON from durable commit history. Authored
// members carry only a domain-separated digest of the body selected by this
// commit's authenticated current-state root. LXCD16 is deliberately rejected
// rather than read through a compatibility decoder.
const COMMIT_DELTA_FORMAT_MAGIC: &[u8] = b"LXCD17";
...
struct CommitDeltaPayloadRef<'a> {
    ...
    authored: bool,
    #[musli(with = storage_codec::option)]
    tombstone_metadata: Option<NativeHistoryMetadata>,
}
...
struct CommitDeltaAuthoredPayloadRef<'a> {
    ...
    base_coordinate: Option<TrackedStateBaseCoordinate>,
    #[musli(with = storage_codec::option)]
    tombstone_metadata: Option<&'a NativeHistoryMetadata>,
}

const NATIVE_HISTORY_METADATA_INLINE: u8 = 1;
const NATIVE_HISTORY_METADATA_REF: u8 = 2;

/// The only payload retained for an authored tombstone. This is a typed
/// metadata scalar, never a serialized row or snapshot fallback.
#[derive(Clone, Debug, musli::Encode, musli::Decode)]
#[musli(packed)]
struct NativeHistoryMetadata {
    kind: u8,
    #[musli(bytes)]
    payload: Vec<u8>,
}

fn native_history_metadata(
    slot: crate::json_store::JsonSlotRef<'_>,
) -> Option<NativeHistoryMetadata> {
    match slot {
        crate::json_store::JsonSlotRef::None => None,
        crate::json_store::JsonSlotRef::Inline(json) => Some(NativeHistoryMetadata {
            kind: NATIVE_HISTORY_METADATA_INLINE,
            payload: json.as_bytes().to_vec(),
        }),
        crate::json_store::JsonSlotRef::Ref(reference) => Some(NativeHistoryMetadata {
            kind: NATIVE_HISTORY_METADATA_REF,
            payload: reference.as_hash_bytes().to_vec(),
        }),
    }
}

fn decode_native_history_metadata(
    metadata: Option<NativeHistoryMetadata>,
) -> Result<crate::json_store::JsonSlot, LixError> {
    let Some(metadata) = metadata else {
        return Ok(crate::json_store::JsonSlot::None);
    };
    match metadata.kind {
        NATIVE_HISTORY_METADATA_INLINE => {
            let json = String::from_utf8(metadata.payload).map_err(|_| {
                replacement_payload_error("native history metadata is not UTF-8")
            })?;
            if json.len() > crate::json_store::JSON_INLINE_MAX_BYTES {
                return Err(replacement_payload_error(
                    "native history metadata has a noncanonical inline layout",
                ));
            }
            Ok(crate::json_store::JsonSlot::Inline(json.into_boxed_str()))
        }
        NATIVE_HISTORY_METADATA_REF => {
            let hash: [u8; 32] = metadata.payload.try_into().map_err(|_| {
                replacement_payload_error("native history metadata ref has the wrong length")
            })?;
            Ok(crate::json_store::JsonSlot::Ref(
                crate::json_store::JsonRef::from_hash_bytes(hash),
            ))
        }
        _ => Err(replacement_payload_error(
            "native history metadata has an unsupported layout",
        )),
    }
}

fn validate_native_history_member_metadata(
    ...
    if value.deleted {
        if body_digest
            != history_body_digest(
                ...
                metadata.as_ref_slot(),
            )
        {
            return Err(replacement_payload_error(
                "native history tombstone metadata disagrees with its body digest",
            ));
        }
    } else if metadata.is_some() {
        return Err(replacement_payload_error(
            "live native history member contains tombstone metadata",
        ));
    }
    Ok(())
}
...
fn history_body_digest(
    snapshot: crate::json_store::JsonSlotRef<'_>,
    metadata: crate::json_store::JsonSlotRef<'_>,
) -> [u8; 32] {
    fn update_slot(hasher: &mut blake3::Hasher, slot: crate::json_store::JsonSlotRef<'_>) {
        match slot {
            crate::json_store::JsonSlotRef::None => {
                hasher.update(&[0]);
            }
            crate::json_store::JsonSlotRef::Inline(json) => {
                hasher.update(&[1]);
                hasher.update(
                    crate::json_store::JsonRef::for_content(json.as_bytes()).as_hash_array(),
                );
            }
            crate::json_store::JsonSlotRef::Ref(reference) => {
                hasher.update(&[1]);
                hasher.update(reference.as_hash_array());
            }
        }
    }

    let mut hasher = blake3::Hasher::new_derive_key("lix tracked history native body v1");
    update_slot(&mut hasher, snapshot);
    update_slot(&mut hasher, metadata);
...
struct StoredCommitStateManifest {
    ...
    #[musli(with = storage_codec::option)]
    authored_history_bodies: Option<Box<AuthoredHistoryBodyInventory>>,
    #[musli(with = storage_codec::option)]
...
fn authored_history_body_binding_digest(
    encoded_key: &[u8],
    value: &TrackedStateIndexValue,
    body_digest: [u8; 32],
) -> [u8; 32] {
    let mut digest = blake3::Hasher::new_derive_key("lix authored history body member v1");
    ...
    digest.update(&(encoded_value.len() as u64).to_be_bytes());
    digest.update(&encoded_value);
    digest.update(&body_digest);
    *digest.finalize().as_bytes()
}

fn authored_history_body_closure_digest(
    commit_id: CommitId,
    inventory: &AuthoredHistoryBodyInventory,
) -> Result<[u8; 32], LixError> {
    let mut digest = blake3::Hasher::new_derive_key("lix authored history body inventory v1");
    digest.update(commit_id.as_uuid().as_bytes());
    digest.update(&inventory.source_root_id);
    digest.update(&inventory.source_transition_digest);
    digest.update(&(inventory.descriptors.len() as u64).to_be_bytes());
    for descriptor in &inventory.descriptors {
        let encoded = storage_codec::encode("authored history body descriptor", descriptor)?;
        digest.update(&(encoded.len() as u64).to_be_bytes());
        digest.update(&encoded);
    }
    digest.update(&(inventory.entries.len() as u64).to_be_bytes());
    for entry in &inventory.entries {
        digest.update(entry.change_id.as_uuid().as_bytes());
        digest.update(&entry.binding_digest);
        match entry.coordinate {
            Some(coordinate) => {
                digest.update(&[1]);
                digest.update(&coordinate.descriptor_index.to_be_bytes());
                digest.update(&coordinate.row_index.to_be_bytes());
            }
            None => {
                digest.update(&[0]);
            }
...
fn validate_authored_history_body_inventory_shape(
    commit_id: CommitId,
    inventory: &AuthoredHistoryBodyInventory,
) -> Result<(), LixError> {
    if inventory
        ...
        .iter()
        .enumerate()
        .any(|(index, descriptor)| inventory.descriptors[..index].contains(descriptor))
    {
        return Err(replacement_payload_error(
            "history body inventory contains a duplicate descriptor",
        ));
    }
    let mut change_ids = BTreeSet::new();
    let mut coordinates = BTreeSet::new();
    let mut next_descriptor = 0u32;
    let mut active_descriptor = None;
    for entry in &inventory.entries {
        if !change_ids.insert(entry.change_id) {
            return Err(replacement_payload_error(
                "history body inventory contains a duplicate change",
            ));
        }
        ...
        let descriptor = inventory
            .descriptors
            .get(usize::try_from(coordinate.descriptor_index).expect("u32 fits usize"))
            .ok_or_else(|| replacement_payload_error("history body descriptor is out of bounds"))?;
        if coordinate.row_index >= descriptor.row_count {
            return Err(replacement_payload_error(
                "history body row coordinate is out of bounds",
            ));
        }
        if !coordinates.insert((coordinate.descriptor_index, coordinate.row_index)) {
            return Err(replacement_payload_error(
                "history body inventory contains a duplicate row coordinate",
            ));
        }
        match active_descriptor {
            Some(index) if index == coordinate.descriptor_index => {}
            _ if coordinate.descriptor_index == next_descriptor => {
                active_descriptor = Some(coordinate.descriptor_index);
                next_descriptor = next_descriptor.checked_add(1).ok_or_else(|| {
                    replacement_payload_error("history body descriptor order overflows")
                })?;
            }
            _ => {
                return Err(replacement_payload_error(
                    "history body inventory has noncanonical descriptor order",
                ));
            }
        }
    }
    if usize::try_from(next_descriptor).expect("u32 fits usize") != inventory.descriptors.len() {
        return Err(replacement_payload_error(
            "history body inventory contains an unreferenced descriptor",
        ));
    }
    if authored_history_body_closure_digest(commit_id, inventory)? != inventory.closure_digest {
        return Err(replacement_payload_error(
            "history body inventory disagrees with its ordered entry closure",
        ));
    }
    Ok(())
}

fn validate_authored_history_body_inventory(
    state: &AuthenticatedReplayCommitStateManifest,
    members: &[CommitDeltaMember],
    inventory: &AuthoredHistoryBodyInventory,
) -> Result<(), LixError> {
    validate_authored_history_body_inventory_shape(state.commit_id, inventory)?;
    let root = state.current_state_scoped_ranges.as_ref().ok_or_else(|| {
        replacement_payload_error("history body inventory has no selected source root binding")
    })?;
    if inventory.source_root_id != root.tree.root_id
        || inventory.source_transition_digest != root.transition_digest
    {
        return Err(replacement_payload_error(
            "history body inventory disagrees with its selected source root",
        ));
    }
    for descriptor in &inventory.descriptors {
        let part = crate::tracked_state::current_state_envelope::scoped_range_part_from_current_state_descriptor(
            &CommitDeltaReplacementScope {
                schema_key: decode_key(&descriptor.first_key)?.schema_key,
                file_id: decode_key(&descriptor.first_key)?.file_id,
            },
            descriptor,
        )?;
        if crate::tracked_state::current_state_envelope::current_state_descriptor_from_scoped_range_part(&part)?
            != *descriptor
        {
            return Err(replacement_payload_error(
                "history body inventory contains a noncanonical descriptor",
            ));
        }
    }
    ...
    for member in members.iter().filter(|member| member.authored) {
        ...
        let entry = inventory_entries.get(&member.value.change_id).ok_or_else(|| {
            replacement_payload_error("history body inventory omitted a requested member")
        })?;
        if entry.change_id != member.value.change_id
            || entry.binding_digest
                != authored_history_body_binding_digest(
                    &encoded_key,
                    &member.value,
                    member.body_digest,
                )
        {
            return Err(replacement_payload_error(
                "history body entry disagrees with its authored member",
            ));
        }
    }
    Ok(())
...
fn validate_loaded_history_body_inventory(
    state: &AuthenticatedReplayCommitStateManifest,
    entries: &[LoadedCommitDeltaEntry],
    inventory: &AuthoredHistoryBodyInventory,
) -> Result<(), LixError> {
    validate_authored_history_body_inventory_shape(state.commit_id, inventory)?;
    let root = state.current_state_scoped_ranges.as_ref().ok_or_else(|| {
    ...
    let inventory_entries = inventory
        ...
        .map(|entry| (entry.change_id, entry))
        .collect::<BTreeMap<_, _>>();
    for entry in entries.iter().filter(|entry| entry.authored) {
        let encoded_key = encode_key_ref(TrackedStateKeyRef {
            schema_key: &entry.change_record.schema_key,
            file_id: entry.change_record.file_id.as_deref(),
            row_pk: &entry.change_record.row_pk,
        });
        let locator = inventory_entries
            .get(&entry.value.change_id)
            .ok_or_else(|| replacement_payload_error("history body inventory omitted a requested member"))?;
        if locator.binding_digest
            != authored_history_body_binding_digest(&encoded_key, &entry.value, entry.body_digest)
        {
            return Err(replacement_payload_error(
                "history body inventory disagrees with a requested member",
            ));
        }
    }
    Ok(())
...
pub(super) async fn certify_authored_history_body_inventory(
    store: &(impl StorageAdapterRead + ?Sized),
...
) -> Result<Option<AuthoredHistoryBodyInventory>, LixError> {
    let staged_segments = staged_commit_delta_segment_bytes(writes, commit_id, mutations)?;
    let members = staged_commit_delta_members(
        store,
        writes,
        commit_id,
        account_id,
    ...
    .await?;
    let authored = members.iter().filter(|member| member.authored).collect::<Vec<_>>();
    if authored.is_empty() {
        return Ok(None);
    }
    let root = root.ok_or_else(|| {
        replacement_payload_error("authored history body inventory has no canonical source root")
    })?;
    let reachable = crate::tracked_state::scoped_range::validate_scoped_range_tree_with_staged(
        store,
        writes,
        &root.tree,
    )
    ...
    let mut candidates = reachable
        .parts
        .iter()
        .map(crate::tracked_state::current_state_envelope::current_state_descriptor_from_scoped_range_part)
        .collect::<Result<Vec<_>, LixError>>()?;
    candidates.sort_unstable_by(|left, right| left.first_key.cmp(&right.first_key));
    if candidates
        .windows(2)
        .any(|pair| pair[0].last_key >= pair[1].first_key)
    {
        return Err(replacement_payload_error(
            "authenticated current-state descriptors overlap or are unordered",
        ));
    }
    let mut descriptors = Vec::<CurrentStatePartDescriptor>::new();
    let mut descriptor_indices = BTreeMap::<usize, usize>::new();
    let mut entries = Vec::with_capacity(authored.len());
    let mut loaded = BTreeMap::<usize, Vec<crate::tracked_state::current_state_data_part::CurrentStateDataRow>>::new();
    for member in authored {
        ...
        let binding_digest = authored_history_body_binding_digest(
            &encoded_key,
            &member.value,
            member.body_digest,
        );
        if member.value.deleted {
            if member.body_digest
                != history_body_digest(
                    crate::json_store::JsonSlotRef::None,
                    member.change.metadata.as_ref_slot(),
                )
            {
                return Err(replacement_payload_error(
                    "authored history tombstone carries a native body digest",
                ));
            }
            entries.push(AuthoredHistoryBodyEntry {
        ...
        let candidate_index = candidates.partition_point(|descriptor| {
            descriptor.last_key.as_slice() < encoded_key.as_slice()
        });
        let Some(descriptor) = candidates.get(candidate_index) else {
            return Err(replacement_payload_error(
                "authored history body does not resolve to one canonical source descriptor",
            ));
        };
        if descriptor.first_key.as_slice() > encoded_key.as_slice()
            || candidates.get(candidate_index + 1).is_some_and(|next| {
                next.first_key.as_slice() <= encoded_key.as_slice()
                    && encoded_key.as_slice() <= next.last_key.as_slice()
            })
        {
            return Err(replacement_payload_error(
                "authored history body does not resolve to one canonical source descriptor",
        ...
        if !loaded.contains_key(&candidate_index) {
            loaded.insert(
                candidate_index,
                load_scoped_current_state_descriptor_rows(
                    store,
                    writes,
        ...
        let rows = &loaded[&candidate_index];
        let row_index = rows
            .binary_search_by(|row| row.encoded_key.as_slice().cmp(encoded_key.as_slice()))
            .map_err(|_| replacement_payload_error("authored history body is absent from its canonical source"))?;
        if !authored_history_body_matches_current_row(
            ...
            return Err(replacement_payload_error(
                "authored history body disagrees with its canonical source row",
            ));
        }
        let descriptor_index = match descriptor_indices.get(&candidate_index).copied() {
            Some(index) => index,
            None => {
                let index = descriptors.len();
                descriptors.push(descriptor.clone());
                descriptor_indices.insert(candidate_index, index);
                index
            }
        };
        entries.push(AuthoredHistoryBodyEntry {
            change_id: member.value.change_id,
            binding_digest,
            coordinate: Some(AuthoredHistoryBodyCoordinate {
                descriptor_index: u32::try_from(descriptor_index).map_err(|_| {
                    replacement_payload_error("history body descriptor count overflows")
                })?,
                row_index: u16::try_from(row_index).map_err(|_| {
                    replacement_payload_error("history body row coordinate overflows")
                })?,
            }),
        });
    }
    let mut inventory = AuthoredHistoryBodyInventory {
        source_root_id: root.tree.root_id,
        source_transition_digest: root.transition_digest,
        closure_digest: [0; 32],
        descriptors,
        entries,
    };
    inventory.closure_digest = authored_history_body_closure_digest(commit_id, &inventory)?;
    Ok(Some(inventory))
}

#[cfg(test)]
pub(crate) async fn certify_authored_history_body_inventory_for_test(
    store: &(impl StorageAdapterRead + ?Sized),
    writes: &mut StorageWriteSet,
    commit_id: CommitId,
    account_id: &str,
    ...
    certify_authored_history_body_inventory(
        store,
        writes,
        commit_id,
        account_id,
...
async fn hydrate_authored_history_members(
    store: &(impl StorageAdapterRead + ?Sized),
    state: &AuthenticatedReplayCommitStateManifest,
    ...
    let inventory = state.authored_history_bodies.as_ref().ok_or_else(|| {
        replacement_payload_error(&format!(
            "authored history member for commit '{}' has no canonical body inventory",
            state.commit_id
        ))
    })?;
    validate_authored_history_body_inventory(state, members, inventory)?;
    let mut writes = StorageWriteSet::new();
    let mut loaded = BTreeMap::<u32, Vec<crate::tracked_state::current_state_data_part::CurrentStateDataRow>>::new();
    let inventory_entries = inventory
        .entries
        .iter()
        .map(|entry| (entry.change_id, entry))
        .collect::<BTreeMap<_, _>>();
    for (member_index, _) in authored {
        let member = &mut members[member_index];
        // `validate_authored_history_body_inventory` proves that authored
        // members and inventory entries are the same canonical ordered
        // closure. Consume that proof directly instead of rescanning the
        // complete inventory once per member.
        let coordinate = inventory_entries
        ...
        if member.value.deleted {
            ...
            {
                return Err(replacement_payload_error(
                    "history tombstone disagrees with its selected current-state root",
                ));
            }
            continue;
        }
        let coordinate = coordinate.ok_or_else(|| {
            replacement_payload_error("live authored history member has no body coordinate")
        })?;
        let descriptor = inventory
            .descriptors
            .get(usize::try_from(coordinate.descriptor_index).expect("u32 fits usize"))
            .ok_or_else(|| replacement_payload_error("history body descriptor is out of bounds"))?;
        if !loaded.contains_key(&coordinate.descriptor_index) {
            loaded.insert(
                coordinate.descriptor_index,
                load_scoped_current_state_descriptor_rows(store, &mut writes, descriptor).await?,
            );
        }
        let row = loaded[&coordinate.descriptor_index]
            .get(usize::from(coordinate.row_index))
            .ok_or_else(|| replacement_payload_error("history body row coordinate is out of bounds"))?
            .clone();
        if !authored_history_body_matches_current_row(
            &row,
            &member.value,
...
async fn hydrate_loaded_history_entries(
    store: &(impl StorageAdapterRead + ?Sized),
    state: &AuthenticatedReplayCommitStateManifest,
    entries: &mut [LoadedCommitDeltaEntry],
) -> Result<(), LixError> {
    if entries.is_empty() {
        return Ok(());
    }
    let authored_indices = entries
        .iter()
        .enumerate()
        .filter_map(|(index, entry)| entry.authored.then_some(index))
        .collect::<Vec<_>>();
    if authored_indices.is_empty() {
        return Ok(());
    }
    let inventory = state.authored_history_bodies.as_ref().ok_or_else(|| {
        replacement_payload_error("authored history entry has no canonical body inventory")
    })?;
    validate_loaded_history_body_inventory(state, entries, inventory)?;
    let inventory_entries = inventory
        .entries
        .iter()
        .map(|entry| (entry.change_id, entry))
    ...
    for index in authored_indices {
        ...
        if entry.value.deleted {
            if coordinate.is_some()
                || entry.body_digest
                    != history_body_digest(
                        crate::json_store::JsonSlotRef::None,
                        entry.change_record.metadata.as_ref_slot(),
                    )
            {
                return Err(replacement_payload_error(
                    "history tombstone disagrees with its selected current-state root",
        ...
        let coordinate = coordinate.ok_or_else(|| {
            replacement_payload_error("live authored history entry has no body coordinate")
        })?;
        let descriptor = inventory
            .descriptors
            .get(usize::try_from(coordinate.descriptor_index).expect("u32 fits usize"))
...
pub(crate) async fn stage_sparse_current_state_scoped_range(
    ...
    for member in members {
        ...
        if !member.authored {
            return Err(replacement_payload_error(
                "sparse authored current-state rewrite received a selected member",
            ));
        }
        let row = certified_rows.remove(&encoded_key).ok_or_else(|| {
            replacement_payload_error(
                "authored current-state rewrite omitted its same-publication body certification",
            )
        })?;
        if row.as_ref().is_some_and(|row| row.value != member.value)
            || (row.is_none() != member.value.deleted)
        {
            return Err(replacement_payload_error(
                "authored current-state body certification disagrees with member identity",
            ));
        }
...
/// Opaque, single-use proof that the authored bodies for one staged LXCD17
/// write set match its complete immutable member closure. The rows exist only
/// in the publication plan; durable bytes are staged once as canonical native
/// current-state parts.
pub(crate) struct CertifiedAuthoredCurrentStateBody {
    write_set_id: u64,
    commit_id: CommitId,
...
impl CertifiedAuthoredCurrentStateBody {
    pub(super) fn into_publication_parts(
        ...
        if self.write_set_id != writes.identity()
            ...
            || self.mutation_authority_digest
                != super::scoped_current_state::current_state_mutation_authority_digest(inventory)?
        {
            return Err(replacement_payload_error(
                "authored current-state body certification belongs to another publication",
            ));
        }
        if self.staged_member_closure_digest
            != staged_member_closure_digest(writes, commit_id, inventory)?
        {
            return Err(replacement_payload_error(
                "staged member closure changed after authored current-state certification",
            ));
        }
...
fn staged_member_closure_digest(
    writes: &StorageWriteSet,
    commit_id: CommitId,
    inventory: &CommitStateMutationInventory,
) -> Result<[u8; 32], LixError> {
    let mut digest =
        blake3::Hasher::new_derive_key("lix initial current-state staged member closure v1");
    digest.update(commit_id.as_uuid().as_bytes());
    digest
        .update(&super::scoped_current_state::current_state_mutation_authority_digest(inventory)?);
    let segments = staged_commit_delta_segment_bytes(writes, commit_id, inventory)?;
    digest.update(&(segments.len() as u64).to_be_bytes());
    for segment in segments {
        let segment = segment.ok_or_else(|| {
            replacement_payload_error(
                "initial current-state certification is missing a staged member segment",
            )
        })?;
        digest.update(&(segment.len() as u64).to_be_bytes());
        digest.update(blake3::hash(&segment).as_bytes());
    }
    Ok(*digest.finalize().as_bytes())
}

pub(crate) async fn certify_authored_current_state_body<'a>(
    store: &(impl StorageAdapterRead + ?Sized),
    writes: &mut StorageWriteSet,
    commit_id: CommitId,
    account_id: &str,
    inventory: &CommitStateMutationInventory,
    stage_initial_root: bool,
    deltas: impl IntoIterator<Item = TrackedStateCommitDeltaRef<'a>>,
) -> Result<Option<CertifiedAuthoredCurrentStateBody>, LixError> {
    use crate::tracked_state::current_state_data_part::CurrentStateDataRow;
    use crate::tracked_state::current_state_envelope::{
        current_state_scope_prefix, scoped_range_part_from_current_state_descriptor,
    };
    use crate::tracked_state::scoped_range::{ScopedRangeCoverageMarker, stage_scoped_range_tree};

    // Selected-source members carry authenticated identity/provenance only;
    // their body remains owned by the selected commit. Current-state body
    // certification therefore consumes exactly the authored subset and must
    // never reinterpret selected references as newly-authored rows.
    let deltas = deltas
        .into_iter()
        .filter(|delta| delta.authored)
        .collect::<Vec<_>>();
    ...
    // A certified complete replacement generation owns the entire current
    // body set through its authenticated packed-generation descriptor. The
    // ordinary row journal is intentionally empty for that path; requiring a
    // second row-shaped body would duplicate authority. The final publication
    // still certifies every authored history member against the resulting
    // current-state root before the manifest can be staged.
    if deltas.is_empty() && inventory.replacement_generation.is_some() {
        return Ok(None);
    }
    let staged_segments = staged_commit_delta_segment_bytes(writes, commit_id, inventory)?;
    let staged_members =
        staged_commit_delta_members(
            store,
            writes,
            commit_id,
            account_id,
    ...
    for member in &staged_members {
        ...
        let member_key = encode_key_ref(TrackedStateKeyRef {
            schema_key: &member.key.schema_key,
            file_id: member.key.file_id.as_deref(),
            row_pk: &member.key.row_pk,
        });
        ...
        let member_matches_current_identity = member.value.change_id == delta.delta.change_id
            ...
            && member.value.created_at == delta.delta.created_at
    ...
    if authored_deltas.is_empty() {
        return Ok(None);
    }
    let mut scopes = BTreeMap::<CommitDeltaReplacementScope, Vec<CurrentStateDataRow>>::new();
    let mut certified_rows = BTreeMap::new();
    for delta in authored_deltas {
        if delta.delta.commit_id != commit_id {
            return Err(replacement_payload_error(
                "authored current-state body belongs to another commit",
            ));
        }
        let scope = CommitDeltaReplacementScope {
            schema_key: delta.delta.schema_key.to_owned(),
            file_id: delta.delta.file_id.map(str::to_owned),
        };
        ...
        let row = (!delta.delta.deleted).then(|| CurrentStateDataRow {
                ...
                snapshot: owned_scoped_json_slot(delta.snapshot),
                metadata: owned_scoped_json_slot(delta.metadata),
            });
        if let Some(row) = row.clone() {
            rows.push(row);
        }
        if certified_rows.insert(encoded_key, row).is_some() {
            return Err(replacement_payload_error(
                "authored current-state certification contains a duplicate identity",
            ));
        }
    }
    if scopes.is_empty() {
    ...
    let initial_root = if stage_initial_root {
        let mut staged_scopes = Vec::with_capacity(scopes.len());
        for (scope, mut rows) in scopes {
        ...
        {
            return Err(replacement_payload_error(
                "initial current-state pack contains a duplicate identity",
            ));
        }
        let mut descriptors = Vec::new();
        stage_scoped_native_current_state_rows(writes, &rows, false, &mut descriptors)?;
        ...
        let tree = stage_scoped_range_tree(writes, staged_scopes)?;
        Some(super::scoped_current_state::attest_scoped_range_root(
            commit_id, None, inventory, tree,
        )?)
    } else {
        None
    };
    Ok(Some(CertifiedAuthoredCurrentStateBody {
        ...
        CurrentStatePartSource::NativeDataPart { .. } => {
                ...
                writes.staged_value(CURRENT_STATE_DATA_PART_SPACE, &physical_key)
            {
                ...
                get_one(store, CURRENT_STATE_DATA_PART_SPACE, physical_key)
                    ...
                    .ok_or_else(|| {
                        replacement_payload_error(&format!(
                            "native current-state part {:?} is missing",
                            descriptor.content_digest
                        ))
                    })?
            };
...
pub(crate) async fn staged_commit_delta_members(
    ...
    let manifest = commit_delta_manifest_from_inventory(inventory);
    if let Some(parts) = inventory.columnar_parts.as_ref() {
        return load_columnar_mutation_members_with_staged(
            store,
            Some(writes),
            commit_id,
            parts,
...
pub(crate) fn stage_commit_state_manifest(
    ...
    if manifest.authored_history_bodies.is_some() {
        return Err(replacement_payload_error(
            "authored body inventories require canonical publication certification",
        ));
    }
...
pub(crate) fn stage_certified_commit_state_manifest(
    ...
    if manifest.authored_history_bodies != publication.authored_history_bodies() {
        return Err(replacement_payload_error(
            "commit manifest disagrees with its certified authored body inventory",
        ));
    }
    if (manifest.mutations.member_count != 0
        || manifest.mutations.selected_source_commit_id().is_some())
...
fn encode_ordered_addressable_commit_delta_segment<'a>(
    ...
    for (ordinal, delta) in deltas.enumerate() {
        ...
        payloads.push(CommitDeltaPayloadRef {
            body_digest: history_body_digest(delta.snapshot, delta.metadata),
            origin_key: delta.origin_key,
            base_coordinate: delta.base_coordinate,
            authored: delta.authored,
            tombstone_metadata: (delta.authored && delta.delta.deleted)
                .then(|| native_history_metadata(delta.metadata))
                .flatten(),
        });
    }
...
async fn route_direct_change_records_for_state(
    ...
    let unique = if let Some(root) = state.mutation_directory_root.as_ref() {
        ...
        if owned_indices.is_empty() {
            ...
            let mut loaded = Vec::with_capacity(owned_indices.len());
            for &selector_index in &owned_indices {
                let locator_index = unique_locator_indices[selector_index];
                loaded.push(decode_change_at_locator_from_decoded(
                    &leaf,
                    &payloads,
            ...
            hydrate_loaded_history_entries(store, state, &mut loaded).await?;
            for (selector_index, entry) in owned_indices.into_iter().zip(loaded) {
...
async fn load_physical_direct_change_records(
    ...
    let mut native_entries = Vec::<(usize, LoadedCommitDeltaEntry)>::new();
    ...
    let mut entries = native_entries
        .iter()
        .map(|(_, entry)| entry.clone())
        .collect::<Vec<_>>();
    hydrate_loaded_history_entries(store, state, &mut entries).await?;
    output.extend(
        native_entries
            .into_iter()
            .map(|(index, _)| index)
            .zip(entries.into_iter().map(|entry| entry.change_record)),
    );
...
async fn load_selected_change_records_from_state(
    ...
    if state.mutations.columnar_parts.is_none() && state.mutations.direct_part_row_counts.is_empty()
    {
        return load_generic_change_records_from_state(store, state, locators).await;
    }
    #[cfg(any(test, feature = "storage-benches"))]
    let mut accounting_guard = super::mutation_directory::DirectRouteAccountingGuard::new();
    #[cfg(any(test, feature = "storage-benches"))]
    ...
    let unique_records = if let Some(root) = state.mutation_directory_root.as_ref() {
        ...
        let mut loaded = unique_locators
            .iter()
            .map(|&locator| {
...
async fn load_generic_change_records_from_state(
    store: &(impl StorageAdapterRead + ?Sized),
    state: &AuthenticatedReplayCommitStateManifest,
    locators: &[CommitDeltaChangeLocator],
) -> Result<Vec<crate::changelog::ChangeRecord>, LixError> {
    ...
    .await?
        ...
        return Err(replacement_payload_error(
            "history locator exceeds its authenticated mutation-part bound",
        ));
    };
    hydrate_authored_history_members(store, state, &mut members).await?;
    locators
        .iter()
        .map(|locator| {
            if locator.commit_id != state.commit_id {
                return Err(replacement_payload_error(
                    "history locator names a different commit authority",
                ));
            }
            let member = members
                .iter()
                .find(|member| {
                    member.segment_index == locator.segment_index
                        && member.ordinal == u32::from(locator.ordinal)
                })
                .ok_or_else(|| replacement_payload_error("history locator lost its member"))?;
            if member.change.change_id != locator.change_id {
                return Err(replacement_payload_error(
                    "history locator points to the wrong native member",
                ));
            }
            Ok(member.change.clone())
        })
        .collect()
}
    ...
    S: AsRef<[u8]>,
{
    ...
    let (body_digest, origin_key, base_coordinate, metadata) = match payloads.decode(ordinal)? {
        CommitDeltaPayload::Authored(payload) => (
            payload.body_digest,
            payload.origin_key,
            payload.base_coordinate,
            decode_native_history_metadata(payload.tombstone_metadata)?,
        ),
    ...
    validate_native_history_member_metadata(&value, body_digest, &metadata)?;
...
async fn load_change_record_at_locator(
    ...
    let change_id = locator.change_id;
    let Some(_) = load_commit_delta_manifest(store, locator.commit_id).await? else {
        return Err(LixError::new(
            LixError::CODE_INTERNAL_ERROR,
            format!(
    ...
    let state = load_point_replay_commit_state(store, locator.commit_id)
        .await?
        .ok_or_else(|| {
            replacement_payload_error("history locator owner has no authenticated commit state")
        })?;
    load_generic_change_records_from_state(store, &state, &[locator])
        .await?
        .pop()
        .ok_or_else(|| replacement_payload_error("history locator lost its authenticated row"))
}
...
async fn load_columnar_owned_entries(
    ...
    for ((group_index, page_index), requests) in grouped {
        ...
        for (output_index, identity) in requests {
            ...
            let body_digest = history_body_digest(
                change_record.snapshot.as_ref_slot(),
                change_record.metadata.as_ref_slot(),
            );
...
async fn load_columnar_mutation_members(
    ...
    load_columnar_mutation_members_with_staged(store, None, commit_id, parts, account_id).await
}

async fn load_columnar_mutation_members_with_staged(
    store: &(impl StorageAdapterRead + ?Sized),
    ...
    let id = crate::columnar_row_group::RowGroupSetId::new(parts.row_group_set_id);
    let staged_manifest = writes
        .map(|writes| crate::columnar_row_group::load_staged_row_group_manifest(writes, id))
        .transpose()?
        .flatten();
    let manifest = match staged_manifest {
        Some(manifest) => manifest,
    ...
    for group_index in 0..manifest.groups.len() {
        let page_count = (manifest.groups[group_index].row_count as usize)
            .div_ceil(crate::columnar_row_group::ROW_GROUP_PAGE_ROWS);
        for page_index in 0..page_count {
            let batch = if let Some(writes) = writes
                && crate::columnar_row_group::load_staged_row_group_manifest(writes, id)?.is_some()
            {
                crate::columnar_row_group::load_staged_row_group_page(
                    writes,
                    id,
            ...
            let body_digest =
                history_body_digest(change.snapshot.as_ref_slot(), change.metadata.as_ref_slot());
...
async fn load_inventory_part_entries_one_ordered(
    ...
    hydrate_selected_loaded_entries(store, state, &mut output).await?;
    Ok(output)
}
...
async fn hydrate_selected_loaded_entries(
    ...
    for ((index, _), change_record) in selected.into_iter().zip(canonical) {
        ...
        entry.body_digest = history_body_digest(
            entry.change_record.snapshot.as_ref_slot(),
            entry.change_record.metadata.as_ref_slot(),
        );
    }
    let positions = entries
        .iter()
        .enumerate()
        .filter_map(|(index, entry)| entry.as_ref().map(|entry| (index, entry.clone())))
        .collect::<Vec<_>>();
    let mut loaded = positions
        .iter()
        .map(|(_, entry)| entry.clone())
        .collect::<Vec<_>>();
    hydrate_loaded_history_entries(store, state, &mut loaded).await?;
    for ((index, _), entry) in positions.into_iter().zip(loaded) {
...
fn projected_change_identity_eq(
    left: &crate::changelog::ChangeRecord,
    right: &crate::changelog::ChangeRecord,
) -> bool {
                ...
                entry_batch.iter().zip(commit_ids.into_iter().zip(states))
            {
                ...
                for member in members {
                    if member.authored {
                        ...
                        if if hydrate_authored_payloads {
                            canonical != member.change
                        } else {
                            !projected_change_identity_eq(&canonical, &member.change)
                        } {
                            return Err(LixError::new(
                                LixError::CODE_INTERNAL_ERROR,
                                format!(
...
pub(crate) async fn scan_commit_delta_inventory(
    ...
    for (&commit_id, manifest) in &manifests {
        ...
        let state = load_point_replay_commit_state(store, commit_id)
            ...
            .ok_or_else(|| {
                replacement_payload_error(
                    "commit-delta inventory lost its authenticated commit state",
                )
            })?;
        hydrate_authored_history_members(store, &state, &mut members).await?;
        hydrate_selected_members(store, &mut members).await?;
...
pub(crate) async fn collect_current_state_part_json_refs(
    store: &(impl StorageAdapterRead + ?Sized),
    part_refs_digests: &BTreeMap<[u8; 32], [u8; 32]>,
    refs: &mut BTreeSet<[u8; 32]>,
) -> Result<(), LixError> {
    if part_refs_digests.is_empty() {
        return Ok(());
    }
    let keys = part_refs_digests
        .keys()
    ...
    .materialize(store, StorageGetOptions::default())
    .await?;
    for ((part_digest, refs_digest), value) in part_refs_digests.iter().zip(loaded.value) {
        let Some(bytes) = value.and_then(full_value_bytes) else {
            return Err(LixError::new(
                LixError::CODE_INTERNAL_ERROR,
                format!(
                    "current-state data part {part_digest:?} references a missing payload-ref summary"
                ),
            ));
        };
...
pub(crate) async fn stage_retire_commit_physical_state(
    ...
    released_part_refs_digests: &mut BTreeMap<[u8; 32], [u8; 32]>,
) -> Result<(), LixError> {
    let manifest = load_commit_state_manifest(store, commit_id)
        .await?
        .ok_or_else(|| {
...
fn collect_strict_commit_delta_members(
    ...
    for entry_index in 0..leaf.len() {
        ...
        let key = decode_key(entry.key)?;
        let (body_digest, origin_key, base_coordinate, authored, selected_tombstone, metadata) =
            match payload {
                CommitDeltaPayload::Authored(payload) => (
                    payload.body_digest,
                    payload.origin_key,
...
pub(crate) fn commit_delta_replay_metadata_from_inventory(
    inventory: &CommitStateMutationInventory,
) -> CommitDeltaReplayMetadata {
...
fn encode_commit_delta_segment(entries: &[EncodedLeafEntry]) -> Vec<u8> {
    let payloads = vec![
        CommitDeltaPayloadRef {
            body_digest: history_body_digest(
                crate::json_store::JsonSlotRef::None,
                crate::json_store::JsonSlotRef::None,
            ),
            origin_key: None,
    ...
    for (entry, payload) in entries.iter().zip(payloads) {
        ...
        if payload.authored {
            ...
            let authored = CommitDeltaAuthoredPayloadRef {
                ...
                tombstone_metadata: payload.tombstone_metadata.as_ref(),
            };
    ...
    for ordinal in 0..decoded.len() {
        ...
        storage_codec::append(
            ...
            &CommitDeltaAuthoredPayloadRef {
                body_digest: history_body_digest(snapshot, metadata),
                origin_key: None,
                base_coordinate: None,
                tombstone_metadata: None,
            },
    ...
    S: AsRef<[u8]>,
{
    ...
    let key = decode_key(entry.key)?;
    let (body_digest, origin_key, base_coordinate, authored, selected_ref, metadata) = match payload {
        CommitDeltaPayload::Authored(payload) => (
            payload.body_digest,
            payload.origin_key,
            payload.base_coordinate,
...
fn encode_commit_state_manifest(
    ...
    let header = StoredCommitStateManifest {
        ...
        authored_history_bodies: manifest.authored_history_bodies.clone(),
...
fn assemble_commit_state_manifest(
    ...
    let manifest = CommitStateManifest {
        ...
        authored_history_bodies: stored.authored_history_bodies,
...
fn validate_commit_state_manifest_header(
    ...
    if let Some(inventory) = stored.authored_history_bodies.as_ref() {
        let root = stored.current_state_scoped_ranges.as_ref().ok_or_else(|| {
            replacement_payload_error("history body inventory has no current-state root binding")
        })?;
        if inventory.source_root_id != root.tree.root_id
            || inventory.source_transition_digest != root.transition_digest
            || inventory.closure_digest == [0; 32]
        {
            return Err(replacement_payload_error(
                "history body inventory has an invalid root or coordinate binding",
            ));
        }
        validate_authored_history_body_inventory_shape(stored.commit_id, inventory)?;
    }
    Ok(())
}
...
fn validate_commit_state_manifest_inner(
    ...
    if let Some(inventory) = manifest.authored_history_bodies.as_ref() {
        let root = manifest.current_state_scoped_ranges.as_ref().ok_or_else(|| {
            replacement_payload_error("history body inventory has no current-state root binding")
        })?;
        if inventory.source_root_id != root.tree.root_id
            || inventory.source_transition_digest != root.transition_digest
        ...
        validate_authored_history_body_inventory_shape(manifest.commit_id, inventory)?;
    }
...
mod tests {
    ...
    use super::{
        ...
        encode_commit_delta_segment_with_raw_sidecar, encode_commit_state_manifest,
        history_body_digest, key, load_change_record_by_id, load_commit_delta_change_ids,
    ...
    fn fixture_commit_state_manifest(
        commit_id: CommitId,
        mutations: CommitStateMutationInventory,
        current_state_scoped_ranges: Option<
        ...
        CommitStateManifest {
            ...
            current_state_scoped_ranges: current_state_scoped_ranges.map(Box::new),
            authored_history_bodies: authored_history_bodies.map(Box::new),
            snapshot_root: None,
        }
    }
    ...
    fn stage_fixture_manifest(
        ...
        stage_fixture_manifest_with_current_state(writes, commit_id, mutations, None, None)
    }

    fn stage_fixture_manifest_with_current_state(
        writes: &mut StorageWriteSet,
        commit_id: CommitId,
        mutations: &CommitStateMutationInventory,
        current_state_scoped_ranges: Option<
        ...
        super::stage_commit_state_manifest_bytes(
            writes,
            &fixture_commit_state_manifest(
                commit_id,
                mutations.clone(),
                current_state_scoped_ranges,
    ...
    struct FixtureCurrentStatePublication {
        root: crate::tracked_state::types::CurrentStateScopedRangeRoot,
        row_coordinates: BTreeMap<Vec<u8>, (CurrentStatePartDescriptor, u16)>,
    }

    fn stage_fixture_current_state_root(
        writes: &mut StorageWriteSet,
        commit_id: CommitId,
        mutations: &CommitStateMutationInventory,
        deltas: &[TrackedStateCommitDeltaRef<'_>],
    ) -> Result<Option<FixtureCurrentStatePublication>, LixError> {
        use crate::tracked_state::current_state_data_part::CurrentStateDataRow;
        use crate::tracked_state::current_state_envelope::{
            current_state_scope_prefix, scoped_range_part_from_current_state_descriptor,
        };
        use crate::tracked_state::scoped_current_state::attest_scoped_range_root;
        use crate::tracked_state::scoped_range::{
            ScopedRangeCoverageMarker, stage_scoped_range_tree,
        };

        let mut scopes = BTreeMap::<
            crate::tracked_state::types::CommitDeltaReplacementScope,
            Vec<CurrentStateDataRow>,
        >::new();
        for delta in deltas {
            let scope = crate::tracked_state::types::CommitDeltaReplacementScope {
                schema_key: delta.delta.schema_key.to_owned(),
            ...
            let rows = scopes.entry(scope).or_default();
            if delta.authored && !delta.delta.deleted {
                rows.push(CurrentStateDataRow {
                    ...
                    snapshot: super::owned_scoped_json_slot(delta.snapshot),
                    metadata: super::owned_scoped_json_slot(delta.metadata),
                });
            }
        }
        if scopes.is_empty() {
        ...
        let mut staged_scopes = Vec::with_capacity(scopes.len());
        let mut row_coordinates = BTreeMap::new();
        for (scope, mut rows) in scopes {
            rows.sort_by(|left, right| left.encoded_key.cmp(&right.encoded_key));
            let mut descriptors = Vec::new();
            super::stage_scoped_native_current_state_rows(writes, &rows, false, &mut descriptors)?;
            for descriptor in &descriptors {
                for (row_index, row) in rows
                    .iter()
                    .filter(|row| {
                        descriptor.first_key <= row.encoded_key
                            && row.encoded_key <= descriptor.last_key
                    })
                    .enumerate()
                {
                    row_coordinates.insert(
                        row.encoded_key.clone(),
                        (
                            descriptor.clone(),
                            u16::try_from(row_index).expect("native fixture part row fits u16"),
                        ),
                    );
                }
            }
        ...
        let tree = stage_scoped_range_tree(writes, staged_scopes)?;
        let root = attest_scoped_range_root(commit_id, None, mutations, tree)?;
        Ok(Some(FixtureCurrentStatePublication {
            root,
            row_coordinates,
        }))
    }

    fn fixture_authored_history_body_inventory(
        commit_id: CommitId,
        root: &crate::tracked_state::types::CurrentStateScopedRangeRoot,
        row_coordinates: &BTreeMap<Vec<u8>, (CurrentStatePartDescriptor, u16)>,
        deltas: &[TrackedStateCommitDeltaRef<'_>],
    ) -> Result<Option<AuthoredHistoryBodyInventory>, LixError> {
        let mut authored = deltas
            .iter()
            .copied()
            .filter(|delta| delta.authored)
            .map(|delta| {
        ...
        let mut descriptors = Vec::<CurrentStatePartDescriptor>::new();
        let mut descriptor_indices = BTreeMap::<Vec<u8>, u32>::new();
        let mut entries = Vec::with_capacity(authored.len());
        for (encoded_key, delta) in authored {
            ...
            let body_digest = history_body_digest(delta.snapshot, delta.metadata);
            let coordinate = if delta.delta.deleted {
                None
            } else {
                let (descriptor, row_index) =
                    row_coordinates.get(&encoded_key).ok_or_else(|| {
                        LixError::new(
                            LixError::CODE_INTERNAL_ERROR,
                            "live fixture history member has no native current-state coordinate",
                        )
                    })?;
                let descriptor_index = match descriptor_indices.get(&descriptor.first_key) {
                    Some(index) => *index,
                    None => {
                        let index = u32::try_from(descriptors.len()).map_err(|_| {
                            LixError::new(
                                LixError::CODE_INTERNAL_ERROR,
                                "fixture history descriptor count overflows",
                            )
                        })?;
                        descriptors.push(descriptor.clone());
                        descriptor_indices.insert(descriptor.first_key.clone(), index);
                        index
                    }
                };
                Some(AuthoredHistoryBodyCoordinate {
                    descriptor_index,
                    row_index: *row_index,
                })
            };
            entries.push(AuthoredHistoryBodyEntry {
                change_id: value.change_id,
                binding_digest: super::authored_history_body_binding_digest(
                    &encoded_key,
                    &value,
                    body_digest,
                ),
        ...
        inventory.closure_digest =
            super::authored_history_body_closure_digest(commit_id, &inventory)?;
        Ok(Some(inventory))
    }

    fn fixture_single_row_body_inventory(
        commit_id: CommitId,
        root: &crate::tracked_state::types::CurrentStateScopedRangeRoot,
        members: Vec<(Vec<u8>, TrackedStateIndexValue, [u8; 32])>,
    ) -> AuthoredHistoryBodyInventory {
        let descriptors = members
            .iter()
            .map(|(encoded_key, _, _)| CurrentStatePartDescriptor {
                first_key: encoded_key.clone(),
                last_key: encoded_key.clone(),
                content_digest: *blake3::hash(encoded_key).as_bytes(),
                source: CurrentStatePartSource::NativeDataPart {
                    payload_refs_digest: *blake3::hash(
                        &[b"fixture payload refs".as_slice(), encoded_key].concat(),
                    )
                    .as_bytes(),
                },
                source_row_offset: 0,
        ...
        let entries = members
            ...
            .enumerate()
            .map(
                |(index, (encoded_key, value, body_digest))| AuthoredHistoryBodyEntry {
                    change_id: value.change_id,
                    binding_digest: super::authored_history_body_binding_digest(
                        encoded_key,
                        value,
                    ...
                    coordinate: Some(AuthoredHistoryBodyCoordinate {
                        descriptor_index: u32::try_from(index)
                            .expect("fixture descriptor index fits u32"),
                        row_index: 0,
                    }),
                },
            )
        ...
        inventory.closure_digest =
            super::authored_history_body_closure_digest(commit_id, &inventory)
                .expect("fixture history inventory should hash");
        inventory
    }

    fn stage_fixture_native_manifest(
        writes: &mut StorageWriteSet,
        commit_id: CommitId,
        mutations: &CommitStateMutationInventory,
        deltas: &[TrackedStateCommitDeltaRef<'_>],
    ) -> Result<(), LixError> {
        let publication = stage_fixture_current_state_root(writes, commit_id, mutations, deltas)?
            .ok_or_else(|| {
            LixError::new(
                LixError::CODE_INTERNAL_ERROR,
                "member-bearing fixture has no authenticated current-state root",
            )
        })?;
        let inventory = fixture_authored_history_body_inventory(
            commit_id,
            &publication.root,
            &publication.row_coordinates,
            deltas,
        )?;
        stage_fixture_manifest_with_current_state(
            writes,
            commit_id,
            mutations,
    ...
    fn stage_commit_deltas(
        ...
        let commit_id = deltas
            ...
            .unwrap_or_default();
        stage_fixture_native_manifest(writes, commit_id, staged.mutation_inventory(), deltas)?;
        Ok(staged.locators)
    }
    ...
    fn stage_addressable_commit_deltas(
        ...
        let fixture_deltas = deltas
            .iter()
            .enumerate()
            .map(|(index, delta)| {
                let mut delta = *delta;
                if staged.assigned_change_ids[index] != ChangeId::default() {
                    delta.delta.change_id = staged.assigned_change_ids[index];
                }
                delta
            })
            .collect::<Vec<_>>();
        stage_fixture_native_manifest(writes, commit_id, &mutations, &fixture_deltas)?;
        Ok(staged)
    }
    ...
    fn stage_addressable_commit_deltas_with_selected_source(
        ...
        let local_publication =
            stage_fixture_current_state_root(writes, commit_id, &mutations, deltas)?;
        let source_key = super::commit_state_manifest_key(selected_source_commit_id);
        let source_header = writes
            .staged_value(
                super::TRACKED_STATE_COMMIT_STATE_MANIFEST_SPACE,
                &source_key,
            )
            .ok_or_else(|| {
                LixError::new(
                    LixError::CODE_INTERNAL_ERROR,
                    "selected-source fixture has no staged source authority",
                )
            })?;
        let source = super::decode_stored_commit_state_manifest(source_header.as_ref())?
            .current_state_scoped_ranges
            .map(|root| *root)
            .ok_or_else(|| {
                LixError::new(
                    LixError::CODE_INTERNAL_ERROR,
                    "selected-source fixture has no staged current-state root",
                )
            })?;
        let root = match local_publication.as_ref() {
            Some(publication) => {
                crate::tracked_state::scoped_current_state::attest_scoped_range_root(
                    commit_id,
                    Some((selected_source_commit_id, &source)),
                    &mutations,
                    publication.root.tree.clone(),
                )?
            }
            None => crate::tracked_state::scoped_current_state::attest_scoped_range_root(
                commit_id,
                Some((selected_source_commit_id, &source)),
                &mutations,
                source.tree.clone(),
        ...
        let empty_coordinates = BTreeMap::new();
        let inventory = fixture_authored_history_body_inventory(
            commit_id,
            &root,
            local_publication
                .as_ref()
                .map_or(&empty_coordinates, |publication| {
                    &publication.row_coordinates
                }),
            deltas,
        )?;
    ...
    where
        I: ExactSizeIterator<Item = Result<TrackedStateCommitDeltaRef<'a>, LixError>> + Clone,
    {
        let fixture_deltas = deltas.clone().collect::<Result<Vec<_>, LixError>>()?;
        let staged =
            super::stage_ordered_addressable_commit_deltas(writes, deltas, order_certified, false)?;
        if let Some(staged) = &staged {
            let fixture_deltas = fixture_deltas
                .into_iter()
                .enumerate()
                .map(|(index, mut delta)| {
                    if let Some(change_id) = staged.change_id_at(index) {
                        delta.delta.change_id = change_id;
                    }
                    delta
                })
                .collect::<Vec<_>>();
            stage_fixture_native_manifest(
                writes,
                staged.commit_id,
    ...
    fn commit_delta_ref<'a>(
        ...
        let snapshot =
            if !fixture.deleted && matches!(snapshot, crate::json_store::JsonSlotRef::None) {
                crate::json_store::JsonSlotRef::Inline("null")
            } else {
                snapshot
            };
    ...
    async fn irregular_ordered_segment_addresses_match_the_manifest() {
        ...
        assert!(
            certificate
                ...
                .all(|&count| usize::from(count) == super::COMMIT_DELTA_SEGMENT_MAX_ROWS),
            "native history members no longer split on whole-row JSON payload bytes"
        );
        let assigned = staged.assigned_change_ids().collect::<Vec<_>>();
    ...
    async fn compact_replacement_direct_change_id_hydrates_through_authenticated_part() {
        ...
        let fixtures = ["compact-000", "compact-001"].map(|row_pk| CommitDeltaFixture {
        ...
        let deltas = fixtures
            ...
            .enumerate()
            .map(|(index, fixture)| {
                let mut delta = commit_delta_ref(
                    commit_id,
                    fixture,
                    crate::json_store::JsonSlotRef::Inline("{\"v\":1}"),
                    crate::json_store::JsonSlotRef::None,
                    None,
                );
                delta.delta.change_id = staged
                    .change_id_at(index)
                    .expect("replacement fixture row should be addressed");
                delta
            })
            .collect::<Vec<_>>();
        stage_fixture_native_manifest(&mut writes, commit_id, &inventory, &deltas)
    ...
    async fn commit_local_authority_sorts_public_ids_and_treats_missing_manifest_as_empty() {
        ...
        let mut fixtures = packed_commit_delta_fixtures()
            .into_iter()
            .filter(|fixture| !fixture.deleted)
            .take(3)
            .collect::<Vec<_>>();
    ...
    #[test]
    fn authored_body_inventory_rejects_duplicate_change_ids_before_publication() {
        ...
        let error = stage_commit_deltas(&mut writes, &deltas)
            .expect_err("duplicate authored change ids must not publish an inventory");
        assert!(error.to_string().contains("duplicate change"));
    }
    ...
    async fn global_inventory_hydrates_selected_payloads_from_canonical_authority() {
        ...
        assert_eq!(
            ...
            "selected live rows must persist only the canonical reference tag"
        );
        assert_eq!(
            scan_commit_delta_inventory(&read)
                .await
    ...
    async fn packed_commit_deltas_preserve_point_and_schema_replay() {
        ...
        assert_eq!(
            ...
            12,
            "generic history plus the fixture's authenticated current-state root has a fixed object budget"
        );
        storage
            .commit_write_set(writes, StorageWriteOptions::default())
            .await
    ...
    fn indexed_payload_point_decoder_skips_unrequested_records() {
        ...
        let payloads = [
            CommitDeltaPayloadRef {
                body_digest: history_body_digest(
                    crate::json_store::JsonSlotRef::Inline(snapshots[0]),
                    crate::json_store::JsonSlotRef::None,
                ),
            ...
            CommitDeltaPayloadRef {
                body_digest: history_body_digest(
                    crate::json_store::JsonSlotRef::Inline(snapshots[1]),
                    crate::json_store::JsonSlotRef::None,
                ),
                origin_key: None,
                base_coordinate: None,
            ...
            CommitDeltaPayloadRef {
                body_digest: history_body_digest(
                    crate::json_store::JsonSlotRef::Inline(snapshots[2]),
                    crate::json_store::JsonSlotRef::None,
                ),
        ...
        let corrupt_range = {
            ...
            assert_eq!(
                ...
                "every commit member must carry an authoritative body digest"
            );
            let range = index
                .payload_range(2)
    ...
    fn indexed_payload_kinds_round_trip_columnar_base_coordinates() {
        ...
        let payloads = [
            CommitDeltaPayloadRef {
                body_digest: history_body_digest(
                    crate::json_store::JsonSlotRef::Inline(r#"{"authored":true}"#),
                    crate::json_store::JsonSlotRef::None,
                ),
                origin_key: None,
    ...
    #[test]
    fn compressed_native_member_sidecar_roundtrips_and_rejects_corruption() {
        ...
        let entries = fixtures
            ...
            .collect::<Vec<_>>();
        let first_origin = format!("first:{}", "a".repeat(8 * 1024));
        let second_origin = format!("second:{}", "b".repeat(8 * 1024));
        let first_digest = history_body_digest(
            crate::json_store::JsonSlotRef::Inline(r#"{"value":"first"}"#),
            crate::json_store::JsonSlotRef::None,
        );
        let second_digest = history_body_digest(
            crate::json_store::JsonSlotRef::Inline(r#"{"value":"second"}"#),
            crate::json_store::JsonSlotRef::None,
        );
        let payloads = [
            CommitDeltaPayloadRef {
                body_digest: first_digest,
        ...
        assert_eq!(
            ...
            "repetitive native member metadata should use zstd"
        );

        let (_, decoded) =
            decode_commit_delta_with_payloads(&encoded, None).expect("sidecar should decode");
        assert_eq!(
            decoded.decode(0).unwrap().authored_payload().body_digest,
            first_digest
        );
        assert_eq!(
            decoded.decode(1).unwrap().authored_payload().body_digest,
            second_digest
        );
    ...
    fn packed_commit_members_reject_empty_authoritative_payload_ranges() {
        ...
        let payloads = [
            ...
            CommitDeltaPayloadRef {
                body_digest: history_body_digest(
                    crate::json_store::JsonSlotRef::Inline(r#"{"second":true}"#),
                    crate::json_store::JsonSlotRef::None,
                ),
                origin_key: None,
                base_coordinate: None,
    ...
    async fn indexed_payload_points_preserve_large_snapshots_null_rows_and_tombstones() {
        ...
        assert!(records[1].as_ref().is_some_and(
            |record| record.snapshot == crate::json_store::JsonSlot::Inline("null".into())
        ));
        assert!(
            records[2]
                .as_ref()
    ...
    fn indexed_payload_directory_rejects_old_truncated_and_invalid_offsets() {
        ...
        let encoded = encode_commit_delta_segment_with_raw_sidecar(
            ...
            &[CommitDeltaPayloadRef {
                body_digest: history_body_digest(
                    crate::json_store::JsonSlotRef::Inline(r#"{"ok":true}"#),
                    crate::json_store::JsonSlotRef::None,
                ),
                origin_key: None,
                base_coordinate: None,
        ...
        old[..COMMIT_DELTA_FORMAT_MAGIC.len()].copy_from_slice(b"LXCD16");
    ...
    async fn single_segment_commit_delta_stays_inline() {
        ...
        assert_eq!(
            writes.stats().staged_puts,
            4,
            "an inline history member plus its fixture current-state root remains bounded"
        );
        storage
            .commit_write_set(writes, StorageWriteOptions::default())
            .await
    ...
    async fn schema_scan_validates_unselected_packed_delta_entries() {
        ...
        let delta_manifest = CommitDeltaManifest {
            ...
            replacement_parts: None,
            columnar_parts: None,
            inline_segment: encode_commit_delta_segment_with_payloads(
                &entries,
                &[
                    CommitDeltaPayloadRef {
                        body_digest: history_body_digest(
        ...
        let alpha_delta = commit_delta_ref(
            commit_id,
            alpha,
            crate::json_store::JsonSlotRef::None,
            crate::json_store::JsonSlotRef::None,
        ...
        let mut beta_delta = commit_delta_ref(
            commit_id,
            beta,
            crate::json_store::JsonSlotRef::None,
            crate::json_store::JsonSlotRef::None,
        ...
        beta_delta.delta.commit_id = wrong_commit_id;
        stage_fixture_native_manifest(
            &mut writes,
            commit_id,
            &super::commit_state_inventory_from_delta_manifest(&delta_manifest),
    ...
    async fn packed_commit_delta_boundary_keeps_file_identity_and_sparse_schema_replay() {
        ...
        assert_eq!(inline_writes.stats().staged_puts, 8);
        storage
        ...
        assert_eq!(indexed_writes.stats().staged_puts, 13);
        storage
    ...
    fn commit_state_manifest_fixture() -> CommitStateManifest {
        ...
        let tree = crate::tracked_state::scoped_range::ScopedRangeRoot {
            ...
            root_digest: *blake3::hash(b"manifest fixture current-state root").as_bytes(),
            marker_count: 1,
            part_count: 1,
            row_count: 1,
            tree_height: 1,
        };
        let root = crate::tracked_state::scoped_current_state::attest_scoped_range_root(
            commit_id, None, &mutations, tree,
        )
        .expect("manifest fixture current-state root should attest");
        let bodies = fixture_single_row_body_inventory(
            commit_id,
            &root,
            vec![(
                encoded_key,
        ...
        let mut manifest =
            fixture_commit_state_manifest(commit_id, mutations, Some(root), Some(bodies));
    ...
    fn external_commit_state_manifest_fixture(
        ...
        let tree = crate::tracked_state::scoped_range::ScopedRangeRoot {
            root_id: *blake3::hash(commit_id.as_uuid().as_bytes()).as_bytes(),
            root_digest: *blake3::hash(label.as_bytes()).as_bytes(),
            marker_count: 1,
            part_count: u32::try_from(part_count).expect("fixture part count fits u32"),
            row_count: part_count as u64,
            tree_height: 1,
        };
        let root = crate::tracked_state::scoped_current_state::attest_scoped_range_root(
            commit_id, None, &mutations, tree,
        )
        .expect("external fixture current-state root should attest");
        let timestamp = LixTimestamp::from_unix_millis_utc_lossy(0);
        let members = mutations
            .parts
            .iter()
            .enumerate()
            .map(|(index, part)| {
                (
                    part.first_key.clone(),
                    TrackedStateIndexValue {
                        change_id: ChangeId::for_test_label(&format!(
                            "{label}-external-member-{index}"
                        )),
                        commit_id,
                        deleted: false,
                        created_at: timestamp,
            ...
            .collect();
        let bodies = fixture_single_row_body_inventory(commit_id, &root, members);
        fixture_commit_state_manifest(commit_id, mutations, Some(root), Some(bodies))
    }
    ...
    fn raw_manifest_constructor_rejects_member_bearing_lxcd17_authority() {
        ...
        assert!(
            error
                .message
                .contains("certified current-state publication")
        );
    }
    ...
    async fn authored_body_certificate_rejects_post_seal_segment_mutation() {
        ...
        assert!(
            error
                .message
                .contains("post-seal duplicate segment mutation")
        );
    }
    ...
    fn commit_state_manifest_codec_authenticates_scoped_range_root() {
        ...
        let inventory = manifest
            .authored_history_bodies
            .as_mut()
            .expect("member-bearing fixture has a body inventory");
        inventory.source_root_id = [7; 32];
        inventory.source_transition_digest = transition_digest;
        inventory.closure_digest =
            super::authored_history_body_closure_digest(manifest.commit_id, inventory)
                .expect("rebound fixture inventory should hash");
        ...
        manifest
            ...
            .unwrap()
            .transition_digest[0] ^= 1;
        let forged_transition = manifest
            .current_state_scoped_ranges
            .as_ref()
            .unwrap()
            .transition_digest;
        let inventory = manifest.authored_history_bodies.as_mut().unwrap();
        inventory.source_transition_digest = forged_transition;
        inventory.closure_digest =
            super::authored_history_body_closure_digest(manifest.commit_id, inventory)
                .expect("forged fixture inventory should hash");
