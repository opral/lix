//! Shared arena, host import, and actor implementation specialized to an
//! execution backend. Both adapters invoke this macro in an ordinary owned
//! module; only Store/resource types and guest calls differ between adapters.
macro_rules! shared_component_runtime {
    () => {
use std::collections::{BTreeSet, HashMap, VecDeque};
use std::sync::{Arc, Mutex};
use web_time::Instant;

use super::component_backend::*;
...
use lix::plugin::runtime::{
    CURRENT_PACKET_FORMAT, WasmByteOutputsHandle, WasmChangeCursorHandle, WasmChangeEffect,
    WasmChangePage, WasmColdFileUpdate, WasmColumnMergeCursorHandle,
    WasmColumnMergeResult as RuntimeColumnMergeResult, WasmColumnMergeResultPage,
    WasmColumnMergeTransition, WasmColumnMergeUpdate, WasmComponentActor, WasmCreateContext,
    WasmDocumentCheckpoint, WasmDocumentHandle, WasmEditCursorHandle, WasmEditPage,
    WasmFileTransition, WasmFileUpdate, WasmGuestBytes, WasmGuestColumnValue, WasmGuestRowPayload,
...
pub(super) const COMPONENT_MAX_CONCURRENT_EXECUTIONS_PER_COMPONENT: usize = 1;

pub(super) fn take_borrowed_resource<T: Send + 'static>(
...
pub(super) type SharedTransitionState = Arc<Mutex<TransitionState>>;
pub(super) type SharedResolutionState = Arc<Mutex<ResolutionState>>;
pub(super) type SharedRowChangeState = Arc<Mutex<RowChangeState>>;
pub(super) type SharedByteBudget = Arc<Mutex<u64>>;
...
pub(super) struct PendingFileEdit {
    offset: u64,
    delete_len: u64,
    insert: Bytes,
}

pub(super) struct PendingFileReplacement {
    expected_len: u64,
    bytes: Vec<u8>,
    complete: bool,
}

pub(super) struct RowChangeState {
    limits: WasmTransitionLimits,
    started: Instant,
    total_bytes: SharedByteBudget,
...
pub(super) enum RowChangeInputSource {
...
#[derive(Clone)]
pub(super) enum PendingChangePage {
    Typed {
        schema_key: String,
...
pub(super) fn decode_typed_change_page(
    schema_key: String,
    schema_fingerprint: [u8; 32],
    mutations: Vec<TypedMutation>,
...
pub(super) fn typed_input_mutations<'a>(
...
pub(super) fn append_replace_all_deletes(
...
pub(super) fn component_limit(message: impl Into<String>) -> LixError {
    LixError::new(LixError::CODE_INVALID_PARAM, message)
}

pub(super) fn validate_source_admission(
    length: u64,
    limits: WasmTransitionLimits,
) -> Result<(), LixError> {
...
pub(super) fn merge_row_input_profile(
...
pub(super) fn read_encoded_bytes(
    bytes: &[u8],
    offset: u64,
    length: u32,
...
#[cfg(test)]
pub(super) fn create_context_from_uuid(value: uuid::Uuid) -> WasmCreateContext {
...
#[cfg(test)]
pub(super) fn create_context_from_generated_row(
...
pub(super) fn ensure_source_page(
    length: u32,
    max_page_bytes: u32,
) -> Result<(), bindings::lix::plugin::host::HostError> {
...
impl bindings::lix::plugin::host::HostSnapshot for HostState {
...
impl bindings::lix::plugin::host::HostTransition for HostState {
...
impl bindings::lix::plugin::host::HostRowSource for HostState {
    fn next_page(
        &mut self,
...
impl bindings::lix::plugin::host::HostColumnMergeSource for HostState {
...
impl bindings::lix::plugin::host::HostColumnMergeSink for HostState {
...
pub(super) struct ComponentWorker {
    pub(super) store: HostStore,
    pub(super) file_projection: Option<FileProjectionGuest>,
    pub(super) column_merger: Option<ColumnMergerGuest>,
    pub(super) limits: WasmLimits,
    pub(super) documents: HashMap<u64, ComponentDocument>,
...
pub(super) struct ResolutionWorkerOutput {
    resolutions: Vec<ComponentResolution>,
    counters: WasmTransitionCounters,
}

pub(super) struct RowWorkerOutput {
    edits: Vec<PendingFileEdit>,
    counters: WasmTransitionCounters,
}

pub(super) struct HydrateWorkerOutput {
    replacement: Option<Bytes>,
    accepted_len: u64,
    counters: WasmTransitionCounters,
}

pub(super) struct FileWorkerOutput {
    pages: VecDeque<PendingChangePage>,
    replace_all_rows: bool,
    counters: WasmTransitionCounters,
}

pub(super) fn hydration_replacement_edit(
    replacement_len: u64,
    accepted_len: u64,
) -> WasmOutputSplice {
    WasmOutputSplice {
...
pub(super) struct ColumnMergeCursorState {
    transition: WasmTransitionHandle,
...
pub(super) struct ComponentEditCursorState {
    transition: WasmTransitionHandle,
    page: Option<WasmEditPage>,
}

pub(super) struct ComponentActor {
    pub(super) worker: ComponentWorker,
    pub(super) execution_permit: Arc<tokio::sync::Semaphore>,
    pub(super) initial_execution_permit: Option<tokio::sync::OwnedSemaphorePermit>,
    pub(super) _timeout_ticker: TimeoutTickerLease,
    pub(super) next_handle: u64,
    pub(super) cursors: HashMap<u64, CursorState>,
    pub(super) column_merge_cursors: HashMap<u64, ColumnMergeCursorState>,
    pub(super) edit_cursors: HashMap<u64, ComponentEditCursorState>,
    pub(super) outputs: HashMap<u64, OutputState>,
    pub(super) transitions: HashMap<u64, WasmTransitionCounters>,
    pub(super) transition_permits: HashMap<u64, tokio::sync::OwnedSemaphorePermit>,
    pub(super) prospective_documents: ProspectiveDocuments,
    pub(super) retired: bool,
    pub(super) next_document: u64,
}

#[derive(Default)]
pub(super) struct ProspectiveDocuments(HashMap<u64, u64>);
...
pub(super) fn is_guest_trap(error: &LixError) -> bool {
...
impl ComponentActor {
    ...
    fn ensure_active(&self) -> Result<(), LixError> {
        if self.retired || store_is_retired(&self.worker.store) {
...
impl WasmComponentActor for ComponentActor {
    ...
    async fn open_file(
        ...
        .await?;
        let output = self.worker.open_file(document, limits, input).await;
    ...
    async fn open_rows(
        ...
        .await?;
        let resolved = self.worker.hydrate_file(document, limits, input).await;
    ...
    async fn cold_file_changed(
        ...
        let output = self
            .worker
            .cold_file_changed(document, limits, update)
            .await;
        let output = match output {
            Ok(output) => output,
            Err(error) => {
    ...
    async fn merge_columns(
        ...
        .await?;
        let resolved = self.worker.merge_columns(limits, update).await;
        let resolved = match resolved {
            Ok(resolved) => resolved,
            Err(error) => {
                self.retire_after_trap(&error);
    ...
    fn is_retired(&self) -> bool {
        self.retired || store_is_retired(&self.worker.store)
    }
...
pub(super) fn required_plugin_path(
    path: Option<String>,
    transition: &str,
...
pub(super) fn component_error(message: impl Into<String>) -> LixError {
    LixError::new(LixError::CODE_INVALID_PLUGIN, message)
}

pub(super) fn component_transition_limits(
