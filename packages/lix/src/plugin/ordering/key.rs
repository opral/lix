...
impl OrderKey {
    ...
    pub fn evenly_between(
        ...
        // Reserve integer space at open ends instead of repeatedly halving the
        // remaining gap, which otherwise grows key storage quadratically.
        if previous.is_some() != next.is_some() {
            let raw = previous.or(next).unwrap().to_snapshot_string();
            let prefix = &raw[..raw.len().min(32)];
            let value = u128::from_str_radix(&format!("{prefix:0<32}"), 16).unwrap();
            let start = (count as u128)
            ...
            if let Some(start) = start {
                let keys = (0..count)
                    .map(|i| {
                        ...
                        Self::from_snapshot_string(&format!(
                            "{:032x}01",
...
mod tests {
    ...
    #[test]
    fn twenty_thousand_end_insertions_keep_bounded_keys() {
        for append in [true, false] {
            ...
            assert!(total <= 680_000, "stored {total} key characters");
        }
    }
    ...
    #[test]
    fn reversed_bounds_return_an_error() {
        let lower = OrderKey::from_snapshot_string("80").unwrap();
        let upper = OrderKey::from_snapshot_string("c0").unwrap();

        assert!(
            OrderKey::evenly_between(Some(&upper), Some(&lower), 1)
                .unwrap_err()
