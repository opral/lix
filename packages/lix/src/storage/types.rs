...
pub struct WriteOptions {
    ...
    ///
    /// Persistent adapters must honor this request or reject it explicitly;
    /// they must not acknowledge a buffered write as durable. Memory is
    /// intentionally ephemeral and does not provide persistence.
    ///
    /// Lix repository handles request durable acknowledgement by default.
    /// [`crate::Durability::Buffered`] opts ordinary writes into earlier
    /// acknowledgement; internal publication and sync requirements can still
    /// require durability. Raw storage clients choose this flag themselves.
    ///
    /// | adapter | flag clear | flag set |
    /// |---|---|---|
    /// | RocksDB | WAL reaches OS buffers; power loss may lose it | WAL synchronized before acknowledgement |
    /// | SlateDB | writes may remain queued in process | waits for WAL upload |
    /// | SQLite on OPFS | SQLite `synchronous=NORMAL` | SQLite `synchronous=FULL` |
    ///
    /// This is a backend persistence boundary, not replication completion or
    /// a promise against hardware that violates synchronization semantics.
    /// RocksDB's boundary is verified by observing synchronization syscalls;
    /// the test suite does not simulate block-device power failures.
