...
use crate::storage_adapter::{PointReadPlan, StorageAdapter, StorageKey, StorageWriteOptions};
...
#[tokio::test]
async fn partial_comment_author_join_survives_a_missing_change_record() {
    const AUTHOR: &str = "01950000-0000-7000-8000-000000000911";
    const CONVERSATION: &str = "01950000-0000-7000-8000-000000000912";
    const COMMENT: &str = "01950000-0000-7000-8000-000000000913";

    let authority = open_lix().await.unwrap();
    authority
        .ensure_account(AUTHOR, "Partial writer", "human")
        .await
        .unwrap();
    ...
    writer
        .execute(
            ...
            &[Value::Text(CONVERSATION.to_owned())],
        )
        .await
        .unwrap();
    writer
        .execute(
            ...
            &[
                Value::Text(COMMENT.to_owned()),
                Value::Text(CONVERSATION.to_owned()),
                Value::Text(r#"{"_type":"zettel_doc","blocks":[]}"#.to_owned()),
            ],
        )
        .await
        .unwrap();
    let source_change = authority
        .execute(
        ...
        .await
        .unwrap();
    let Value::Text(change_id) = source_change.rows()[0]
        .get::<Value>("lixcol_change_id")
        .unwrap()
    else {
        panic!("tracked source comment should have a change id");
    };
    let change_id = crate::changelog::ChangeId::parse_lix(&change_id, "comment change id").unwrap();
    ...
    let state = PartialReplicaState::new(
        format!("https://example.test/lix/{}", authority.lix_id()),
        AUTHOR.to_owned(),
        "00000000-0000-7000-8000-000000000914".to_owned(),
        descriptor,
    )
    .unwrap();
    let memory = Memory::new();
    ...
    let join_sql = "SELECT c.lixcol_author_id, c.lixcol_change_id, a.name, a.profile_uri FROM lix_comment AS c JOIN lix_account AS a ON a.id = c.lixcol_author_id WHERE c.id = $1";
    let params = [Value::Text(COMMENT.to_owned())];
    let mut fetches = Fetches::default();
    let joined = execute_hydrating(
        &session,
        &storage,
        &state,
        &authority,
    ...
    .unwrap();
    assert_eq!(
        joined.rows()[0].values(),
        &[
            Value::Text(AUTHOR.to_owned()),
            Value::Text(change_id.to_string()),
            Value::Text("Partial writer".to_owned()),
            Value::Null,
        ]
    );

    // Remove the matching retained changelog fact after the state row and
    // account are resident. The row's author must remain readable from state.
    let change_key = StorageKey(bytes::Bytes::copy_from_slice(
        change_id.as_uuid().as_bytes(),
    ));
    let mut writes = storage.new_write_set();
    writes.delete(crate::changelog::CHANGE_SPACE, change_key.clone());
    storage
        .commit_partial_replica_write_set(
            super::partial_replica_write_capability(),
            writes,
        ...
        .unwrap();
    let read = storage.begin_read(Default::default()).await.unwrap();
    let missing_change = PointReadPlan::new(
    ...
    .flatten();
    assert!(
        missing_change.is_none(),
        "matching local change record must be absent"
    );
    drop(read);
    let joined_without_change = session.execute(join_sql, &params).await.unwrap();
    assert_eq!(
        joined_without_change.rows()[0].values(),
        &[
            Value::Text(AUTHOR.to_owned()),
            Value::Text(change_id.to_string()),
            Value::Text("Partial writer".to_owned()),
    ...
    session
        .execute(
            "INSERT INTO lix_key_value(key, value, lixcol_untracked) VALUES ('partial-author-untracked', 'local', true)",
            &[],
        )
        .await
        .unwrap();
    let untracked = session
        .execute(
            "SELECT lixcol_author_id, lixcol_change_id, lixcol_commit_id FROM lix_key_value WHERE key = 'partial-author-untracked'",
            &[],
        )
        .await
        .unwrap();
    let [Value::Text(author_id), Value::Text(change_id), Value::Null] =
        ...
        panic!("partial untracked row should retain author and change IDs without a commit");
    };
