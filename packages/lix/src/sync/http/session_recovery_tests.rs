...
async fn concurrent_stale_clones_share_one_replacement() {
    ...
    let (a, b) = tokio::time::timeout(std::time::Duration::from_secs(2), async {
        futures_util::join!(
            transport.send(transport.request(Method::POST, "/sync/update", "update")),
            clone.send(clone.request(Method::POST, "/sync/update", "update"))
        )
    })
    .await
    .unwrap();
...
async fn slow_failed_handshake_backs_off_from_failure_not_start() {
    ...
    let task = tokio::spawn(async move {
        clone
            .send(clone.request(Method::POST, "/sync/update", "update"))
            .await
    });
    ...
    assert!(
        transport
            .send(transport.request(Method::POST, "/sync/update", "update"))
            .await
            .is_err()
    );
    assert_eq!(
