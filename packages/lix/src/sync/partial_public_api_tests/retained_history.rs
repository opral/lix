...
async fn unavailable_retained_history_does_not_block_background_file_updates() {
    tokio::time::timeout(std::time::Duration::from_secs(30), Box::pin(async {
        ...
        let before = authority.execute("SELECT commit_id FROM lix_create_checkpoint('Checkpoint', '{\"_type\":\"zettel_doc\",\"blocks\":[]}'::JSONB)", &[]).await.unwrap().rows()[0].get::<String>("commit_id").unwrap();
        authority.upsert_file_content("/gtm/edit.txt", b"historical change".to_vec()).await.unwrap();
        let after = authority.execute("SELECT commit_id FROM lix_create_checkpoint('Checkpoint', '{\"_type\":\"zettel_doc\",\"blocks\":[]}'::JSONB)", &[]).await.unwrap().rows()[0].get::<String>("commit_id").unwrap();
