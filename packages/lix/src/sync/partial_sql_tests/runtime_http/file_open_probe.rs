...
impl RawHttpClient for TimedClient {
    fn send(&self, request: RawHttpRequest) -> SyncTransportFuture<'_, RawHttpResponse> {
        Box::pin(async move {
            ...
            let request_interests = (operation == "read-fulfillment")
                .then(|| request.body.as_ref())
                .flatten()
                .and_then(|body| serde_json::from_slice::<serde_json::Value>(body).ok())
                .and_then(|body| body.get("interests").cloned());
...
/// A cold file query should bring its native graph, path, and content closure
/// in one request, regardless of the projection that SQL evaluates locally.
#[tokio::test]
async fn synthetic_markdown_path_versus_id_probe() {
