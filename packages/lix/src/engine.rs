    ...
    StorageImpl: Storage + Clone + Send + Sync + 'static,
{
    ...
    /// current `lix_branch_ref` rows in live state.
...
mod tests {
    ...
    async fn register_json_pointer_schema_in_scope(session: &SessionContext<Memory>, global: bool) {
        ...
        assert_eq!(
            session
                .execute(
                    "INSERT INTO lix_registered_schema (value, lixcol_global) VALUES (lix_json($1), $2)",
                    &[
    ...
    async fn tracked_entity_public_fast_path_falls_back_for_global_tracked_rows() {
        ...
        global_session
            .execute(
                "INSERT INTO json_pointer (path, value, lixcol_global) \
                 VALUES ('/global', lix_json('{\"source\":\"global\"}'), true)",
                &[],
            )
            .await
