...
/// The old benchmark fixture and raw-space inventory surface was tied to
/// deleted physical owners.  It is intentionally not compiled: benchmark
/// consumers must use the closed ForkTree acceptance facade once that wave is
/// landed, rather than reviving the deleted spaces through a support module.
#[cfg(any())]
mod obsolete_benchmark_support {
        ...
        StorageImpl: Storage,
    {
        ...
        let right =
            crate::changelog::CommitId::parse_lix(right_commit_id, "merge benchmark right")?;
        reader
        ...
        StorageImpl: Storage,
    {
        ...
        let nodes = match mode {
            ...
            CommitGraphBenchMode::ReachableNodes | CommitGraphBenchMode::LegacyReachableNodes => {
                reader
        ...
        R: StorageAdapterRead,
    {
        ...
        for entry in locator_entries {
            ...
            let change_id =
                crate::changelog::ChangeId::new(uuid::Uuid::from_bytes(change_id_bytes));
        ...
        R: StorageAdapterRead,
    {
        ...
        let chunk_entries =
            scan_layout_entries(read, crate::binary_cas::BINARY_CAS_CHUNK_SPACE).await;
        ...
        for entry in &manifest_entries {
            ...
            match crate::binary_cas::decode_binary_cas_manifest(value)? {
                ...
                crate::binary_cas::BinaryCasManifest::Delta {
                    ...
                    crate::binary_cas::StorageBinaryCasDeltaBaseLayout::SingleChunk {
                        chunk_hash,
                    } => {
                        retained_chunks.insert(crate::binary_cas::BlobId::from_bytes(chunk_hash));
                    }
        ...
        for entry in entries {
            ...
            match manifest {
                ...
                crate::binary_cas::BinaryCasManifest::Delta { base_blob_hash, .. } => {
                    ...
                    delta_bases
                        .push((owner, crate::binary_cas::BlobId::from_bytes(base_blob_hash)));
                }
            }
        }
        ...
        for entry in chunk_entries {
            ...
            let owner = match owners {
                ...
                Some(owners) if owners.len() == 1 => {
                    owners.first().expect("one chunk owner").clone()
                }
                Some(owners) => format!(
                    "shared_chunk:{}",
    ...
    mod tests {
        ...
        async fn checkpoint_commit_scan_baseline_matches_materialized_records_across_pages() {
            ...
            let materialized = scan_checkpoint_commits_for_bench(
                &adapter,
                CheckpointCommitScanBenchMode::Materialize,
            )
            .await
        ...
        async fn streamed_layout_accounting_matches_full_space_inventory() {
            ...
            let inventory =
                super::space_inventory(&read, crate::changelog::COMMIT_SPACE.name).await;
