...
async fn execute_fast_lix_file_id_path_writes_inner(
    ...
    // Every conflict class resolves its existing rows through the cached
    // filesystem path index. There is no second, uncached whole-repository
    // index behind this any more: when the indexed route declines, the caller
    // runs the original DataFusion plan rather than rebuilding the filesystem.
    ...
    // A plain INSERT onto a path that already has a visible file is a
    // constraint violation, not a fast write. Decline so the general plan
    // raises it with its own diagnostics.
    if conflict == FastLixFilePathWriteConflict::None
        && indexed.existing.iter().any(Option::is_some)
    {
        return Ok(None);
    }
    stage_indexed_file_path_writes(
        ctx,
        &active_branch_id,
        parsed_writes,
        indexed,
...
async fn stage_indexed_file_path_writes(
    ...
    // `None` reaches here only when no write collides with an existing path;
    // the colliding case is declined upstream so the general plan can raise the
    ...
    for (write, entry) in writes.into_iter().zip(indexed.existing) {
        ...
        if let Some(entry) = entry {
            ...
            match conflict {
                ...
                FastLixFilePathWriteConflict::None => unreachable!(
                    "a plain insert onto an existing path is declined before staging"
                ),
                FastLixFilePathWriteConflict::DoNothing
                | FastLixFilePathWriteConflict::IdDoNothing => {
                    unreachable!("do-nothing conflicts skip existing entries above")
                }
            }
    ...
    // `None` is the only class that must reject a colliding path, so it is the
    // only one staged as an insert. Every other class replaces, exactly as the
    // route this replaced did.
