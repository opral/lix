    ...
    S: StorageAdapterRead + Clone + Send + Sync + 'static,
{
    ...
    async fn plan_scan(
        ...
        let metadata_projection =
            HistoryMetadataProjection::from_scan(&schema, filters).with_native_entity_rows();
        Ok(PlannedScan {
...
fn native_text_value(
    ...
    match value {
        ...
        Some(BodyValue::Jsonb(value)) if column_type == EntityColumnType::Json => {
