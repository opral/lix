    ...
    S: StorageAdapterRead + Clone + Send + Sync + 'static,
{
    fn call(&self, args: &[Expr]) -> Result<Arc<dyn TableProvider>> {
        ...
        // The relation exposes today's columns, but an older endpoint can
        // predate a required column added with a default. Its historical value
        // is NULL, not the default generated by the later amendment.
        let schema = if let StateRelationKind::Schema { spec, .. } = &kind
