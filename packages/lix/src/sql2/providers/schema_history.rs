...
static ROW_HISTORY_SYSTEM_COLS: ColumnTable<SchemaHistoryRow> = ColumnTable {
    columns: &[
        ...
        (
            ...
            Col::Bool(|row| {
                Some(row.change.snapshot_content.is_none() && row.change.decoded_snapshot.is_none())
            }),
        ),
    ],
};
...
fn row_history_record_batch(
    ...
    for row in rows {
        if let Some(typed) = row.change.decoded_snapshot.as_deref() {
            typed
    ...
    if rows.iter().any(|row| row.change.decoded_snapshot.is_some()) {
    ...
    fn history_projection_rejects_stale_durable_typed_fingerprint() {
        ...
        let row = SchemaHistoryRow {
            change: MaterializedChange {
                ...
                metadata: None,
                decoded_snapshot: Some(Arc::new(WasmTypedRow {
