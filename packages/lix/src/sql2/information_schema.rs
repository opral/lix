...
mod tests {
    ...
    #[test]
    fn public_sql_type_preserves_logical_types() {
        use super::super::result_metadata::{json_field, row_ref_field};
        use datafusion::arrow::datatypes::TimeUnit;

        assert_eq!(public_sql_type(&json_field("value", true)), "JSONB");
        assert_eq!(public_sql_type(&row_ref_field("ref", false)), "ROW_REF");
        assert_eq!(public_sql_type(&Field::new("text", DataType::Utf8, false)), "TEXT");
        assert_eq!(public_sql_type(&Field::new("time", DataType::Timestamp(TimeUnit::Microsecond, Some("UTC".into())), false)), "TIMESTAMPTZ");
    }
