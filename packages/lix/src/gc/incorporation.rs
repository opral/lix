...
mod tests {
    ...
    async fn complete_proof_deferred_header_does_not_excuse_unmarked_parent() {
        ...
        let checkpoint = lix.execute("SELECT commit_id FROM lix_create_checkpoint('Checkpoint', '{\"_type\":\"zettel_doc\",\"blocks\":[]}'::JSONB, ARRAY(SELECT row_ref FROM lix_diff('lix_key_value')))", &[]).await.unwrap().rows()[0].get::<String>("commit_id").unwrap();
    ...
    async fn checkpoint_proof_survives_payload_retirement_and_second_gc() {
        ...
        for (index, value) in ["new", "latest"].into_iter().enumerate() {
            ...
            .await
            .unwrap();
            lix.execute("SELECT commit_id FROM lix_create_checkpoint('Checkpoint', '{\"_type\":\"zettel_doc\",\"blocks\":[]}'::JSONB, ARRAY(SELECT row_ref FROM lix_diff('lix_key_value') UNION ALL SELECT row_ref FROM lix_diff('lix_file')))", &[]).await.unwrap();
        }
        for pass in 0..2 {
            let read = adapter.begin_read(Default::default()).await.unwrap();
            let mut writes = adapter.new_write_set();
