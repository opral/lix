    ...
    row.filter(|row| !row.deleted())
        .map(|row| {
            let snapshot = if typed {
                let snapshot = row.decoded_snapshot().cloned().ok_or_else(|| {
                    LixError::new(
                        LixError::CODE_INVALID_PLUGIN,
                ...
                let snapshot = match (row.decoded_snapshot(), row.snapshot_content()) {
                    (Some(snapshot), _) => snapshot.to_json_shared()?,
                    (None, Some(snapshot)) => snapshot.clone(),
                    (None, None) => {
                        return Err(LixError::new(
                            LixError::CODE_INTERNAL_ERROR,
                            "live engine merge row has no payload",
                        ));
                    }
                };
...
fn push_transaction_row_from_tracked_row_ref(
    ...
    if let Some(decoded_snapshot) = row.decoded_snapshot().cloned() {
        rows.push_typed_parts(
    ...
    S: crate::storage_adapter::StorageAdapterRead,
{
    ...
    for (index, resolved) in resolved_rows.iter().enumerate() {
        ...
        let change = match (resolved.snapshot_json(), resolved.decoded_snapshot()) {
            (Some(snapshot), None) => {
                let target_snapshot = target
                    .map(
                        |row| match (row.decoded_snapshot(), row.snapshot_content()) {
                            (Some(snapshot), _) => snapshot.to_json_shared(),
                            (None, Some(snapshot)) => Ok(snapshot.clone()),
                            (None, None) => Err(LixError::new(
                                LixError::CODE_INTERNAL_ERROR,
                                "live engine target row has no payload",
                            )),
                        },
                    )
                    .transpose()?;
            ...
            (None, Some(snapshot)) => {
                let target_snapshot = target
                    .map(|row| {
                        row.decoded_snapshot().map(Arc::as_ref).ok_or_else(|| {
                            LixError::new(
                                LixError::CODE_INTERNAL_ERROR,
