    ...
    R: StorageAdapterRead,
{
    ...
    let requested_keys = request
        .rows
        .iter()
        .map(|requested| crate::forktree::StateKey {
        ...
        .collect::<Vec<_>>();
    let values = view
        .current_entity_state_exact(&requested_keys, request.include_tombstones)
        .await?;
    if values.len() != request.rows.len() {
        return Err(LixError::new(
            LixError::CODE_INTERNAL_ERROR,
            "ForkTree exact entity pack read returned the wrong slot count",
        ));
    }
    for (requested, value) in request.rows.iter().zip(values) {
        ...
        let decoded_key = decode_state_key(&encoded_key)?;
