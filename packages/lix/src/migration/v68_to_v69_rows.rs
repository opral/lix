...
impl ConversionPlan {
    pub(crate) fn compile(old: &Schema, target: &Schema) -> Result<Self, ConversionError> {
        ...
        if old.columns.len() != target.columns.len() {
            return Err(ConversionError::new(format!(
                "schema '{}' changed its column set during v68 -> v69 migration",
                old.key
            )));
        }
...
mod tests {
    use super::*;
    use crate::plugin::wire::typed::decode_native_row_payload;
    use serde_json::json;
    ...
    fn converts_known_text_jsonb_and_builds_verified_durable_payload() {
        ...
        let decoded = decode_native_row_payload(&converted.durable_payload)
            .expect("durable payload should decode");
        assert_eq!(
            decoded.schema_fingerprint,
            *target.wire_fingerprint().unwrap().as_bytes()
        );
        assert_eq!(decoded.row_pk, converted.primary_key);
