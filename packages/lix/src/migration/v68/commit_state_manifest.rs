...
use crate::tracked_state::{
    ...
    CommitStateReplayDebt, LAYOUT_BOUNDED_DIRECT, LAYOUT_BOUNDED_INDIRECT,
    LAYOUT_COMPACT_REPLACEMENT, LAYOUT_DIRECT_ROWS_ONLY, MutationDirectoryRoot, ScopedRangeRoot,
    ...
    TrackedStateCommitRootParent, TrackedStateRootId,
};
...
impl From<TrackedStateCommitRootV10> for TrackedStateCommitRoot {
    fn from(root: TrackedStateCommitRootV10) -> Self {
        Self {
...
async fn decode_manifest(
    ...
    let entries = match catalog.directory_root.as_ref() {
        Some(root) => load_all(store, root).await?,
        None => Vec::new(),
    };
...
async fn authenticate_bounded_segments(
    store: &(impl StorageAdapterRead + ?Sized),
    commit_id: CommitId,
    entries: Vec<DirectoryEntry>,
) -> Result<Vec<UpgradedDirectoryEntry>, LixError> {
    let mut keys = Vec::new();
    let mut bounded_positions = Vec::new();
    for (index, entry) in entries.iter().enumerate() {
        if let DirectoryEntry::Bounded {
            replacement_part, ..
        } = entry
        {
            ...
            key.extend_from_slice(
                &u32::try_from(index)
                    .map_err(|_| authority_error("segment index overflows u32"))?
                    .to_be_bytes(),
            );
            if let Some(replacement) = replacement_part {
                key.extend_from_slice(&replacement.content_digest);
            }
            keys.push(StorageKey(Bytes::from(key)));
            bounded_positions.push(index);
        }
    }
    let mut digests = vec![None; entries.len()];
    if !keys.is_empty() {
        let requests = [StorageGetManyRequest {
            space: TRACKED_STATE_COMMIT_DELTA_SEGMENT_SPACE,
            keys: &keys,
            opts: StorageGetOptions::default(),
        }];
        let values = exact_get_many(store, &requests).await?.values;
        for (position, value) in bounded_positions.into_iter().zip(values) {
            let Some(StorageProjectedValue::FullValue(bytes)) = value else {
                return Err(authority_error("directory references a missing segment"));
            };
            digests[position] = Some(*blake3::hash(&bytes).as_bytes());
        }
    }
    ...
    entries
        ...
        .map(|(index, entry)| match entry {
            DirectoryEntry::Bounded {
                ...
                replacement_part,
                direct_row_count,
            } => Ok(UpgradedDirectoryEntry::Bounded {
                part: MutationPartWire {
                    first_key,
                    last_key,
                    content_digest: digests[index]
                        .ok_or_else(|| authority_error("bounded segment was not authenticated"))?,
                    replacement_part: replacement_part.map(|part| ReplacementPartWire {
                        content_digest: part.content_digest,
                        owner_commit_id: part.owner_commit_id,
                        first_address: part.first_address,
            ...
            DirectoryEntry::CompactReplacement {
                content_digest,
                direct_row_count,
            } => Ok(UpgradedDirectoryEntry::CompactReplacement {
                content_digest,
                direct_row_count,
            }),
            DirectoryEntry::DirectAddress { direct_row_count } => {
                Ok(UpgradedDirectoryEntry::DirectAddress { direct_row_count })
            }
        })
        .collect()
}
...
fn assemble_manifest(
    ...
    entries: Vec<UpgradedDirectoryEntry>,
) -> Result<CommitStateManifest, LixError> {
    ...
    match catalog.directory_root.as_ref().map(|root| root.layout) {
        Some(LAYOUT_BOUNDED_INDIRECT | LAYOUT_BOUNDED_DIRECT) => {
            ...
            for entry in entries {
                let UpgradedDirectoryEntry::Bounded {
                    part,
                    direct_row_count,
                } = entry
                else {
        ...
        Some(LAYOUT_COMPACT_REPLACEMENT) => {
            for entry in entries {
                let UpgradedDirectoryEntry::CompactReplacement {
                    content_digest,
                    direct_row_count,
                } = entry
                else {
        ...
        Some(LAYOUT_DIRECT_ROWS_ONLY) => {
            for entry in entries {
                let UpgradedDirectoryEntry::DirectAddress { direct_row_count } = entry else {
                    return Err(authority_error(
