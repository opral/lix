...
pub(in crate::migration) async fn preflight_standalone_changelog(
    ...
    max_rows: usize,
    max_bytes: usize,
) -> Result<(Vec<V68StandaloneChange>, usize), LixError> {
...
async fn materialize_slot(
    ...
    let json_ref = match slot {
        LegacyJsonValue::None => return Ok(None),
        LegacyJsonValue::Inline(json) => return Ok(Some(json.into_string())),
        LegacyJsonValue::Ref(json_ref) => json_ref,
    };
...
mod tests {
    use super::*;
    use crate::json_store::{JsonWritePlacementRef, LegacyJsonValueRef, NormalizedJsonRef};
    ...
    fn frozen_change_bytes(
        row_label: &str,
        snapshot: LegacyJsonValueRef<'_>,
        metadata: LegacyJsonValueRef<'_>,
    ) -> Vec<u8> {
    ...
    async fn scans_every_page_and_materializes_inline_and_referenced_json() {
        ...
        for index in 0..=crate::storage_adapter::MAX_SCAN_PAGE_ROWS {
            ...
            let (snapshot, metadata) = if index == 0 {
                (
                    LegacyJsonValueRef::Ref(&refs[0]),
                    LegacyJsonValueRef::Inline(inline),
                )
            } else {
                (LegacyJsonValueRef::None, LegacyJsonValueRef::None)
            };
            writes.put(
                CHANGE_SPACE,
    ...
    async fn reports_malformed_change_keys_and_missing_json_refs() {
        ...
        missing_writes.put(
            ...
            frozen_change_bytes(
                "missing",
                LegacyJsonValueRef::Ref(&missing_ref),
                LegacyJsonValueRef::None,
            ),
        );
