...
/// v69 replaces the legacy JSON snapshot slot with native snapshot bytes and
/// therefore uses a deliberately incompatible decoder.
#[derive(musli::Decode)]
#[musli(packed)]
struct ChangeRecordView<'a> {
    ...
    #[musli(with = crate::json_store::json_slot_storage)]
    metadata: LegacyJsonValue,
    created_at: LixTimestamp,
    #[musli(with = crate::storage_codec::option)]
    origin_key: Option<String>,
}

/// Fully owned logical v68 changelog record.
///
/// This type deliberately lives behind the migration boundary. The current
/// `ChangeRecord` cannot represent an outer JSON snapshot after the v69 hard
/// cut, so decoding legacy bytes directly into that type would reintroduce the
/// dual-format state the migration is meant to eliminate.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::migration) struct V68ChangeRecord {
...
mod tests {
    use super::*;
    use crate::json_store::{JsonRef, LegacyJsonValueRef};
    ...
    struct FrozenV68ChangeRecordRef<'a> {
        ...
        snapshot: LegacyJsonValueRef<'a>,
        #[musli(with = crate::json_store::json_slot_storage_ref)]
        metadata: LegacyJsonValueRef<'a>,
        created_at: LixTimestamp,
        #[musli(with = crate::storage_codec::option)]
    ...
    fn frozen_v68_bytes() -> (Vec<u8>, RowPk, LixTimestamp) {
        ...
        let record = FrozenV68ChangeRecordRef {
            ...
            snapshot: LegacyJsonValueRef::Ref(&snapshot_ref),
            metadata: LegacyJsonValueRef::Inline(r#"{"source":"v68"}"#),
            created_at,
    ...
    fn decodes_frozen_v68_change_record_bytes() {
        ...
        assert_eq!(
            decoded.snapshot,
            LegacyJsonValue::Ref(JsonRef::for_content(b"v68 snapshot"))
        );
        assert_eq!(
            decoded.metadata,
