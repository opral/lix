...
fn decode_json_slot(
    ...
    match take_exact(encoded, cursor, 1)?[0] {
        ...
        0 => Ok(JsonSlot::None),
        1 => Ok(JsonSlot::Ref(crate::json_store::JsonRef::from_hash_bytes(
        ...
        2 => {
            ...
            Ok(JsonSlot::Inline(json.to_owned().into()))
        }
...
mod tests {
    ...
    fn json_slot(out: &mut Vec<u8>, slot: crate::json_store::JsonSlotRef<'_>) {
        match slot {
            crate::json_store::JsonSlotRef::None => out.push(0),
    ...
    fn frozen_raw_fixture(snapshot: &str) -> Vec<u8> {
        ...
        json_slot(&mut bytes, crate::json_store::JsonSlotRef::Inline(snapshot));
        json_slot(&mut bytes, crate::json_store::JsonSlotRef::None);
        bytes.extend_from_slice(&0u16.to_be_bytes());
        bytes.extend_from_slice(&4u16.to_be_bytes());
        bytes.extend_from_slice(b"beta");
        let reference = crate::json_store::JsonRef::for_content(b"v68 snapshot");
        json_slot(&mut bytes, crate::json_store::JsonSlotRef::Ref(&reference));
        json_slot(
            &mut bytes,
            crate::json_store::JsonSlotRef::Inline(r#"{"source":"v68"}"#),
        );
        bytes
    }
    ...
    fn decodes_frozen_raw_lxrpi003_into_owned_rows() {
        ...
        assert_eq!(rows[0].snapshot, JsonSlot::from_json(r#"{"name":"alpha"}"#));
