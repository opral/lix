...
mod tests {
    ...
    async fn exercise_repair(corrupt: bool) {
        ...
        session
            ...
            .await
            .unwrap();
        let checkpoint = session.execute("SELECT commit_id FROM lix_create_checkpoint('Checkpoint', '{\"_type\":\"zettel_doc\",\"blocks\":[]}'::JSONB, ARRAY(SELECT row_ref FROM lix_diff('lix_key_value')))", &[]).await.unwrap().rows()[0].get::<String>("commit_id").unwrap();
        drop(session);
        drop(engine);
