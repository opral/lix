...
{
    pub(super) fn materialize_schema_defaults<'a>(
        &'a mut self,
        schemas: &'a [(String, JsonValue)],
    ) -> futures_util::future::BoxFuture<'a, Result<(), LixError>> {
        // Construct the storage-heavy future only when this optional hook is
        // invoked. Boxing at an async call site still leaves its construction
        // temporary in every ordinary staging poll frame.
        Box::pin(async move {
            ...
            if !updates.is_empty() {
                ...
                Box::pin(self.stage_write_without_schema_default_backfill(
