    ...
    R: StorageAdapterRead + Sync,
{
    ...
    for write in &prepared.file_content_writes {
        let manifest = if let Some(receipt) = write.prepared_cas_receipt() {
            ...
            if let Some(edit) = write.edit_blob_splice() {
                if write.untracked {
                    return Err(writer_error(
                        "verified variable blob splice cannot target an untracked file owner",
                    ));
                }
                ...
                let state_key = StateKey {
                    ...
                    entity_pk: EntityPk::uuid_from_canonical(file_id).map_err(|error| {
                        writer_error(format!(
                            "verified variable blob splice file identity is not a canonical UUID: {error}"
                        ))
                    })?,
                };
                publication
                    .stage_verified_inline_blob_edit(view, &state_key, payload, edit)
                    .await
                    .map_err(LixError::from)?
            } else if let Some(splice) = write.same_length_blob_splice() {
                if write.untracked {
                    return Err(writer_error(
