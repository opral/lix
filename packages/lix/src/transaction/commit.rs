    ...
    R: StorageAdapterRead + Clone,
{
    ...
    let state_keys = tracked_rows
        ...
        .collect::<Vec<_>>();
    let previous_state_rows = view.state_points(&state_keys, true).await?;
    if previous_state_rows.len() != tracked_rows.len() {
        return Err(writer_error(
            "batched state-point lookup returned the wrong slot count",
        ));
    }
