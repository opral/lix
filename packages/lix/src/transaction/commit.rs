...
use crate::forktree::{
    ...
    CommitObjectV1, ObjectId, OrderedBranchHistoryTransition, PreparedPublication,
    RepositoryRootV1, SelectorExpectation, SnapshotRole, SnapshotSelectorId, StateCellRef,
    StateKey, StateKeyRef, StateMutationAudit, StateSource, StateTreeMutation, StateValueRef,
    UntrackedValueRef, encode_state_key, encode_state_value, load_change_catalog_entries_on_view,
    load_commit, load_commit_summary, open_coherent_view_on_read,
    ...
    R: StorageAdapterRead + Clone,
{
    ...
    let mut authenticated_selected_members = BTreeSet::new();
        ...
        for batch in &draft.selected_change_batches {
            for selected in batch.iter() {
                ...
                let (member, source_commit, source_change) =
                    ...
                    .await?;
                let Some((source_commit_object_id, source_ordinal)) = member.source() else {
                    return Err(writer_error(
                        "selected history member lost its authenticated source edge",
                    ));
                };
                authenticated_selected_members.insert((
