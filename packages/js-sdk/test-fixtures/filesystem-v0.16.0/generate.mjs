...
if (!archivePath) throw new Error('Usage: node generate.mjs <repository-path> <plugin_markdown.lixplugin>');
const lix = await openLix({storage: new FilesystemStorage({path: process.argv[2] ?? './repository'})});
try {
 const markdown = new Uint8Array(await readFile(archivePath));
 await lix.execute("INSERT INTO lix_file (path, content) VALUES ('/.lix/plugins/plugin_markdown.lixplugin', $1)", [markdown]);
