...
import {
	createHosted,
	convertJsStorageReplicaToPartial,
	retryJsStorageReplicaMigrationCleanup,
	deleteHosted,
	openJsStorage,
...
export async function convertReplicaBinding(storage:LixStorageConfig,server:SyncServerBindingOptions,branchId?:string):Promise<void> {
 if(storage.kind!=="jsStorage")throw new TypeError("Browser conversion requires persistent JavaScript storage");
 await initializeWasm();
 ...
 const provider=await module.createLixStorageProvider(storage.options);
 return withConversionProvider(provider,()=>convertJsStorageReplicaToPartial(provider,server,branchId));
}
...
export async function retryReplicaMigrationCleanupBinding(storage:LixStorageConfig,server:SyncServerBindingOptions):Promise<number> {
 if(storage.kind!=="jsStorage")throw new TypeError("Browser migration cleanup requires persistent JavaScript storage");
 await initializeWasm();
 const module=await import(/* @vite-ignore */storage.moduleUrl) as LixStorageProviderModule;
 if(typeof module.createLixStorageProvider!=="function")throw new TypeError("Invalid storage provider module");
 const provider=await module.createLixStorageProvider(storage.options);
 return withConversionProvider(provider,()=>retryJsStorageReplicaMigrationCleanup(provider,server));
}
