...
function toPluginEntityChange(change) {
	const normalized = {
		...
		schemaVersion: String(
			change?.schema_version ?? change?.schemaVersion ?? "",
		),
	};
...
function resolveComponentApi(exportsObject) {
	...
	if (exportsObject && typeof exportsObject[COMPONENT_API_KEY] === "object") {
...
async function prepareTranspiledComponent(wasmBytes) {
	...
	if (!pending) {
		pending = (async () => {
			...
			const [jsFileName, jsSourceBytes] = readTranspiledFile(
				transpiled.files,
				".js",
			);
			...
			if (typeof jsModule.instantiate !== "function") {
				throw new Error(
					`transpiled module '${jsFileName}' is missing instantiate()`,
				);
			}
...
export function createNodeWasmRuntime() {
	return {
		async initComponent(bytes) {
			...
			const exportsObject = await compiled.instantiate(async (coreFileName) => {
				const coreModulePromise = compiled.coreModulePromises.get(coreFileName);
				if (!coreModulePromise) {
					throw new Error(
						`transpiled component requested unknown core module '${coreFileName}'`,
					);
				}
