...
export type MemoryBackendContractOptions = {
	name: string;
	loadSdk: () => Promise<ContractSdk>;
	operationTimeoutMs?: number;
};

export function registerMemoryBackendContract({
	name,
	loadSdk,
	operationTimeoutMs = 5_000,
}: MemoryBackendContractOptions): void {
	const wait = <T>(promise: Promise<T>, operation: string): Promise<T> =>
		withTimeout(promise, operation, operationTimeoutMs);

	describe(`${name} memory-backend public contract`, () => {
