...
fn w5_r7_publish(owner: OwnerId, epoch: u64, progress: u64, selector: u64) {
    ...
    read.upload();
    read.object();
    let first = PreparedPublication::new(SELECTOR_SPACE);
    let second = PreparedPublication::new(SELECTOR_SPACE);
    first.into_storage_plan(read);
    second.into_storage_plan(read);
