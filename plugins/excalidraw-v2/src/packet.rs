...
mod tests {
    ...
    fn change_stream_pages_and_roundtrips_complete_changes() {
        let expected = EntityChange {
            schema_key: "excalidraw_element".to_owned(),
            entity_pk: vec!["key".to_owned()],
            snapshot: Some(
                br#"{"id":"id","order_key":"80","element_json":"{\"id\":\"id\",\"type\":\"line\"}"}"#
                    .to_vec(),
            ),
    ...
    fn oversized_snapshot_uses_one_page_local_attachment() {
        let expected = EntityChange {
            schema_key: "excalidraw_file".to_owned(),
    ...
    fn conflict_scanner_marks_an_absent_b_as_delete() {
        let mut record = Vec::new();
        encode_key(&mut record, "excalidraw_element", &["deleted".to_owned()]).unwrap();
