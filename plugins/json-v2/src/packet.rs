...
impl ChangeStream {
    ...
    pub fn next_page(
        ...
        loop {
            ...
            let inline_record = encode_merge_group(&change, None)?;
            let inline_framed_len = 4usize
                .checked_add(inline_record.len())
                .ok_or_else(|| "change record length overflow".to_owned())?;
            let (record, attach_snapshot) = if inline_framed_len <= limit {
                if payload.len() + inline_framed_len > limit {
                    *pending = Some(change);
                    break;
                }
                (inline_record, false)
                ...
                let record = encode_merge_group(&change, Some(attachment_index))?;
                ...
                return Err(format!(
                    "change record requires {inline_framed_len} bytes, page cap is {limit}"
                ));
            };
            put_u32(
                &mut payload,
...
mod tests {
    ...
    fn oversized_snapshot_uses_one_page_local_attachment() {
        ...
        let mut stream = ChangeStream::ready(vec![expected.clone()]);
        let mut pending = None;
        let page = stream.next_page(&mut pending, 256).unwrap().unwrap();
