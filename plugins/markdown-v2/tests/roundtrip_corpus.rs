...
//!
//! Every accepted fixture must reopen from projected entities with the exact
//! committed bytes. Semantic GFM equivalence and stability are also checked.

use chardetng::{EncodingDetector, Iso2022JpDetection, Utf8Detection};
use encoding_rs::Encoding;
use plugin_markdown_incremental_v2::{EntityState, File, MarkdownPlugin};
...
#[test]
fn cold_projection_reopens_every_local_fixture_byte_exact() {
    for fixture in fixtures() {
        let output = MarkdownPlugin::render(project(&fixture.input))
            .unwrap_or_else(|error| panic!("{} failed to render: {error:?}", fixture.name));
        assert_eq!(
            output, fixture.input,
            "{} changed accepted bytes on cold reopen",
            fixture.name
        );
    }
}
