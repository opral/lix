...
fn inline_atom_id(state: &[plugin_markdown_incremental_v2::EntityState], kind: &str) -> String {
...
fn top_level_inline_atom_ids(
    state: &[plugin_markdown_incremental_v2::EntityState],
    leaf_kind: &str,
    atom_kind: &str,
) -> Vec<String> {
...
fn upserts_for_kind<'a>(
    delta: &'a [plugin_markdown_incremental_v2::DetectedChange],
    kind: &str,
) -> Vec<&'a plugin_markdown_incremental_v2::DetectedChange> {
    delta
        .iter()
        .filter(|change| {
...
fn exact_cross_parent_subtree_move_preserves_every_descendant_id() {
    ...
    assert_eq!(upserts_for_kind(&delta, "list").len(), 1, "{delta:#?}");
    let document = upserts_for_kind(&delta, "document");
    assert_eq!(document.len(), 1, "{delta:#?}");
    assert_eq!(
        document[0].metadata.as_deref(),
...
#[test]
fn extra_blank_lines_are_retained_as_a_document_lexical_fallback() {
    let gaps = changes(state_from_source("A\n\nB\n"), "A\n\n\n\nB\n");
    assert_eq!(gaps.len(), 1, "{gaps:#?}");
    assert_eq!(change_snapshot(&gaps[0])["kind"], "document");
    assert_eq!(gaps[0].metadata.as_deref(), Some(r#"{"impact":"format"}"#));
}

#[test]
fn accepted_noncanonical_spelling_is_retained_on_the_document() {
    let cases = [
        ("# Heading\n", "# Heading #\n"),
        (
            "| A | B |\n| --- | ---: |\n| x | y |\n",
    ...
    for (before, after) in cases {
        let delta = changes(state_from_source(before), after);
        assert_eq!(delta.len(), 1, "{delta:#?}");
        assert_eq!(change_snapshot(&delta[0])["kind"], "document");
        assert_eq!(delta[0].metadata.as_deref(), Some(r#"{"impact":"format"}"#));
    }
}
