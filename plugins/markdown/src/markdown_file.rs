...
pub(crate) fn render_tree(root: &NodeTree) -> Result<Vec<u8>, PluginError> {
    if root.node.kind != NodeKind::Document || root.node.id != ROOT_ENTITY_PK {
        return Err(PluginError::InvalidInput(
            "Markdown state must contain a document node with id 'root'".to_string(),
        ));
    }
