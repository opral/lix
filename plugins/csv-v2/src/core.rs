...
impl EntityImportBuilder {
    ...
    pub(crate) fn finish(mut self) -> Result<(Document, ByteEdit), String> {
        ...
        let rendered_len = self.rows.iter().try_fold(0usize, |total, row| {
            let mut row_len = self.dialect.terminator.bytes().len();
            ...
            for index in 0..usize::from(row.cell_count) {
                ...
                row_len = row_len
                    .checked_add(rendered_cell_len(cell, self.dialect)?)
                    .ok_or_else(|| "CSV rendered length overflowed".to_owned())?;
            }
            total
                .checked_add(row_len)
        ...
        for (index, row) in self.rows.iter().copied().enumerate() {
            ...
            for cell_index in 0..usize::from(row.cell_count) {
                ...
                let cell = read_import_cell(&self.cell_bytes, &mut cursor)?;
                let quoted = render_import_cell(&mut blob, cell, self.dialect)?;
            ...
            blob.extend_from_slice(self.dialect.terminator.bytes());
            ...
            chunk_rows.push(CompactRow {
                ...
                ending: match self.dialect.terminator {
                    Terminator::Lf => 1,
                    Terminator::CrLf => 2,
                    Terminator::Cr => 3,
                },
...
fn rendered_cell_len(cell: &[u8], dialect: Dialect) -> Result<usize, String> {
    if !imported_cell_needs_quotes(cell, dialect)? {
        return Ok(cell.len());
    }
...
fn render_import_cell(output: &mut Vec<u8>, cell: &[u8], dialect: Dialect) -> Result<bool, String> {
    let quoted = imported_cell_needs_quotes(cell, dialect)?;
    if !quoted {
...
impl Document {
    ...
    pub fn file_changed(
        ...
        let after = self.0.blob.splice(splices)?;
    ...
    fn row_snapshot(&self, location: RowLocation) -> Result<Vec<u8>, String> {
        ...
        row_snapshot_bytes(&self.0.blob, chunk, row, &id, &order, self.0.dialect.quote)
    }
    ...
    fn update_or_reorder_sparse_row(
        ...
        let insert = render_row(&semantic.cells, self.0.dialect, source_row.ending())?;
    ...
    fn replace_sparse_row(
        ...
        let (chunk, row) = self.0.index.row(location);
        let start = chunk.byte_start + row.relative_start;
        let insert = render_row(&semantic.cells, self.0.dialect, row.ending())?;
        let splice = InputSplice {
            offset: u64::from(start),
            delete_len: u64::from(row.byte_len),
            insert: &insert,
    ...
    fn insert_sparse_row(
        ...
        let mut insert = render_row(
            &semantic.cells,
            self.0.dialect,
            Some(self.0.dialect.terminator),
        )?;
...
fn match_rows(
    ...
    let new_cells = new_rows
        .iter()
        .map(|row| draft_cells(new_blob, row, document.0.dialect))
        .collect::<Result<Vec<_>, _>>()?;
...
fn changed_entities(
    ...
    for (&slot, draft) in &new_slots {
        let changed = if let Some(&old_location) = old_by_slot.get(&slot) {
            ...
            old_cells != new_cells || old_row.order_rank != draft.order_rank.expect("assigned rank")
        } else {
            true
        };
        if changed {
...
fn row_snapshot_bytes(
    ...
    for (index, field) in chunk.data.fields[first..end].iter().copied().enumerate() {
        ...
        let value = decoded_field(&row_bytes, 0, field, quote)?;
...
pub fn parse_row_snapshot(bytes: &[u8]) -> Result<RowSnapshot, String> {
    ...
    {
        return Err("CSV row snapshot must contain only id, order_key, and cells".to_owned());
    }
    let id = object
        .get("id")
        .and_then(Value::as_str)
...
pub fn render_row(
    ...
    for (index, cell) in cells.iter().enumerate() {
        ...
        let needs_quote = imported_cell_needs_quotes(cell.as_bytes(), dialect)?;
        if needs_quote {
            let quote = dialect.quote.expect("quoting was required");
            output.push(quote);
            for &byte in cell.as_bytes() {
