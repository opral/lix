...
- 2026-03-11 21:07 PDT - completed the Phase 4 runtime collapse for public writes: `Sql2PreparedWrite` now carries an explicit tracked/untracked execution contract with schema registrations, append preconditions, semantic effects, and filesystem-payload handling; `query_runtime/shared_path.rs` now uses that sql2 artifact instead of recomputing liveness/effects from planner fragments and target-name checks; and the Phase 4 canaries passed across sql2 runtime units plus `commit`, `plugin_install`, `state_view`, and `entity_view`
- 2026-03-11 22:14 PDT - completed the Phase 5 legacy-bridge deletion pass: `query_runtime/preprocess.rs` now rejects public-surface batches structurally instead of sending them through `internal_state`, `internal_state/mod.rs` no longer contains public read routing/lowering logic, the old public read-only write checks were removed from `internal_state/canonical_write.rs`, `sql2::write_resolver` stopped calling `preprocess_with_surfaces_to_plan` for selector reads, and the Phase 5 guardrails plus `state_view`, `entity_view`, and the explicit transaction commit canary all passed
