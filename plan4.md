...
- 2026-03-12: Split `write_resolver` into a thin dispatcher plus child modules for the shared state-backed write flow (`write_resolver/state_backed_writes.rs`) and shared selector-query construction (`write_resolver/selector_queries.rs`), leaving admin writes in the parent and keeping filesystem writes in their extracted child module.
