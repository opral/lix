...
number. Packet v1 must not ship by rounding, coercing to binary floating point,
or rejecting an otherwise valid coefficient/exponent. The production breaking
change must first add a versioned durable JSON representation with a tagged
normalized-decimal node (sign, arbitrary-precision coefficient, arbitrary-
precision exponent), migrate/hash/compare it with the rules above, and pass the
same golden vectors through storage, conflict resolution, and every SDK. Until
that durable codec and migration exist, this WIT/packet is an executable
research contract rather than an implementable production ABI. If the project
instead chooses bounded existing Lix numbers, that is a new packet-version
decision with explicit range/equality rules, not an implicit v1 limitation.
