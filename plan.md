...
- 2026-03-10 12:02 PDT: Deleted the last migrated admin read-step wrappers by rewriting the legacy `lix_active_version`, `lix_active_account`, and `lix_version` row-loader helpers to parse a simple public `SELECT`, pass it through the shared `sql2` public-read rewrite boundary, and then lower/bind/execute that internal SQL directly; this removed the obsolete `lix_*_view_read.rs` modules without restoring another legacy read bridge.
- 2026-03-10 12:14 PDT: Deleted the dead legacy `state_pushdown.rs` module and the orphaned `build_effective_state_by_version_*` / target-version pushdown branch from `steps/vtable_read.rs`; those helpers were no longer on any live or test path after the `sql2` read cutover, so the legacy rewrite engine now carries less dormant read-shape machinery while keeping its remaining write-side helpers intact.
- 2026-03-10 12:26 PDT: Deleted another dead legacy helper batch by removing the unused AST compatibility wrappers (`default_alias`, `rewrite_query_with_select_rewriter`, `rewrite_table_factors_in_select`), dropping two unreferenced entity-target helper APIs, gating the rewrite-engine-only SQL parse/bind helpers to tests, and trimming the unused visible-column helper from `steps/state_columns.rs`; the remaining legacy rewrite engine is now narrower and more explicitly write-focused.
- 2026-03-10 12:58 PDT: Moved the remaining live nested-read bridge points onto the shared preprocess/sql2 seam by lowering entity-write subqueries and vtable INSERT ... SELECT materialization through `lower_public_read_query_with_sql2_backend(...)`, then dropped the last stale `rewrite_engine` reexport that only existed for the removed query-engine session cache.
- 2026-03-10 13:24 PDT: Deleted the entire dead `rewrite_engine/lowering/**` cluster and switched the remaining test-only preprocess helper to the shared AST lowerer, leaving one lowering implementation for live code while reducing the legacy rewrite-engine warning surface to the still-live query-engine and vtable-read remnants.
- 2026-03-10 14:07 PDT: Deleted the legacy read `query_engine` / query-phase rule stack and replaced it with a smaller top-level statement split: migrated public reads now go through `sql2`, internal `lix_internal_state_vtable` queries still rewrite through the dedicated internal vtable path, and the canonical write rule no longer owns `SELECT` / `EXPLAIN`; kept sqlite `entity_view`, sqlite `checkpoint`, sqlite `transaction_execution`, sqlite `filesystem_view`, and `sql_guardrails` green so the cleanup stays on the no-fallback `sql2` public path while preserving internal vtable reads until those callers are removed.
- 2026-03-10 14:21 PDT: Deleted the dead query-helper surface left behind by the query-engine removal by stripping the unused select/table walker/rewrite APIs and their obsolete tests from `rewrite_engine/ast_utils.rs`, removing the stale reexports from `rewrite_engine/mod.rs`, and tightening `pipeline.rs` to its still-live test-only imports; kept sqlite `sql_guardrails` and sqlite `entity_view` green so the remaining legacy rewrite engine is narrower and more explicitly focused on the still-live internal vtable and write-side helpers.
- 2026-03-10 15:12 PDT: Extended the shared `sql2` public-read relation rewriter so joined builtin entity surfaces like `lix_entity_label` and `lix_label` lower to internal derived queries instead of bouncing into whole-query canonicalization, then kept the checkpoint label mutation itself on the internal authoritative state table to match bootstrap seeding while the broader checkpoint/runtime cleanup continues; kept sqlite `checkpoint` green and added a lowerer regression for joined builtin entity-surface rewrites.
- 2026-03-10 16:02 PDT: Removed the remaining statement-level read bridge from the legacy rewrite engine by moving top-level `SELECT` / `EXPLAIN SELECT` handling into `sql/planning/preprocess.rs`, deleting the `QueryRead` / `ExplainRead` rule variants and their modules, and leaving the legacy statement pipeline with only canonical write rewriting plus passthrough; kept sqlite `checkpoint`, sqlite `filesystem_view`, and `sql_guardrails` green so migrated public reads now bypass the legacy statement-rule engine entirely while internal vtable reads still use the dedicated internal rewriter.
- 2026-03-10 16:19 PDT: Collapsed the remaining legacy statement-pipeline indirection by deleting the dead rule registry, removing the rule-dispatch helpers from `pipeline/rules/statement/mod.rs`, and making `StatementPipeline` call canonical write rewriting directly before passthrough; kept sqlite `checkpoint` and `sql_guardrails` green so the surviving legacy pipeline is now explicitly just “write canonicalization or passthrough” instead of pretending to be a multi-rule planner.
- 2026-03-10 16:33 PDT: Deleted the final `StatementPipeline` wrapper by exporting direct rewrite helpers from `rewrite_engine/mod.rs`, switching both production preprocess and the remaining write-side tests onto those helpers, and deleting `pipeline/statement_pipeline.rs`; kept sqlite `checkpoint` and `sql_guardrails` green so the last legacy statement-shell layer is gone and the surviving legacy path now enters canonical write rewriting directly.
- 2026-03-10 17:11 PDT: Deleted the thin canonical write-wrapper modules under `pipeline/rules/statement/canonical/**` by inlining their remaining normalization logic directly into `canonical/mod.rs` (entity backend insert evaluator/provider setup plus `vtable_write` update/delete conversion to `RewriteOutput`), leaving the legacy write shell with fewer indirection files while keeping behavior fixed on the still-live internal/vtable write path; kept sqlite `checkpoint`, sqlite `transaction_execution`, and `sql_guardrails` green.
- 2026-03-10 18:07 PDT: Moved another production write cut line onto `sql2` by making execution prep fail closed when a top-level writable public surface is semantically owned by the `sql2` catalog but `sql2` cannot prepare the write, then deleted the remaining legacy admin/state/version public-write step modules and stripped those branches out of the canonical write loop; restored only the internal `stored_schema` vtable helper because init/bootstrap still needs that authoritative internal rewrite, and kept sqlite `transaction_execution` plus `sql_guardrails` green so migrated public writes no longer depend on the legacy write shell while internal stored-schema bootstrap still works.
- 2026-03-10 18:31 PDT: Deleted the now-orphaned legacy entity-write rewrite cluster (`rewrite_engine/entity_views/**`) and the dead state-column helper module, then trimmed stale rewrite-engine reexports/helpers down to what the surviving internal vtable path still uses; updated the internal stored-schema helper to import mutation types directly instead of through the deleted reexports, and kept sqlite `transaction_execution` plus `sql_guardrails` green so the migrated public-write cleanup no longer carries the old entity rewrite stack as dead code.
- 2026-03-10 19:02 PDT: Deleted the duplicate rewrite-engine placeholder binder (`rewrite_engine/params.rs`) and rewired the remaining row-resolution/test-only callers onto the shared AST placeholder utilities, then trimmed the now-unused duplicate `resolve_insert_rows()` helper and leftover warning-only `mut` bindings from the surviving canonical write shell; kept sqlite `transaction_execution` plus `sql_guardrails` green so the last legacy placeholder-binding seam no longer exists beside `sql/ast/utils.rs` and `sql_shared/placeholders.rs`.
- 2026-03-10 19:24 PDT: Deleted the tiny read-only public-write step modules for `lix_change` and `lix_state_history` by inlining their remaining reject logic directly into the surviving canonical write loop, then removed those files and their step registrations so the legacy write shell carries fewer standalone surface-specific shims; kept sqlite `change_view` plus `sql_guardrails` green.
- 2026-03-10 19:46 PDT: Deleted the last `rewrite_engine/pipeline.rs` compatibility shim by moving its surviving rule/validator modules directly under `rewrite_engine`, switching `vtable_read` and `row_resolution` tests onto local/shared helpers, and cleaning up the stale error-catalog `include_str!` anchors that still referenced already-deleted legacy files; kept the internal-vtable lib tests and `sql_guardrails` green so the remaining rewrite-engine surface is now the direct internal vtable/bootstrap shell rather than a fake pipeline wrapper.
- 2026-03-10 20:03 PDT: Deleted the remaining duplicated helper files `rewrite_engine/ast_utils.rs` and `rewrite_engine/steps/inline_lix_functions.rs` by rehoming their tiny APIs directly into `rewrite_engine/mod.rs` or onto the shared planning helper path, leaving the surviving legacy rewrite engine with fewer free-floating utility files and a smaller contract surface; kept internal `vtable_write` lib tests plus `sql_guardrails` green.
- 2026-03-10 14:17 PDT: Deleted two more alias-only rewrite-engine layers by moving `RowSourceResolver` / resolved insert-row helpers into the shared AST utilities and folding `rewrite_engine/types.rs` into the surviving module root, so the remaining internal-vtable/bootstrap shell now depends less on local duplicate contracts while keeping internal `vtable_write` lib tests and `sql_guardrails` green on the sqlite-only loop.
- 2026-03-10 14:19 PDT: Collapsed the last legacy statement-shell wrapper around the rewrite engine by deleting `pipeline/rules/**` except for the canonical write module, moving passthrough/context/helper/outcome logic into `rewrite_engine/mod.rs` and `canonical/mod.rs`, and leaving the surviving legacy rewrite engine as a much smaller direct internal-vtable/bootstrap shell; kept internal `vtable_write` lib tests plus `sql_guardrails` green on the sqlite-only loop.
- 2026-03-10 14:23 PDT: Deleted two more wrapper layers around the surviving internal-vtable/bootstrap shell by folding the legacy rewrite validator into `rewrite_engine/mod.rs`, replacing `steps/mod.rs` with direct path modules, and leaving the remaining rewrite engine as just the module root plus the actual `stored_schema`, `vtable_read`, `vtable_write`, and canonical write implementation files; kept internal `vtable_write` lib tests plus `sql_guardrails` green on the sqlite-only loop.
- 2026-03-10 14:26 PDT: Flattened the last deep legacy path segment by moving the surviving canonical write implementation out of `pipeline/rules/statement/canonical/` into `rewrite_engine/canonical_write.rs`, updating the error-catalog anchors and guardrails to the new location, and leaving the remaining legacy rewrite engine as a flat shell over its real internal-vtable/bootstrap step implementations; kept internal `vtable_write` lib tests plus `sql_guardrails` green on the sqlite-only loop.
- 2026-03-10 14:34 PDT: Deleted the `rewrite_engine` production path entirely by rehoming the remaining internal-vtable/bootstrap implementation under `sql/planning/internal_vtable/**`, switching preprocess and production imports to that neutral shell, removing the old `rewrite_engine` directory, and updating guardrails/error anchors to lock in the rename; kept internal `vtable_read` lib tests plus `sql_guardrails` green on the sqlite-only loop.
- 2026-03-10 14:39 PDT: Rehomed the last execution-side legacy naming by moving `sql/execution/followup.rs` to `internal_vtable_followup.rs` and switching the runtime onto that neutral internal-vtable postprocess module, leaving the remaining non-`sql2` SQL path explicitly scoped to internal vtable/bootstrap behavior instead of legacy “followup” semantics; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 14:53 PDT: Rehomed the remaining internal-vtable/bootstrap shell out of `src/sql/**` entirely by moving its planner/runtime/postprocess pieces into the neutral top-level `internal_state/**` module, deleting the old `sql/planning/internal_vtable/**` directory and `sql/contracts/postprocess_actions.rs`, and wiring preprocess/execution/contracts/error anchors/guardrails to the new module so the last non-`sql2` SQL path is now explicitly internal-state/bootstrap code instead of a legacy SQL planner subsystem; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 14:58 PDT: Deleted another leftover planner adapter by inlining `StatementRewriteOutput` directly into `sql/planning/preprocess.rs` and removing `sql/planning/rewrite_output.rs`, so the remaining internal-state/bootstrap bridge no longer bounces through a separate rewrite-output shim before producing `PlannedStatementSet`; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 15:07 PDT: Moved another internal-vtable-only planner helper out of `sql/planning` by rehoming `materialize_vtable_insert_select_sources()` into `internal_state/materialize.rs`, then hid the last direct `vtable_read` calls behind `internal_state` entrypoints so `preprocess.rs` no longer reaches into internal-state submodules or keeps duplicate postprocess/internal-vtable helper logic; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 15:13 PDT: Rehomed the remaining explicit-transaction/vtable-insert script coalescer from `sql/planning/script.rs` into `internal_state/script.rs`, updated `api.rs`, `engine.rs`, `sql/scripts.rs`, and preprocess to use the neutral internal-state module, and removed another legacy planning file so the last non-`sql2` shell owns more of its own bootstrap helpers instead of leaking them through `sql/planning/**`; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 15:18 PDT: Collapsed the remaining `PostprocessPlan::Vtable*` execution branch out of `sql/execution/run.rs` into `internal_state/followup.rs`, so the SQL runtime now delegates internal-vtable postprocess execution as one neutral internal-state operation instead of matching legacy plan variants directly while still preserving cache-target and state-commit-stream derivation; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 15:25 PDT: Moved the last raw vtable-specific planner contract out of the old SQL types by wrapping `PostprocessPlan` inside `internal_state::InternalStatePlan`, updating `PlannedStatementSet` and planner invariants to treat internal-state postprocess as an internal-state extension rather than a generic SQL planner field, and keeping the runtime on the new wrapped contract while sqlite `transaction_execution` plus `sql_guardrails` stayed green.
- 2026-03-10 15:42 PDT: Collapsed the remaining internal-state prepare branch out of sql/planning/preprocess.rs by moving top-level public-read lowering, internal-state read/write rewrite selection, placeholder binding/rendering, insert-select materialization, and transaction insert coalescing behind new internal_state prepare entrypoints; kept sqlite transaction_execution and sql_guardrails green so preprocess is now a thin API layer over sql2 plus the neutral internal-state shell instead of owning vtable/bootstrap orchestration.
- 2026-03-10 15:48 PDT: Trimmed the last obvious sql-shell wrappers by moving script coalescing/binding behind internal_state::script::prepare_statement_script_sql_statements() and collapsing run.rs onto internal_state execution helpers that handle both plain prepared execution and postprocess transactions; kept sqlite transaction_execution and sql_guardrails green so the remaining sql runtime is thinner and delegates internal-state execution as one neutral subsystem.
- 2026-03-10 17:12 PDT: Rehomed the generic SQL runtime contract layer out of src/sql by moving planner/execution contracts into a new query_runtime module and switching shared_path, run, internal_state, validation, commit runtime, and semantic planning helpers onto that neutral namespace; started the side-effect rehome by moving the old sql/side_effects engine-method implementation under engine as runtime_effects without widening engine visibility, and kept sqlite transaction_execution plus sql_guardrails green so sql/contracts and sql/planning/plan.rs are no longer the live outer-shell contract surface.
- 2026-03-10 17:41 PDT: Deleted the first batch of obsolete legacy planner-structure coverage by removing engine-unit tests and one transaction integration case that only pinned the old multi-statement postprocess fallback, plus the matching string-match guardrail and internal-state validator tests that only asserted vtable postprocess shape rather than semantic behavior; kept sqlite transaction_execution plus sql_guardrails green so the remaining coverage is more focused on runtime semantics than the removed planner shell.
- 2026-03-10 18:05 PDT: Deleted the pure read-side internal-vtable rewrite unit tests from `internal_state/vtable_read.rs`, since they only asserted exact lowered SQL fragments, pushdown placement, and plugin-key extraction shapes for the shrinking legacy internal-vtable shell rather than user-visible semantics; kept sqlite transaction_execution plus sql_guardrails green so the semantic integration suites remain the source of truth while the old rewrite-shape coverage is removed.
- 2026-03-10 18:17 PDT: Deleted the remaining stored-schema rewrite unit tests that only pinned bootstrap/internal insert-shape details in `internal_state/stored_schema.rs`, leaving stored-schema behavior covered by the sqlite integration/runtime path instead of exact internal rewrite-row expectations; the sqlite compile loop remained green through transaction_execution plus sql_guardrails.
- 2026-03-10 18:36 PDT: Moved the remaining generic execution-plan derivation helpers (`dependency_spec`, `derive_effects`, `derive_requirements`, and `invariants`) out of `src/sql/planning` into `query_runtime`, updated `query_runtime::plan`, observe, and shared-path execution wiring to use the neutral module, and removed those module exports from the old planning shell; kept sqlite transaction_execution plus sql_guardrails green so another chunk of planner/runtime ownership now lives outside `src/sql/**`.
- 2026-03-10 19:08 PDT: Moved the preprocess entrypoints out of `src/sql/planning/preprocess.rs` into `query_runtime/preprocess.rs`, rewired runtime/plugin/filesystem/deterministic callers to that neutral module, and removed `preprocess` from the legacy `sql/planning` module surface; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 19:09 PDT: Moved plan execution and statement-script execution façades out of `src/sql/execution/run.rs` and `src/sql/scripts.rs` into `query_runtime/execute.rs` and `statement_scripts.rs`, updated API/shared-path/transaction callers, and trimmed the old `sql/mod.rs` / `sql/execution/mod.rs` exports so more of the runtime shell now lives outside `src/sql/**`; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 19:27 PDT: Deleted another obsolete batch of planner-structure guardrails that only pinned the moved preprocess/script façade layout instead of semantic behavior, leaving the remaining guardrails focused on preventing reintroduction of legacy runtime/rewrite bridges rather than freezing the new module layout.
- 2026-03-10 19:46 PDT: Rehomed the last helper files out of `src/sql/planning/**` by moving parse/lower helpers into `query_runtime` and the placeholder-binding / inline-function / predicate-normalization helpers into `internal_state`, then deleted the now-empty `src/sql/planning` module surface; kept sqlite `transaction_execution` plus `sql_guardrails` green so another whole legacy shell directory is gone instead of merely thinned.
- 2026-03-10 20:38 PDT: Moved the remaining generic execution helpers out of `src/sql/execution/**` by rehoming prepared-statement execution and intent derivation into `query_runtime`, moving post-commit/runtime SQL effect application into neutral runtime modules, and deleting the old `sql/execution` helper files so only the transitional shared-path shim remains under `src/sql/**`; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 16:44 PDT: Moved the last runtime-facing `sql` entrypoints out of `src/sql/**` by rehoming the shared-path shim into `query_runtime/shared_path.rs`, moving the transaction and in-transaction engine modules out of `src/sql/`, deleting `src/sql/execution/**`, and shrinking `src/sql/mod.rs` down to generic SQL namespaces only; kept sqlite `transaction_execution` plus `sql_guardrails` green.
- 2026-03-10 16:50 PDT: Rehomed the last generic `src/sql/**` namespaces by moving `sql/ast`, `sql/history`, `sql/semantics`, and `sql/storage` into `sql_ast`, `query_history`, `query_semantics`, and `query_storage`, deleting `src/sql/mod.rs` and the empty `src/sql/` directory, and updating the remaining runtime/materialization/internal-state callers to the new neutral module names; kept sqlite `transaction_execution` plus `sql_guardrails` green so the old top-level `sql` shell is now gone entirely.
- 2026-03-10 16:52 PDT: Deleted the dedicated `vtable_read.rs` and `vtable_write.rs` integration suites now that the old `src/sql/**` shell is gone, since those tests only pinned internal-state vtable mechanics instead of the public `sql2` semantic contract; kept sqlite `transaction_execution` plus `sql_guardrails` green so the remaining coverage continues to focus on live runtime semantics instead of obsolete internal vtable behavior.
- 2026-03-10 16:56 PDT: Deleted the remaining `internal_state` rewrite-shape unit tests in `internal_state/vtable_write.rs` plus the small validator/empty test modules in `internal_state/mod.rs` and `internal_state/canonical_write.rs`, keeping only live sqlite semantic/runtime coverage instead of exact internal rewrite expectations for the shrinking internal-state shell; kept sqlite `transaction_execution` plus `sql_guardrails` green.
